---
paths:
  - ".pipeline/**/*.md"
  - "watcher_core.py"
  - "watcher_dispatch.py"
  - "watcher_prompt_assembly.py"
  - "watcher_state.py"
  - "verify_fsm.py"
  - "pipeline_runtime/**/*.py"
  - "pipeline-launcher.py"
  - "controller/**/*.py"
  - "controller/**/*.html"
  - "scripts/pipeline_runtime_gate.py"
---

# Pipeline Runtime Rules

- Persistent truth lives in `work/` and `verify/`; `.pipeline/` files are rolling control slots.
- Claude executes `.pipeline/implement_handoff.md` only when Claude is the active implement owner and the newest valid control has `STATUS: implement`.
- Treat `.pipeline/claude_handoff.md`, `.pipeline/gemini_request.md`, and `.pipeline/gemini_advice.md` as read-only compatibility aliases for role-based controls, not separate execution slots.
- Do not answer `.pipeline/operator_request.md`, `.pipeline/advisory_request.md`, or `.pipeline/advisory_advice.md` as if they were implement execution slots.
- If the assigned handoff is blocked, stale, already implemented, or contradicted by the latest `work/` + `verify/`, emit the exact `implement_blocked` sentinel and stop rather than improvising a new slice.
- If a `needs_operator` slot only presents labeled choices, such as lettered, numbered, inline parenthesized, or Korean `n안` options, that current docs, milestones, and latest `work/` + `verify/` can resolve, send it through advisory-first triage before waiting on the operator. Keep safety, destructive, auth/credential, approval-record, and truth-sync blockers in the decision header as real stops.
- If watcher prompts you for operator retriage, write one newer control slot. An idle return with no next control is treated as `operator_retriage_no_next_control` and watcher may escalate the same gated request to Gemini.
- In Codex-only or `ADVISORY_DISABLED` retriage, `commit_push_bundle_authorization + internal_only` and `PUBLISH_HELD: true` mean hold the publish backlog by default; do not commit, push, publish a branch/PR, create/reuse a PR, or merge from that prompt, and write the next safe non-publish local control instead.
- When a verify/retriage prompt includes `RUNTIME_STATUS_AT_DISPATCH`, treat that dispatcher surface as the runtime-liveness authority over lane-local `status --json`, `doctor --json`, or tmux access conflicts.
- Treat stale redispatch seriously: if the same `HANDOFF_SHA` is already closed by the latest persistent notes, report `BLOCK_REASON_CODE: already_implemented`.
- Do not hardcode current branch names, commit SHAs, `CONTROL_SEQ` values, pane ids, Korean display labels, exact operator prose, or one-off control-file bodies in runtime logic. Use structured metadata, shared parsers, schema helpers, status-label helpers, and fixtures.
- Do not duplicate near-copy watcher/supervisor/launcher/controller truth logic. Move shared truth to the owning module or `pipeline_runtime/*` helper and keep clients thin.
- Do not keep adding unrelated branches to a large function or file. Extract parsing, labeling, control writing, lane-surface, or event-contract responsibilities when concentration makes the next fix harder.
- For runtime / launcher work, prefer focused incident replay and live stability checks over rerunning long soak by default.
- Only reopen broad soak expectations when supervisor / watcher / tmux adapter / wrapper-event contracts changed, state-writer contracts changed, or an adoption-final gate is explicitly required.
- When stop / degraded / dispatch incidents change, keep the surface truthful in runtime status, launcher recent log, and controller-visible state; do not hide a repeated stall behind generic READY / WORKING copy.
- Stale handoff gate: `_duplicate_control_marker()` blocks any `STATUS: implement` handoff whose `control_updated_at` predates the current supervisor session start (`self.started_at`), has no artifact truth (`completed_implement_handoff_truth()` returns None), and carries no `REISSUE: true` marker. This prevents old handoffs from auto-dispatching on restart. To allow intentional re-dispatch, add `REISSUE: true` to the handoff. `start_runtime=False` supervisors (status-only) are exempt.
- Wrapper-events are the primary lane status truth channel. `TASK_ACCEPTED`, `TASK_DONE`, and `BROKEN` events from `wrapper_events.py` determine lane state before pane text is read. Pane text (`tail_surface`) is a fallback used only when no decisive wrapper event exists (`wrapper_status_decisive = False`). Do not widen pane text heuristics while wrapper event coverage is sufficient.
- `pane_text_fallback_used` is observability telemetry, not a failure signal. Run-level normal range for Codex: 2–4 events covering `BOOTING`/`READY` initial surface. If this event appears after `TASK_ACCEPTED` or during `WORKING` state maintenance, investigate structured output or wrapper bridge coverage. Never include raw pane text in the event payload.
