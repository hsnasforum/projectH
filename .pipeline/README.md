...
- wrapper는 active task hint가 살아 있는 동안 prompt-visible + no-busy 상태가 짧게 settle되더라도 이를 `READY` 관찰로만 남깁니다. `TASK_DONE`은 matching task hint가 해제되거나 산출물/receipt 기반 terminal evidence가 있을 때만 유효하며, prompt-visible alone으로 verify/implement task를 닫으면 안 됩니다.
...
- current run의 `VERIFY_PENDING` job이 `dispatch_seen_missing` 또는 `task_accept_missing` stage에 있으면 non-degraded 상태에서는 `automation_health=recovering`, `automation_reason_code=dispatch_stall`, `automation_next_action=retrying`으로 보이는 편이 맞습니다. 같은 fingerprint 반복으로 `degraded_reason=dispatch_stall`이 붙은 뒤에만 `attention + verify_followup`으로 승격합니다.
- public status가 `turn_state=IDLE`로 내려왔더라도 current `active_round.state`가 `VERIFY_PENDING` 또는 `VERIFYING`이면 plain `ok/continue`가 아닙니다. non-degraded 상태에서는 launcher health를 `automation_health=recovering`, `automation_reason_code=dispatch_stall`, `automation_next_action=retrying`으로 표면화해 active verify work를 숨기지 않습니다.
...
- Codex pane에 dispatch 전부터 `[Pasted Content ...]` 미제출 paste 잔상이 보이면 watcher는 새 prompt로 덮어쓰거나 literal fallback으로 재주입하지 않고 fail-closed 합니다. watcher가 새로 paste한 fresh prompt가 첫 `Enter` 뒤에도 그대로 보일 때만 Codex TUI submit 호환성 보강으로 `C-j`를 한 번 더 보냅니다. 그 뒤에도 잔상이 남으면 cleanup을 먼저 확인하고, 입력줄이 비워진 경우에만 one-line literal fallback을 한 번 사용합니다. cleanup 또는 fallback이 실패하면 `dispatch_failed_submit` + `codex_verify_dispatch_failure_loop` 또는 `codex_stale_paste_blocked`로 멈춥니다.
- Codex verify prompt 자체가 tmux lane에 submit되지 못하는 실패가 retry budget을 소진하면 watcher는 `dispatch_failed_submit` stage와 `degraded_reason=codex_verify_dispatch_failure_loop`을 남기고 추가 자동 재주입을 멈춥니다. launcher health는 이를 `automation_health=needs_operator`, `automation_next_action=operator_required`로 보여야 합니다.
...
- Gemini pane의 `Allow execution of [git]?` permission prompt는 visible `Shell` command가 read-only git 조회(`branch`, `log`, `status`, `show`, `diff`, `rev-parse` 등)로만 구성된 경우에 한해 watcher가 `2. Allow for this session`을 입력해 막힘을 풀 수 있습니다. non-git 또는 `git push/reset/checkout/commit` 같은 write/publication command는 자동 승인하지 않으며, 자동 입력 시 `gemini_git_permission_auto_allow_session` event를 남깁니다.
- advisory turn이 recovery/advice로 이미 넘어갔거나 현재 turn이 더 이상 advisory가 아닌데 Gemini pane이 `ROLE: advisory` / `.pipeline/harness/advisory.md` 프롬프트에서 계속 `esc to cancel` busy를 보이면 watcher는 좁은 grace 뒤 `Escape`로 stale advisory를 취소하고 `advisory_lane_cancelled` event를 남깁니다. active `request_open` advisory 자체는 이 guard가 취소하지 않으며 기존 `advisory_recovery`가 맡습니다.
- no-silent-stall contract상 자동화가 멈추거나 주의 상태로 내려가면 supervisor status/events에는 최소 `automation_reason_code`, `automation_incident_family`, `automation_next_action` 중 하나가 남아야 합니다. canonical incident family는 `signal_mismatch`, `dispatch_stall`, `completion_stall`, `duplicate_handoff`, `waiting_next_control`, `operator_retriage_no_next_control`, `stale_control_advisory`, `idle_release_pending`, `local_socket_guard_auto_held`, `lane_recovery_exhausted`, `session_recovery_exhausted`입니다.
- supervisor와 watcher runtime exporter는 launcher용 derived field `automation_health = ok|recovering|attention|needs_operator`, `automation_reason_code`, `automation_incident_family`, `automation_next_action = continue|retrying|advisory_followup|verify_followup|operator_required|pr_boundary`, `control_age_cycles`, `stale_control_seq`, `stale_control_cycle_threshold`와 profile `runtime_controls`를 `status.json`에 함께 씁니다. `stale_control_seq=true`는 같은 active `CONTROL_SEQ`가 오래 이어졌다는 surface입니다. advisory grace까지 지나면 `automation_health=attention`, `automation_reason_code=stale_control_advisory`로 승격해 launcher가 이를 정상 진행으로 숨기지 않습니다. advisory가 활성화된 profile은 `automation_next_action=advisory_followup`, 현재 Codex-only처럼 `advisory_enabled=false`인 profile은 `automation_next_action=verify_followup`으로 표면화합니다. 같은 health/reason/action 조합이 새로 승격되면 `events.jsonl`에는 `automation_incident` event를 남깁니다.
- runtime launch 중 local tmux socket permission denial이 발생하면 raw detail은 `logs/launch-error.log`에 보존하되 canonical reason은 `local_socket_guard_auto_held`로 표면화합니다. 이 reason은 local environment-held 상태이므로 `BROKEN` snapshot에서도 `automation_health=attention`, `automation_next_action=verify_followup`으로 라우팅하고, generic `runtime_launch_failed:RuntimeError`처럼 즉시 operator stop으로 과분류하지 않습니다.
- completed duplicate handoff나 active control 없이 lane note가 `waiting_next_control`로 남은 상태는 plain idle이 아닙니다. public `control=none`, `turn_state=IDLE`, `progress={}` 정리는 유지하되 launcher health는 각각 `duplicate_handoff` 또는 `waiting_next_control` + `automation_next_action=verify_followup`으로 보여 다음 control 정리가 필요하다는 사실을 숨기지 않습니다. 이 상태에서 최신 `/work`와 matching `/verify`가 이미 있고 오래된 verify job만 남아 있으면 public `active_round`도 비워 controller가 예전 `VERIFYING`을 현재 진행으로 오해하지 않게 합니다.
- `m84_publish_bundle_authorization` 같은 milestone별 publish authorization reason은 `commit_push_bundle_authorization`으로 정규화합니다. `publish_authorization` decision class도 `release_gate`로 정규화해 internal publish follow-up이 `hibernate`로 빠지지 않고 verify/handoff triage에 남게 합니다.
- active `implement_handoff`가 남아 있는데 `IMPLEMENT_ACTIVE` 구현 lane이 오래 `READY/prompt_visible` 또는 `READY/closed`로 돌아왔거나, `implement_idle_timeout` 뒤 same handoff retry cooldown에 들어간 상태는 정상 진행이 아닙니다. supervisor health는 전자를 `automation_health=attention`, `automation_reason_code=implement_active_idle`, `automation_next_action=retrying`으로, 후자를 `automation_health=recovering`, `automation_reason_code=idle_release_pending`, `automation_next_action=retrying`으로 보여 launcher가 이를 `ok/continue`로 숨기지 않습니다.
...
- `commit_push_bundle_authorization`은 operator가 이미 큰 검증 묶음을 publish 대상으로 승인한 뒤의 자동화 follow-up reason입니다. `OPERATOR_POLICY: internal_only`와 함께 오면 unattended hibernate가 아니라 verify/handoff-owner triage(`routed_to=verify_followup`)로 보내며, 작은 local slice의 dirty state를 자동 publish하라는 뜻은 아닙니다. 다만 `authorize ... or explicitly hold publication`처럼 승인과 보류를 동시에 묻는 문구는 승인 완료가 아니라 publish backlog 후보로 보고, Codex 단독 retriage prompt에서는 commit/push/PR을 실행하지 않고 다음 non-publish local control을 쓰게 합니다.
...
- 오래 남은 release-gate compatibility header는 `pipeline_runtime.operator_autonomy.resolve_operator_control(...)`에서 canonical metadata로 정규화합니다. `mNN_commit_push_milestones_doc_sync` 같은 milestone commit/push/doc-sync reason이나 `bN_release_gate_commit_authorization_dirty_tree`, `publish_boundary_accumulated_dirty_tree`, `dirty_bundle_publication_or_hold_decision`, `OPERATOR_POLICY: pr_creation_gate + commit_push_bundle_authorization`, `OPERATOR_POLICY: operator_only_publication_boundary`, `DECISION_CLASS: publication`, `publication_or_hold`, `publish_bundle_authorization`, 또는 `commit_publish_authorization`은 `commit_push_bundle_authorization + internal_only + release_gate`로 해석해 verify/handoff-owner follow-up에 보냅니다. `pr_creation_gate + gate_24h + release_gate`처럼 canonical token이 섞인 compound policy도 같은 shared resolver가 읽습니다. 알 수 없는 publish label이나 구조화 metadata 누락은 계속 fail-closed immediate `needs_operator`이며, 외부 publication 실행으로 해석하지 않습니다.
...
- `pr_merge_completed` recovery 뒤의 기존 PR merge backlog는 완료되었거나 local publish backlog로 보류된 사실 자체가 next-slice evidence일 뿐, implement lane에 commit/push/PR/merge 작업을 넘기는 근거가 아닙니다. 이후 verify/retriage가 docs-only local guard closeout만 반복하게 되면 같은 narrow guard handoff를 한 번 더 쓰지 말고, 남은 drift를 한 번에 닫는 bounded docs bundle 또는 실제 operator-only boundary 중 하나로 수렴하는 편이 맞습니다.
- `auth_login_required`와 credential/auth repair 계열은 agent가 좁힐 수 있는 release-gate follow-up이 아니므로 `gate_24h`가 붙어도 즉시 operator-visible 경계로 남깁니다.
- 이 publish follow-up은 implement handoff로 넘기지 않습니다. implement prompt는 commit/push/branch publish/PR을 계속 금지합니다. 현재 Codex-only local profile에서는 publish backlog를 보류하고 다음 local slice control을 쓰는 것이 기본이며, advisory가 활성화된 별도 profile에서만 verify/handoff owner가 advisory escalation을 선택할 수 있습니다.
...
- supervisor status는 legacy top-level fields를 유지하면서 `runtime_snapshot`을 함께 제공합니다. `runtime_snapshot`은 `control_state`, `round_state`, `active_lane`, `health`, `queue`, `lanes`, `invariants.violations`를 담는 consumer-facing contract이며, controller/launcher는 Queue 표시를 이 snapshot에서 먼저 읽고 legacy fields는 fallback/debug로만 씁니다.
...
- readiness 판정은 최근 tail 안에서 마지막 busy marker 이후 입력 prompt가 다시 보이면 prompt-ready를 우선하되, 현재 진행형 `Waiting for background terminal`, `thinking with ...`, `esc to interrupt/cancel` 같은 active busy marker가 같은 tail에 남아 있으면 busy가 유지됩니다. 과거형 `Waited for background terminal` 완료 로그 뒤에 `❯` / `›` / `>` prompt가 보이면 prompt-ready로 내려가는 편이 맞습니다.
...
- `verify_operator_retriage`처럼 `require_active_control=false`인 control-resolution prompt도 `expected_control_path` / `expected_control_slot` / `expected_control_seq`가 있으면 dispatch 직전에 current active control과 다시 대조합니다. 더 높은 `CONTROL_SEQ`의 `safety_stop + immediate_publish` operator stop이 들어온 뒤에는 이미 queued 또는 stale prompt가 같은 `.pipeline/operator_request.md` 경로를 가리키더라도 `control_seq_drift`로 drop하고 Codex pane에 다시 paste하지 않는 편이 맞습니다.
...
- `pane_text_fallback_used`는 failure가 아니라 observability telemetry입니다. run당 2~4회 정도의 Codex `BOOTING`/`READY` 초기 fallback은 허용 범위로 보고, `TASK_ACCEPTED` 이후 같은 lane에서 반복되거나 `WORKING` 유지가 pane fallback으로 이어질 때만 구조화 출력/bridge 보강 후보로 봅니다. raw pane text는 이 event payload에 남기면 안 됩니다.
...
- 다음 `CONTROL_SEQ`를 계산할 때는 `STATUS: superseded`가 된 control slot의 번호도 포함해 단조 증가를 유지합니다. superseded advisory request를 복구한 직후 startup verify가 같은 번호를 재사용하면 current follow-up/advisory pane truth가 다시 헷갈릴 수 있기 때문입니다.
...
- `STATUS: request_open`이 오래 닫히지 않고 같은 `CONTROL_SEQ`의 current `advisory_advice.md`가 없으면 watcher는 operator stop을 열지 않고 `advisory_recovery` event를 남긴 뒤 verify/handoff owner에게 recovery prompt를 보낼 수 있습니다. 기본 recovery window는 약 5분(`advisory_recovery_sec`, 기본 300초)이며, visible advisory busy indicator가 더 오래된 것으로 보이면 그 나이를 우선합니다. 이 prompt는 `.pipeline/advisory_advice.md`를 대필하지 않고, 기존 request/work/verify를 읽어 더 높은 `CONTROL_SEQ`의 `.pipeline/implement_handoff.md`, 더 좁은 `.pipeline/advisory_request.md`, 또는 실제 operator boundary인 `.pipeline/operator_request.md` 중 하나만 쓰게 합니다. recovery된 같은 stale advisory request는 다시 열지 않고, 새 advisory가 꼭 필요할 때만 더 좁은 새 근거를 요구합니다. 같은 recovery chain에서 두 번째 stale advisory recovery가 발생하면 watcher는 `ADVISORY_FOLLOWUP_ALLOWED: false`를 전달해 verify/handoff owner가 또 다른 `.pipeline/advisory_request.md`를 열지 않고 implement handoff 또는 real operator boundary 중 하나로 수렴하게 합니다.
...
- watcher가 `routed_to=verify_followup` gated operator retriage를 verify/handoff owner에게 이미 보냈고, verify lane이 다시 prompt-ready idle이 됐는데도 더 높은 `CONTROL_SEQ`의 `implement_handoff.md`, `advisory_request.md`, `operator_request.md`가 생기지 않았다면, 같은 stop에 재고정하지 않고 `operator_retriage_no_next_control` incident를 남깁니다. `advisory_enabled=true`이면 `.pipeline/advisory_request.md`를 다음 `CONTROL_SEQ`로 machine-write해 advisory-first arbitration으로 승격할 수 있고, `advisory_enabled=false`인 Codex-only profile이면 advisory slot을 쓰지 않고 Codex verify lane에 `ADVISORY_DISABLED: true`, `PUBLISH_HELD: true` retriage prompt를 다시 보내 `verify_followup_no_next_control`로 표면화합니다. `approval_required` / `safety_stop` / `truth_sync_required` / 아직 merged로 확인되지 않은 `pr_merge_gate`처럼 실제 승인·안전·publication 경계가 남은 stop은 이 승격 대상이 아니라 즉시 operator wait current truth로 남습니다.
- `ADVISORY_DISABLED: true` retriage가 `pr_merge_completed` 뒤 같은 PR의 metadata-only `/work` 또는 docs-only local guard만 계속 만들고 있으면, runtime truth는 멈춘 것이 아니라 next control 선택이 과도하게 좁아진 상태입니다. 이 경우 verify/handoff owner는 advisory slot을 쓰지 않고 `implement_handoff.md`를 하나 써서 현재 docs/control truth를 정리하는 bounded bundle로 닫거나, 실제 남은 merge/release/destructive publication boundary가 있을 때만 `operator_request.md`를 새로 씁니다.
...
- supervisor/watcher는 operator stop publish/gate를 `OPERATOR_POLICY` 우선, `REASON_CODE` 다음, 설명 prose 마지막 참고 순서로 판정합니다. 구조화 metadata가 없거나 알 수 없으면 fail-closed로 즉시 `needs_operator` operator stop을 surface하며, 외부 publication 실행으로 해석하지 않는 편이 맞습니다.
- runtime은 오래 남은 control slot compatibility를 위해 `OPERATOR_POLICY: stop_until_exact_slice_selected`를 `gate_24h`로, `REASON_CODE: gemini_axis_switch_without_exact_slice`를 `slice_ambiguity`로 정규화해 false immediate publish를 막습니다. launcher 종료 뒤 다음 우선순위를 묻는 `REASON_CODE: next_direction_after_launcher_close`, `OPERATOR_POLICY: direction_selection_after_feature_complete`, `DECISION_CLASS: milestone_direction`도 operator stop이 아니라 `slice_ambiguity + gate_24h + next_slice_selection`으로 낮춰 verify/handoff follow-up을 먼저 태웁니다. release-gate compatibility도 같은 helper에서 `commit_push_bundle_authorization` / `pr_creation_gate` / `release_gate` canonical metadata로 낮춥니다. 새 slot은 canonical 값(`gate_24h`, `slice_ambiguity`, `commit_push_bundle_authorization`, `pr_creation_gate`, `release_gate`)을 그대로 쓰는 편이 맞습니다.
...
6. advisory owner가 current advisory report path의 advisory log와 `.pipeline/advisory_advice.md`에 `STATUS: advice_ready`를 남깁니다. `report/gemini/`는 role-neutral report path가 생기기 전까지 historical compatibility path로 유지합니다.
...
- Advisory round도 pane-only answer로 닫지 않습니다. current advisory report path의 advisory log와 `.pipeline/advisory_advice.md` recommendation slot이 둘 다 있어야 round가 완료된 것으로 봅니다.
...
- pane 1: Claude Code - disabled in the current Codex-only local profile unless active `role_bindings` assign it a role
- pane 2: Codex CLI - implement and verify/handoff owner lane in the current Codex-only local profile
...
- active profile이 깨져 있거나, implement/verify owner가 비어 있거나, 둘 중 하나가 `Claude`/`Codex` 밖이면 tmux session을 띄우기 전에 fail-closed로 종료합니다. 이 blocked auto-triage helper는 두 개의 shim lane이 필요한 smoke라서 current self-verify profile(`implement=Codex`, `verify=Codex`)에서는 explicit distinct-owner override로 실행합니다.
...
- topology 기본값 `PIPELINE_SMOKE_TOPOLOGY=swapped`는 catalog default를 따르지만, 현재 운영 profile은 `selected_agents=["Codex"]`, `implement=Codex`, `verify=Codex`, `advisory=""`, `advisory_enabled=false`, `session_arbitration_enabled=false`입니다. Claude advisory는 별도 active profile/override가 명시된 경우에만 사용합니다.
- 필요하면 `PIPELINE_SMOKE_ROLE_BINDINGS_JSON='{\"implement\":\"Claude\",\"verify\":\"Codex\",\"advisory\":\"Gemini\"}'` 같이 explicit override를 줄 수 있습니다. blocked auto-triage smoke처럼 distinct implement/verify lane이 필요한 경우 이 override를 사용합니다.
- 성공 기준:
  - `advisory_advice.md`가 `STATUS: advice_ready`
  - current advisory report path에 advisory log 생성
