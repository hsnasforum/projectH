...
- 이 publish follow-up은 implement handoff로 넘기지 않습니다. implement prompt는 commit/push/branch publish/PR을 계속 금지합니다. Codex-only 또는 `ADVISORY_DISABLED` profile에서는 publish backlog를 보류하고 다음 local slice control을 쓰는 것이 기본이며, advisory가 활성화된 profile에서는 verify/handoff owner가 advisory escalation을 선택할 수 있습니다.
...
- pane 1: Claude Code - verify/handoff owner lane in the current Codex+Claude two-agent local profile
...
- active profile이 깨져 있거나, implement/verify owner가 비어 있거나, 둘 중 하나가 `Claude`/`Codex` 밖이면 tmux session을 띄우기 전에 fail-closed로 종료합니다. 이 blocked auto-triage helper는 두 개의 shim lane이 필요한 smoke라서 self-verify profile(`implement=Codex`, `verify=Codex`)에서는 explicit distinct-owner override로 실행합니다.
...
- topology 기본값 `PIPELINE_SMOKE_TOPOLOGY=swapped`는 catalog default를 따르지만, 현재 운영 profile은 `selected_agents=["Codex","Claude"]`, `implement=Codex`, `verify=Claude`, `advisory=Codex`, `advisory_enabled=true`, `session_arbitration_enabled=false`입니다. Gemini advisory는 별도 active profile/override가 명시된 경우에만 사용합니다.
