...
- 오래 남은 release-gate compatibility header는 `pipeline_runtime.operator_autonomy`에서 canonical metadata로 정규화합니다. `mNN_commit_push_milestones_doc_sync` 같은 milestone commit/push/doc-sync reason이나 `bN_release_gate_commit_authorization_dirty_tree`, `OPERATOR_POLICY: pr_creation_gate + commit_push_bundle_authorization`, `DECISION_CLASS: publication` 또는 `commit_publish_authorization`은 `commit_push_bundle_authorization + internal_only + release_gate`로 해석해 verify/handoff-owner follow-up에 보냅니다. `pr_creation_gate + gate_24h + release_gate`처럼 canonical token이 섞인 compound policy도 같은 shared evaluator가 읽습니다. 알 수 없는 publish label이나 구조화 metadata 누락은 계속 fail-safe immediate `needs_operator`입니다.
