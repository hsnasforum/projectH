...
- Codex pane에 `[Pasted Content ...]` 미제출 paste 잔상이 보이면 watcher는 새 prompt로 덮어쓰거나 `C-j` / literal fallback으로 재주입하지 않고 fail-closed 합니다. verify dispatch에서는 `dispatch_failed_submit` + `codex_verify_dispatch_failure_loop`로 operator-visible incident를 남기고, control-resolution pending prompt는 `codex_stale_paste_blocked`로 보류합니다.
