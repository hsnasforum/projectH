...
for raw in sys.stdin:
    ...
    if stripped.startswith("BLOCK_ID: ") and not sentinel_sent and handoff and handoff_sha:
        sys.stdout.write("STATUS: implement_blocked\n")
        sys.stdout.write("BLOCK_REASON: smoke_handoff_blocked\n")
        sys.stdout.write("REQUEST: codex_triage\n")
        sys.stdout.write(f"HANDOFF: {handoff}\n")
        sys.stdout.write(f"HANDOFF_SHA: {handoff_sha}\n")
        sys.stdout.write(f"BLOCK_ID: {handoff_sha}:smoke_handoff_blocked\n")
        sys.stdout.flush()
        sentinel_sent = True
        prompt()
        continue
