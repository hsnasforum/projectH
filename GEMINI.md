...
owner로 동작합니다. 현재 로컬 launcher profile은 Codex+Claude two-agent
profile(`selected_agents=["Codex","Claude"]`, `verify="Claude"`,
`advisory="Codex"`, `advisory_enabled=true`)이므로, Gemini는 다시 선택되고
advisory에 바인딩될 때만 이 파일을 실행 role memory로 봅니다.
