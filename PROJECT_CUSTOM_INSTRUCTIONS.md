...
- commit/push/PR 생성 자동화는 큰 검증 묶음 경계에서만 다룹니다. operator가 명시 승인한 release, soak, PR stabilization, direct publish bundle이 아니면 small/local slice는 `/work` closeout과 local dirty state로 끝내고 commit/push/PR operator stop을 새로 열지 않습니다.
- `pr_creation_gate + gate_24h + release_gate`는 승인된 큰 묶음의 draft PR 생성 follow-up이므로 operator에게 다시 묻지 않고 verify/handoff owner가 PR을 생성하거나 기존 PR을 재사용하고, PR URL을 `/work`에 기록한 뒤 다음 control을 씁니다. 이미 merge 승인 대기 중인 오래된 draft PR이 있으면 그 pending merge 후보는 기본적으로 더 이상 덧대지 않고 안정적으로 유지합니다. 이 경우 다음 검증 묶음은 parent branch를 base로 하는 stacked child branch/PR로 publish하고, parent/child 연결 정보를 `/work`에 남긴 뒤 parent merge 후 child PR의 base를 저장소 기본 base branch로 다시 맞춥니다. `pr_merge_gate + internal_only + merge_gate`는 merge 승인 대기 backlog로 남기되 local 구현을 막지 않으며, verify/handoff owner가 merge를 직접 실행하지 않고 다음 안전한 local control을 씁니다. auth/credential, destructive publication change, approval-record/truth-sync blocker, external publication boundary는 여전히 operator stop입니다.
...
- 이 commit/push/PR follow-up이나 merge 실행을 implement handoff로 넘기지 않습니다. implement lane은 여전히 commit, push, branch/PR publish, merge 실행이 금지되어 있으므로 verify/handoff owner가 직접 처리하거나, 직접 처리할 수 없으면 advisory로 넘겨야 합니다. merge 대기 PR이 있어도 새 local slice는 별도 control로 계속 진행할 수 있습니다.
