...
3. browser folder picker search flow with `선택 결과 요약` source-type label and multi-source count-based metadata (`출처 2개`) in both quick-meta and transcript meta, plus response detail preview panel alongside summary body with first-card filename, full-path tooltip, match badge, and snippet visibility, and transcript preview panel with item count, both cards' filenames, full-path tooltips, match badges (`파일명 일치` / `내용 일치`), and snippet visibility
