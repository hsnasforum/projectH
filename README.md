...
- Internal pipeline controller:
  ...
  - canonical runtime API는 `/api/runtime/status`, `/api/runtime/monitor-snapshot`, `/api/runtime/agent-inspector?agent=&lines=`, `/api/runtime/capture-tail?lane=&lines=`, `/api/runtime/start|stop|restart`, `/api/runtime/send-input`입니다.
  ...
  - controller/browser UI는 supervisor가 쓴 run-scoped runtime status만 읽고, direct pane/log/file scan을 current truth로 사용하지 않습니다. `/api/runtime/status`는 source payload에 `runtime_snapshot`이 없으면 reducer-owned snapshot을 보강하고, Queue 표시는 status의 `runtime_snapshot.queue`를 우선 사용하며, legacy `control` / `active_round` / `compat.control_slots`는 fallback과 debug drift 확인에만 씁니다.
  ...
  - `.pipeline/operator_request.md`는 `CONTROL_SEQ`, `REASON_CODE`, `OPERATOR_POLICY`, `DECISION_CLASS`, `DECISION_REQUIRED`, `BASED_ON_WORK`, `BASED_ON_VERIFY`를 top header로 함께 두는 편이 canonical입니다. supervisor/watcher는 `pipeline_runtime.operator_autonomy.resolve_operator_control(...)` 공유 resolver로 `OPERATOR_POLICY` 우선, `REASON_CODE` 다음 순서의 publish/gate와 stale recovery를 판정하고, 구조화 metadata가 없거나 알 수 없으면 fail-safe로 즉시 publish합니다.
  - 오래된 release-gate stop의 compound header는 shared evaluator에서 canonical metadata로 정규화됩니다. 예를 들어 `mNN_commit_push_milestones_doc_sync`, `bN_release_gate_commit_authorization_dirty_tree`, 또는 `dirty_bundle_publication_or_hold_decision`과 `OPERATOR_POLICY: pr_creation_gate + commit_push_bundle_authorization` / `operator_only_publication_boundary`, `DECISION_CLASS: publication` / `publication_or_hold` / `commit_publish_authorization`은 `commit_push_bundle_authorization + internal_only + release_gate`로 낮춰 verify/handoff follow-up에 보냅니다. 알 수 없는 publish label이나 구조화 header 누락은 계속 fail-safe `needs_operator`입니다.
  ...
  - browser log modal은 tail read-model 위에 bounded one-line lane input(`POST /api/runtime/send-input`)을 함께 제공합니다. permission/plan prompt처럼 operator 선택이 필요한 경우 현재 열린 lane에 `1`, `2`, `3` 같은 짧은 응답을 바로 보낼 수 있습니다. 서버 route는 object JSON의 `lane`/`text`만 backend로 넘기며 empty/missing/blank/malformed/non-object/invalid-length body는 JSON 오류로 fail closed하고 backend send를 호출하지 않습니다. lane pause/resume/restart나 attach 같은 backend 미연결 액션은 계속 노출하지 않습니다.
...
127. review-queue 액션 버튼(`data-testid="review-accept"`)이 `data-testid="review-context-turns"` 요소보다 DOM 상 앞에 위치하는지 확인 (evidence badges 직후, context turns 이전)
128. preference 카드가 top-level `injected_count > 0`일 때 `data-testid="preference-injected-count"` 배지를 표시하고, `applied_count > 0`이면 `4회 주입 (50% 적용)`처럼 적용 전환율을 함께 표시하며 `injected_count = 0`인 선호에는 배지를 숨기는지 확인
...
10. controller renders Queue presentation from runtime payloads — stubs `/api/runtime/status` with Queue payload variants and asserts the sidebar `Queue` row text/class plus `#marquee-text` Queue payload match the reducer output
11. agent cards expose `data-fatigue` attribute for fatigue observability — verifies that a working agent's sidebar card carries `data-agent` and `data-fatigue` attributes, with no `.agent-fatigue` indicator visible before the fatigue threshold is reached
12. `setAgentFatigue` hook transitions card to `fatigued` state — injects fatigue via `window.setAgentFatigue("Claude", "fatigued")` and asserts `data-fatigue="fatigued"` with `💦 피로 누적` label
13. `setAgentFatigue` hook transitions card to `coffee` state — injects coffee via `window.setAgentFatigue("Claude", "coffee")` and asserts `data-fatigue="coffee"` with `☕ 커피 충전 중` label
14. idle agents settle into lounge rest bounds — stubs one idle lane, checks `window.getAgentPositions().Claude` is already inside `window.getRoamBounds().restZones.Claude`, then forces 30 picks via `window.testPickIdleTargets("Claude", 30)` and asserts every point stays inside that lounge rest zone
15. lounge rest zones keep idle agents partitioned — places a phantom at the center of `codex_desk`, forces 50 picks via `window.testAntiStacking("Claude", cx, cy, 50)`, and asserts every pick still lies inside Claude's own lounge `restZones.Claude` partition
16. lounge idle roam uses continuous micro-roam (no spot history) — asserts `window.testHistoryPenalty("Claude", [0,1,2,3,4], 120)` returns an empty array because lounge micro-roam no longer tracks spot history, and that `window.testPickIdleTargets("Claude", 20)` still yields 20 points inside the assigned lounge rest zone
...
- current memory behavior is intentionally narrow: grounded-brief trace anchoring, normalized original-response snapshots, explicit `corrected_text` submission plus minimum `accepted_as_is` / `corrected` / `rejected` outcome capture, approval-linked reject / reissue reason traces, and the first reviewed-memory slice (review queue, aggregate apply trigger, emitted/apply/result/active-effect path, stop-apply, reversal, and conflict-visibility) are shipped; broader structured correction memory, durable preference memory, cross-session memory, and user-level memory remain unshipped later work
