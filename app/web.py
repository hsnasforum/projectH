...
class LocalAssistantHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/preferences/audit":
            ...
            return
        if parsed.path == "/api/corrections/summary":
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if parsed.path == "/api/corrections/dismiss-pattern":
                ...
                self._send_json(HTTPStatus.OK, response)
                return
            if parsed.path == "/api/corrections/promote-pattern":
                response = self.server.service.promote_correction_pattern(payload)
                self._send_json(HTTPStatus.OK, response)
                return
    ...
    def do_DELETE(self) -> None:
        ...
        try:
            self._validate_same_origin()
            response = self.server.service.delete_preference(preference_id)
            self._send_json(HTTPStatus.OK, response)
        except WebApiError as exc:
            self._send_json(exc.status_code, {"ok": False, "error": {"message": localize_text(exc.message)}})
        except Exception as exc:
