...
- Playwright smoke covers 17 core browser scenarios:
  ...
  - entity-card 붉은사막 자연어 reload 후 follow-up → source path plurality(`namu.wiki`, `ko.wikipedia.org`, `blog.example.com` provenance 포함) context box 유지
  ...
  - entity-card 붉은사막 자연어 reload 후 두 번째 follow-up → source path(`namu.wiki`, `ko.wikipedia.org`, `blog.example.com` provenance 포함) context box 유지 + `WEB` badge, `설명 카드`, `설명형 다중 출처 합의`, `백과 기반` response-origin drift 없음
