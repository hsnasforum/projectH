...
- Current reviewed-candidate preference audit visibility:
  ...
  - `get_global_audit_summary()` should include `injected_count` per preference in `per_preference_stats`, counting `preference_injected` task-log events by `preference_id`; a preference with no injected events should yield `injected_count == 0`; events without a matching `preference_id` in existing stats should be ignored
