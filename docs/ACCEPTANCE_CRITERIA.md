...
- Playwright smoke covers 17 core browser scenarios:
  ...
  - entity-card 붉은사막 actual-search browser 자연어 reload 후 두 번째 follow-up → source path(`namu.wiki`/`ko.wikipedia.org`) context box 유지 + `WEB` badge, `설명 카드`, `설명형 다중 출처 합의`, `백과 기반` response-origin drift 없음
