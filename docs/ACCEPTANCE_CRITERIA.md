...
- Playwright smoke covers 17 core browser scenarios:
  ...
  - history-card entity-card noisy single-source claim `다시 불러오기` 후 follow-up → `출시일`, `2025` 미노출 + `설명형 다중 출처 합의`, `백과 기반`, `namu.wiki`, `ko.wikipedia.org` 유지
  - history-card entity-card noisy single-source claim `다시 불러오기` 후 두 번째 follow-up → `출시일`, `2025` 미노출 + `설명형 다중 출처 합의`, `백과 기반`, `namu.wiki`, `ko.wikipedia.org` 유지
