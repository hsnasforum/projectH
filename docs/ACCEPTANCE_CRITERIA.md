...
- Internal/operator tooling such as `controller.server`, `pipeline_gui/`, `windows-launchers/`, and `_data/` pipeline helpers is outside the current release gate unless explicitly promoted later. The cozy controller runtime lives under shared `/controller-assets/js/cozy.js` ownership and is loaded from `controller/index.html` via a single `<script src="/controller-assets/js/cozy.js">` tag rather than one standalone inline runtime copy. Controller has its own Playwright smoke (`e2e/playwright.controller.config.mjs`) covering the shared cozy module load, scene feature hooks for the time-of-day window / pneumatic tube / courier packet-owl / pettable cat-audio path via `window.getSceneDebug()` and `window.testPetCat()`, the `#storage-warn` toolbar chip, event-log storage warning contract, deduplicated repeated `/api/runtime/status` fetch failures with one recovery log on reconnect, runtime-owned automation attention detail (`automation_health`, stale control/advisory fields) in the Incident Room, the main operator attention board for active `needs_operator` reasons, repository release-gate target fallback, and required action/evidence display, agent-card `data-fatigue` observability attribute, deterministic `fatigued`/`coffee` state transitions via `window.setAgentFatigue` test hook, lounge rest-zone roam bounds via `window.testPickIdleTargets` + `window.getRoamBounds`, actual scene placement visibility via `window.getAgentPositions`, lounge partition preservation via `window.testAntiStacking`, and `window.testHistoryPenalty` returning an empty array because idle roam uses continuous micro-roam rather than discrete spot history, runnable via `make controller-test` (port override: `CONTROLLER_SMOKE_PORT`). `ready`/`idle`/`off` agents rest inside the shared `lounge` in agent-specific `restZones`; role anchor keys (`claude_desk`, `codex_desk`, `gemini_desk`) still represent implement / verify / advisory desks and visible owner labels via `/api/runtime/status.role_owners`, but non-working sprites no longer idle at those desks. `sampleIdleTarget()` samples only within the current rest zone for non-working agents, while `working`/`booting`/`broken`/`dead` agents keep their desk/status anchoring.
- The internal pipeline runtime currently uses a supervisor-owned run-scoped status/events/receipt surface; `controller.server`, `pipeline_gui`, and `pipeline-launcher.py` read that surface as thin clients while `tmux` remains attach/debug substrate only, and operator tooling may consume both `degraded_reason` and `degraded_reasons` from the runtime status payload. The status payload also carries reducer-owned `runtime_snapshot` with Queue presentation fields; controller Queue UI must prefer that snapshot and must not show `No queued pipeline task` when a live active control slot, active verify round, or non-`ok` automation health exists. CLI/GUI/TUI start paths block on shared doctor required-check failures before spawning a supervisor. Completed duplicate `STATUS: implement` handoffs can be proven either by receipt or by matching `/work` plus `STATUS: verified` `/verify` truth; in both cases canonical `control` returns to `none` while debug `compat.control_slots` may still show the historical slot. Stop returns success after forced cleanup once supervisors are actually gone and status/orphan cleanup has completed.
- Socket-free controller unit coverage fixes the internal JSON route contract: `/api/runtime/status` returns normalized status and backfills missing `runtime_snapshot`, monitor/agent-inspector/capture-tail are read-oriented runtime JSON surfaces, start/stop/restart are POST JSON action routes, `/api/runtime/send-input` only forwards object JSON with `lane` and `text`, and empty, missing, blank, malformed, non-object, or invalid `Content-Length` input fails closed without calling the backend sender. Controller shell and asset routes remain local; bad asset paths return JSON 404.
...
- When `compat.turn_state` is present, internal current-turn labels should follow `turn_state.active_role` and `turn_state.active_lane` ahead of the legacy enum name alone, so swapped profiles do not mislabel `Claude` / `Codex` ownership on the home card.
...
- Recent results can show:
  ...
  - applied preferences popover는 각 선호 항목에 현재 상태가 `active`가 아닐 때
    status 배지를 표시한다.
  - applied preferences popover는 `last_transition_reason`이 있는 선호 항목에
    `이유: ...` 문구를 표시한다.
  - correction-submit 응답이 `auto_activated: true`와 `preference_id`를 반환하면
    React preview `PreferencePanel`은 `선호도로 자동 저장됨` inline notice와
    `#pref-card-{preference_id}` 대상 `선호에서 보기` 링크를 표시한다.
  - `GET /api/corrections/<correction_id>` returns the selected correction detail, and
    `PreferencePanel` recent correction rows use `fetchCorrectionDetail()` to render
    `data-testid="correction-detail-panel"` with original text, corrected text, and
...
- Response-body section headers for web investigation are annotated with the currently emitted status tags: `확인된 사실 [교차 확인]:`, `상충하는 정보 [정보 상충]:`, `단일 출처 정보 [단일 출처] (추가 확인 필요):`, `확인되지 않은 항목 [미확인]:`. The claim-coverage panel hint maps each emitted tag to its explanation so users can connect sections to statuses at a glance.
...
- Reject / reissue approval traces can persist one normalized `approval_reason_record` without changing the current approval UI; corrected-save reissue uses `reason_label = corrected_text_reissue`.
...
- Current session payloads and the existing shell can now also expose one local `review_queue_items` / `검토 후보` surface fed only by current `durable_candidate` items with `promotion_eligibility = eligible_for_review`, with source-session linkage (`source_session_id`, `source_session_title`), up to three preceding `context_turns`, `evidence_summary` counts, and `accept`/`reject`/`defer`/`edit` review actions that each record source-message `candidate_review_record` with the corresponding status, remove the item from the pending queue, and persistently show the review outcome label (`검토 수락됨`/`검토 거절됨`/`검토 보류됨`/`검토 편집됨`) on the source-message transcript meta and quick meta; a later correction clears the review outcome with the stale record.
...
- Session JSON keeps:
  ...
  - `active_context` — `{kind, label, source_paths, summary_hint, summary_hint_basis, suggested_prompts, record_path, claim_coverage_progress_summary}`; `summary_hint_basis` is `current_summary` when the hint reflects the most recent document/search/web summary and `recorded_correction` when a correction submit has rewritten the hint; sessions saved before this field existed are backfilled by the session-store owner only when a same-session grounded-brief message carries `corrected_text` whose compacted form matches the stored hint — ambiguous or unmatched legacy sessions fall back to the safe `current_summary`
  - timestamps
...
- Each current source-message review queue item should carry `item_type = durable_candidate`, source anchor fields, `source_session_id`, `source_session_title`, the underlying `derived_from` / `derived_at` projection metadata, up to three compact `context_turns`, and `evidence_summary` counts so future queue families cannot be confused with this source-message durable-candidate path.
- When a candidate review with `review_action = accept` succeeds and the source message exposes a valid `candidate_recurrence_key` with a `normalized_delta_fingerprint`, the accepted reviewed candidate is persisted as a local preference candidate record through the existing `PreferenceStore` / `SQLitePreferenceStore` via `record_reviewed_candidate_preference`. The record uses `status = candidate` and is not auto-activated. Idempotent on the same fingerprint: refreshes timestamps and appends distinct source refs instead of creating a duplicate record. Source refs can include session title and optional review reason note, and the existing `/api/preferences` list path surfaces them as top-level `source_session_title` and `review_reason_note` for the preference audit trace block plus `active_count` / `candidate_count` / `paused_count` for the status summary, active-only `total_applied` / `total_corrected` reliability aggregates, active-only `high_quality_active_count` quality aggregate, active-only `highly_reliable_active_count` reliability aggregate, and per-preference `is_highly_reliable` reliability signal.
...
- Minimum corrected-outcome capture is now implemented for the three truthful cases currently supported:
  ...
  - optional `corrected_outcome.reason_label`, currently `explicit_correction_submitted` on explicit correction submit
  - `recorded_at`
  - `artifact_id`
  - `source_message_id`
...
- Explicit correction submit is now implemented on the grounded-brief response surface:
  ...
  - the same submit persists `corrected_outcome.reason_label = explicit_correction_submitted`
  - unchanged submit must fail validation instead of becoming `accepted_as_is`
  - when the session has an `active_context`, correction submit also updates `active_context.summary_hint` to the corrected text and flips `active_context.summary_hint_basis` from `current_summary` to `recorded_correction`, so subsequent same-session follow-up responses use the corrected version as their basis and the serialized basis is explicit (not inferred from text shape)
  - the correction helper copy makes this basis contract visible: no recorded correction → `아직 기록된 수정본이 없어 같은 세션의 후속 질문과 재요약은 원본 요약 기준으로 이어집니다.`; a recorded correction with no unrecorded editor change → `기록된 수정본이 같은 세션의 후속 질문과 재요약 기준이 됩니다.`; a recorded correction with an unrecorded editor change → `후속 질문과 재요약도 직전 기록본 기준으로 이어지며, 입력창의 새 변경을 기준으로 바꾸려면 다시 수정본 기록을 눌러 주세요.`
  - the document context box labels `active_context.summary_hint` as `후속 질문 / 재요약 기준 (기록된 수정본):` when the serialized `summary_hint_basis` is `recorded_correction` and `후속 질문 / 재요약 기준 (현재 요약):` otherwise, with the hint text rendered on the following line; one same-session composer follow-up after correction must keep the context box on the `recorded_correction` basis label
...
- Current corrected-save snapshot rule:
  ...
  - corrected-save approval execution preserves `corrected_outcome.outcome = corrected` and `corrected_outcome.reason_label = explicit_correction_submitted` on the source message while adding optional `approval_id` / `saved_note_path` linkage there
...
- Current implementation should stay additive:
  - serialized grounded-brief source messages should expose one computed optional `session_local_memory_signal` only when current session state has a relevant same-anchor correction, content reason, approval reason, or completed save signal
  - the signal should stay read-only and source-message-anchored
  - the signal should include fixed `signal_version = session_local_memory_signal_v1`, same-anchor `artifact_id` / `source_message_id`, and `derived_at`
  - the signal should keep `correction_signal`, `content_signal`, `approval_signal`, and `save_signal` separate
...
- Minimum shipped `candidate_review_record` fields should include:
  ...
  - `recorded_at`
  - optional `reason_note` (present when the submitted review includes an explicit rationale or edited review statement)
...
- Current reviewed-candidate preference audit visibility:
  ...
  - `/api/preferences` should include active-only aggregate fields `total_applied` and `total_corrected`; no active preferences should yield zero totals
  ...
  - `/api/preferences` should include per-preference `is_highly_reliable`; when `injected_count >= 3` and `injection_correction_rate > 0.25`, it should be `false` even if a stored explicit value exists; otherwise a stored explicit value should be honored, and records without one should be `true` only when `quality_info.is_high_quality == True`, `reliability_stats.applied_count >= 3`, and `reliability_stats.corrected_count / reliability_stats.applied_count < 0.15`
  - immediately after a user manually activates a reviewed-candidate preference through `activate_preference()`, the stored record should carry `is_highly_reliable == True` and the preference should be recognized as an eligible same-session injection candidate without waiting for runtime reliability stats unless later injection-correction evidence crosses the demotion threshold; statistics-based auto-activation should remain separate
  - `_get_active_preferences(user_input=...)` should return only eligible ACTIVE preferences whose `description` or `corrected_text` shares at least one non-stop-word term with `user_input` when such preferences exist, ordered by overlap-term count descending with `is_highly_reliable == True` preferences first on equal scores
  - `_get_active_preferences(user_input=...)` should return the full eligible ACTIVE preference set when `user_input` is absent, when all `user_input` words are stop-words, or when no preference matches the filtered terms (`fallback_all`)
  ...
  - `get_global_audit_summary()` should include `injected_count` per preference in `per_preference_stats`, counting `preference_injected` task-log events by `preference_id`; a preference with no injected events should yield `injected_count == 0`; events with a valid `preference_id` but no existing applied/corrected stats should create a zeroed stats entry before incrementing `injected_count`, while events without a valid `preference_id` should be ignored; when a session has both injected preferences and a correction event, `injection_correction_count` should increment once per injected preference for that session; `/api/preferences` should expose the same injection counts plus `injection_correction_rate = injection_correction_count / injected_count` rounded to three decimals while keeping `reliability_stats` limited to `applied_count` and `corrected_count`; the shared reliability projection should demote preferences from `is_highly_reliable` when `injected_count >= 3` and `injection_correction_rate > 0.25`
  ...
  - `/api/preferences` should include `high_severity_conflict_count` as an active-only aggregate counting preferences whose `conflict_info.conflict_severity` is `"high"`
  ...
  - `PreferencePanel` should render `총 적용 N회 · 총 교정 N회` in the header only when at least one active preference exists, and should keep per-card reliability stats (`적용 N회 · 교정 N회`) unchanged
  - `PreferencePanel` should render `고품질 N개` in the header only when `high_quality_active_count > 0`, and should keep per-card `고품질` badge behavior unchanged
  - `PreferencePanel` should render `신뢰도 높음 N개` in the header only when `highly_reliable_active_count > 0`, and should render `신뢰도 높음` on qualifying preference cards while omitting it when `is_highly_reliable` is not `true`, without changing the existing `고품질` badge or panel-level `고품질 N개` aggregate
  - correction-submit responses may include `auto_activated: true` and `preference_id` when a repeated correction activates a highly reliable preference; the React preview should route that event to `PreferencePanel` and render `data-testid="preference-auto-activated-notice"` plus `data-testid="preference-auto-activated-link"`
  - `GET /api/corrections/<correction_id>` should return correction detail JSON for existing correction ids; the frontend `fetchCorrectionDetail()` client should call that endpoint, and `PreferencePanel` should render `data-testid="correction-detail-panel"` after a recent correction row is selected, showing original text, corrected text, and a delta-summary based reason
  - preference delete in PreferencePanel: preference 카드의 `data-testid="delete-preference-btn"` 버튼 클릭이 `DELETE /api/preferences/<preference_id>`를 호출하고, 응답 후 해당 카드가 목록에서 제거된다; ACTIVE 상태 선호 삭제 시 pause 후 제거, 존재하지 않는 id에 404 반환
  - preference reliability toggle in PreferencePanel: `data-testid="toggle-reliability-btn"` 버튼 클릭이 `POST /api/preferences/<preference_id>/toggle-reliability`를 호출하고, 갱신된 `is_highly_reliable` 값으로 배지 상태가 갱신된다; 저장된 값은 일반 runtime projection보다 우선하지만 `injected_count >= 3` 및 `injection_correction_rate > 0.25` 강등 조건이 최종 우선함; unknown id에 404 반환
  - preference text edit in PreferencePanel: `data-testid="edit-preference-btn"` 클릭이 인라인 편집 모드를 열고, `data-testid="save-preference-text-btn"` 클릭이 `PATCH /api/preferences/<preference_id>`를 호출해 `corrected_text`를 갱신한다; 빈 텍스트는 저장 버튼을 비활성화하고 400으로 거부된다; unknown id에 404 반환
  - correction history status filter in PreferencePanel: `data-testid="correction-status-filter"` 드롭다운 선택이 `fetchCorrectionList({ status })` 호출을 트리거하고 교정 목록이 해당 상태로 필터링된다; "전체" 선택 시 status 파라미터 없이 호출된다
  - correction history search and limit expansion in PreferencePanel: `data-testid="correction-search-input"` 입력이 `fetchCorrectionList({ query })` 호출을 트리거하고 목록이 텍스트 매칭 교정으로 필터링된다; `data-testid="correction-show-more-btn"` 버튼이 limit를 확장해 더 많은 항목을 표시한다; 기본 limit는 5에서 20으로 증가됐다
...
- Focused regression should verify at least:
  - a grounded-brief source message omits `session_local_memory_signal` when no correction, content reason, approval reason, or completed save exists yet
  - `correction_signal`, `content_signal`, `approval_signal`, and `save_signal` remain separate instead of collapsing into one label
  - later correction or explicit save can clear stale reject state from current `content_signal` while earlier save linkage may still remain in `save_signal`
- Focused regression should now also verify at least:
  ...
  - the helper does not overwrite `session_local_memory_signal.correction_signal`
...
- The current implementation should now expose one optional normalized `session_local_candidate` on serialized grounded-brief source messages only when:
  ...
  - `session_local_memory_signal.correction_signal.corrected_outcome.outcome = corrected`
...
- The current extraction rule should stay conservative:
  ...
  - current `session_local_memory_signal.correction_signal` is the primary basis
...
- The first explicit-confirmation projection path should stay minimal:
  ...
  - set `derived_at` from `candidate_confirmation_record.recorded_at`
...
- The first post-aggregate promotion boundary should stay contract-only and conservative:
  ...
  - the shipped operator-audit slice should stay read-only and narrow:
    ...
    - shipped transition mutation identity is stricter than the audit-contract label: every shipped transition mutation request must include the same `canonical_transition_id` and an `aggregate_fingerprint` matching `aggregate_identity_ref.normalized_delta_fingerprint`; this applies to `/api/aggregate-transition-apply`, `/api/aggregate-transition-result`, `/api/aggregate-transition-stop`, `/api/aggregate-transition-reverse`, and `/api/aggregate-transition-conflict-check`
  ...
  - the current shipped operator-visible trigger-source layer stays separate from both the shipped audit contract and the emitted record:
    ...
    - aggregate-card metadata shows `mutation canonical_transition_id+aggregate_fingerprint` when `transition_mutation_identity_requirement = canonical_transition_id_and_aggregate_fingerprint_required`
...
- Focused regression for the current same-session aggregation slice should verify at least:
  ...
  - the raw aggregate projection remains read-only by itself and does not create repeated-signal promotion or user-level memory; the shipped reviewed-memory apply lifecycle lives in the separate aggregate transition path above it
- Focused regression for the first future post-aggregate marker slice should verify at least:
  ...
  - the marker stays a read-only promotion marker and must not be treated as the shipped reviewed-memory transition lifecycle or rollback result
...
- Focused regression for the current operator-audit-contract slice should verify at least:
  ...
  - `transition_mutation_identity_requirement` stays `canonical_transition_id_and_aggregate_fingerprint_required`
  - apply, result-confirmation, stop-apply, reversal, and conflict-visibility mutations also require the requested `aggregate_fingerprint` to match `aggregate_identity_ref.normalized_delta_fingerprint`; wrong-fingerprint requests fail without changing transition stage, `applied_at`, `result_at`, `apply_result`, active effects, stopped/reversed state, or conflict-visibility records
...
- First checks should verify:
  ...
  - an explicit correction submit updates that same original grounded-brief source message with `corrected_text`, `corrected_outcome.outcome = corrected`, and `corrected_outcome.reason_label = explicit_correction_submitted`
  ...
  - a reissued pending approval can expose that same normalized `approval_reason_record` on the live approval payload, using `reason_label = corrected_text_reissue` when the approval being reissued targets `save_content_source = corrected_text`
...
- Not implemented today:
  - richer reject / reissue labels beyond `explicit_rejection`, `path_change`, and `corrected_text_reissue`
...
- Playwright smoke launch through `e2e/start-server.sh` health-checks `http://127.0.0.1:8879/healthz`, reuses an already healthy smoke-port server, otherwise clears inherited provider/model overrides for the newly started fallback, forces `LOCAL_AI_MODEL_PROVIDER=mock`, waits for health, and later cleans up the isolated mock server with a temporary SQLite DB.
- Playwright smoke covers 128 core browser scenarios (document-level browser coverage inventory count, not the raw `test(...)` count in `e2e/tests/web-smoke.spec.mjs`):
  ...
  - review queue item source-session linkage, previous conversation `context_turns`, evidence-summary counts, and panel rendering for conversation context / evidence chips
  ...
  - reviewed-candidate decision rationale persists into preference source refs when provided, `/api/preferences` exposes `review_reason_note` / `source_session_title` plus active-only `total_applied` / `total_corrected`, `high_quality_active_count`, `highly_reliable_active_count`, `high_severity_conflict_count`, and per-preference `is_highly_reliable`; user-triggered preference activation stores `is_highly_reliable == True` so the activated preference can be injected in the same session before runtime stats accrue unless later injection-correction evidence crosses the demotion threshold; `PreferencePanel` conditionally renders `출처 세션` / `결정 사유`, the active-only `총 적용 N회 · 총 교정 N회`, `고품질 N개`, `신뢰도 높음 N개`, and `충돌 위험 N건` header lines, the per-card `신뢰도 높음` badge only for preferences whose final projected `is_highly_reliable` remains true, and the per-card `N회 주입` / `N회 주입 (R% 적용)` badge only when top-level `injected_count > 0`
  - correction history detail in `PreferencePanel`: recent correction rows call `fetchCorrectionDetail()` / `GET /api/corrections/<correction_id>` and render `data-testid="correction-detail-panel"` with original text, corrected text, and delta-summary reason
