...
- Playwright smoke covers 126 core browser scenarios (document-level browser coverage inventory count, not the raw `test(...)` count in `e2e/tests/web-smoke.spec.mjs`):
  ...
  - reviewed-candidate decision rationale persists into preference source refs when provided, `/api/preferences` exposes `review_reason_note` / `source_session_title` plus active-only `total_applied` / `total_corrected`, and `PreferencePanel` conditionally renders `출처 세션` / `결정 사유` plus the active-only `총 적용 N회 · 총 교정 N회` header
