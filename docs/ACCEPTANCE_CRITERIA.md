...
- Playwright smoke covers 17 core browser scenarios:
  ...
  - browser folder picker with `선택 결과 요약` source-type label and multi-source count-based metadata (`출처 2개`) assertion in both quick-meta and transcript meta, plus transcript preview panel with item count, first card filename, full-path tooltip, match badge, and snippet visibility
