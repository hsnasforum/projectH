...
- Current reviewed-candidate preference audit visibility:
  ...
  - `get_global_audit_summary()` should increment per-preference `corrected_count` for each preference fingerprint on a preference-applied response that has saved negative feedback (`incorrect` / `unclear`)
  - positive feedback should not increment per-preference `corrected_count`
  - the existing explicit text correction (`corrected_text`) path should continue to increment per-preference `corrected_count`
...
- Playwright smoke covers 126 core browser scenarios (document-level browser coverage inventory count, not the raw `test(...)` count in `e2e/tests/web-smoke.spec.mjs`):
  ...
  - reviewed-candidate decision rationale persists into preference source refs when provided, `/api/preferences` exposes `review_reason_note` / `source_session_title` plus active-only `total_applied` / `total_corrected`, negative feedback (`incorrect` / `unclear`) on preference-applied responses increments per-preference `corrected_count` while positive feedback does not, and `PreferencePanel` conditionally renders `출처 세션` / `결정 사유` plus the active-only `총 적용 N회 · 총 교정 N회` header
