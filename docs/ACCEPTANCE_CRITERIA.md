...
- Current reviewed-candidate preference audit visibility:
  ...
  - `/api/preferences` should expose `source_session_title` and `review_reason_note` only when those fields exist on the selected source ref
  - `PreferencePanel` should render the `출처 세션` / `결정 사유` audit block conditionally and should not treat the block as automatic activation, rollback, scope selection, or cross-session application
...
- Playwright smoke covers 126 core browser scenarios (document-level browser coverage inventory count, not the raw `test(...)` count in `e2e/tests/web-smoke.spec.mjs`):
  ...
  - reviewed-candidate decision rationale persists into preference source refs when provided, `/api/preferences` exposes `review_reason_note` / `source_session_title`, and `PreferencePanel` conditionally renders `출처 세션` / `결정 사유`
