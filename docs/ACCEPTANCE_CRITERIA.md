...
- Playwright smoke covers 17 core browser scenarios:
  ...
  - entity-card noisy single-source claim browser 자연어 reload 후 follow-up → 본문/origin detail에 `출시일`, `2025`, `blog.example.com` 미노출 + `설명형 다중 출처 합의`, `백과 기반`, `namu.wiki`, `ko.wikipedia.org` 유지, source_paths에 `blog.example.com` provenance 포함
  - entity-card noisy single-source claim browser 자연어 reload 후 두 번째 follow-up → 본문/origin detail에 `출시일`, `2025`, `blog.example.com` 미노출 + `설명형 다중 출처 합의`, `백과 기반`, `namu.wiki`, `ko.wikipedia.org` 유지, source_paths에 `blog.example.com` provenance 포함
