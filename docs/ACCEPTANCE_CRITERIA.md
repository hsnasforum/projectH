...
- Current reviewed-candidate preference audit visibility:
  ...
  - preference reliability toggle in PreferencePanel: `data-testid="toggle-reliability-btn"` 버튼 클릭이 `POST /api/preferences/<preference_id>/toggle-reliability`를 호출하고, 갱신된 `is_highly_reliable` 값으로 배지 상태가 갱신된다; 저장된 값이 runtime projection보다 우선함; unknown id에 404 반환
