...
- Current reviewed-candidate preference audit visibility:
  ...
  - `/api/preferences` should include per-preference `is_highly_reliable`, and it should be `true` only when `quality_info.is_high_quality == True`, `reliability_stats.applied_count >= 3`, and `reliability_stats.corrected_count / reliability_stats.applied_count < 0.15`
