...
- each current `recurrence_aggregate_candidates` item may now also expose one read-only `aggregate_promotion_marker`, one read-only `reviewed_memory_precondition_status`, one read-only `reviewed_memory_boundary_draft`, one read-only `reviewed_memory_rollback_contract`, one read-only `reviewed_memory_disable_contract`, one read-only `reviewed_memory_conflict_contract`, one read-only `reviewed_memory_transition_audit_contract`, one read-only `reviewed_memory_unblock_contract`, one read-only `reviewed_memory_capability_status`, one additive read-only `reviewed_memory_planning_target_ref`, one read-only `reviewed_memory_capability_basis`, one conditional `reviewed_memory_transition_record`, and one optional read-only `reviewed_memory_conflict_visibility_record`; none of these objects is persisted as a separate store
...
- computed top-level `recurrence_aggregate_candidates`
  ...
  - current items may also expose one read-only `aggregate_promotion_marker`, one read-only `reviewed_memory_precondition_status`, one read-only `reviewed_memory_boundary_draft`, one read-only `reviewed_memory_rollback_contract`, one read-only `reviewed_memory_disable_contract`, one read-only `reviewed_memory_conflict_contract`, one read-only `reviewed_memory_transition_audit_contract`, one read-only `reviewed_memory_unblock_contract`, one read-only `reviewed_memory_capability_status`, one additive read-only `reviewed_memory_planning_target_ref`, one read-only `reviewed_memory_capability_basis`, one conditional `reviewed_memory_transition_record`, and one optional read-only `reviewed_memory_conflict_visibility_record`
...
- the first post-aggregate promotion boundary should stay smaller than repeated-signal promotion itself:
  ...
  - current architecture now also emits one read-only aggregate-level disable-contract object:
    ...
    - `audit_trace_expectation = operator_visible_local_transition_required`
    - `defined_at = last_seen_at`
  - the current blocked marker remains correct only because the reviewed-memory boundary is still missing every required precondition:
    ...
    - `rollback_ready_reviewed_memory_effect`
      - future reviewed-memory effect must be reversible without mutating source-message records or aggregate identity
      ...
      - after rollback, aggregate identity, supporting refs, the current boundary draft, and operator-visible rollback trace must remain while only the later applied effect deactivates
    - `disable_ready_reviewed_memory_effect`
      - future reviewed-memory effect must support explicit local stop-apply without deleting candidate traces, aggregate evidence, the current boundary draft, or the current rollback contract
      ...
      - disable means future influence stop without claiming reversal of the already-applied effect
  ...
  - the current truthful capability-path source stays separate from both shipped status surfaces:
    ...
    - the exact later local proof boundary beneath that fact-source-instance helper should stay one shared internal `reviewed_memory_local_effect_presence_proof_boundary`
    - the exact later canonical local proof record beneath that proof-boundary helper should stay one internal `reviewed_memory_local_effect_presence_proof_record`, and the proof-boundary helper now materializes only from one exact matching proof record inside one same-session internal `reviewed_memory_local_effect_presence_proof_record_store` boundary for the same aggregate
    ...
    - that target should stay shared by later rollback and later disable handles while each handle keeps its own contract linkage and capability semantics
