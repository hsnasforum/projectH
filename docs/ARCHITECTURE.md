...
| `applied_preferences` | list \| null | applied preference records; when non-empty the assistant message renders a `선호 N건 반영` badge with tooltip; the popover can show fetched full-preference status, `last_transition_reason`, per-preference reliability stats, and a `고품질` badge when available |
...
- `preference_injected` — detail: `{preference_id, reason, user_input_snippet}` where `reason` is `context_match` or `fallback_all`
...
`list_preferences_payload()` enriches each preference with reliability, injection, quality, and conflict metadata, then resolves the latest relevant reviewed-candidate source ref. Per-preference `reliability_stats` includes `applied_count` and `corrected_count`; `corrected_count` includes explicit corrected-text traces and negative feedback labels (`incorrect` / `unclear`). Per-preference top-level `injected_count`, `injection_correction_count`, and `injection_correction_rate` come from `per_preference_stats` and default to `0` / `0.0` when no injection stats exist, without changing the existing `reliability_stats` shape. Per-preference `quality_info` carries the `is_high_quality()` result; the helper accepts mid-range similarity as useful signal while excluding too-low noise and near-identical unchanged text. Per-preference `is_highly_reliable` is forced `False` when `injected_count >= 3` and `injection_correction_rate > 0.25`; otherwise it honors an explicit stored boolean first, and records without one are true only when `quality_info.is_high_quality is True`, `reliability_stats.applied_count >= 3`, and `corrected_count / applied_count < 0.15`. Per-preference `conflict_info` includes the existing `has_conflict` / `conflicting_preference_ids` fields plus `conflict_severity` (`high` when either side is highly reliable, `normal` for other conflicts, `none` when no conflict exists). When available, the payload exposes `review_reason_note` and `source_session_title` as top-level fields for `PreferencePanel`; these fields are audit visibility only and do not activate, pause, reject, apply, or scope a preference by themselves. The payload also returns `active_count`, `candidate_count`, and `paused_count` as integer counts of preferences by status, plus active-only aggregates `total_applied`, `total_corrected`, `high_quality_active_count`, and `highly_reliable_active_count`. `PreferencePanel` renders status filter tabs (전체 / 후보 / 활성 / 일시중지) with per-tab counts; rejected preferences are always hidden from the list regardless of the selected tab. `activate_preference()`, `pause_preference()`, and `reject_preference()` each accept an optional `transition_reason` string; when provided it is recorded in the task log `detail` for the corresponding `preference_activated`, `preference_paused`, or `preference_rejected` event. `activate_preference()` also stores `is_highly_reliable = True` in both JSON and SQLite preference stores, making user-activated reviewed-candidate preferences immediately eligible for the `AgentLoop._get_active_preferences()` / `is_highly_reliable_preference()` injection filter before runtime reliability stats exist unless later injection-correction evidence crosses the demotion threshold. `_auto_activate_candidate_if_ready()` remains the separate statistics-based auto-activation path. When a preference has a recorded `transition_reason` in the system task log for a `preference_activated`, `preference_paused`, or `preference_rejected` event, the most recent such value is exposed as `last_transition_reason` in the enriched preference record.

`AgentLoop._get_active_preferences()` has the signature `user_input: str | None = None` and `session_id: str | None = None` in addition to the existing budget and reliability parameters. It enriches active preferences with `get_global_audit_summary().per_preference_stats`, so the shared `is_highly_reliable_preference()` eligibility filter can demote injected preferences when `injected_count >= 3` and `injection_correction_rate > 0.25` before term ranking runs. It then delegates term extraction to `_preference_context_terms(text)` — which lowercases, whitespace-splits, filters to length ≥ 2, and removes a minimal stop-word set (common English function words plus Korean particles) — and selection to `_select_context_relevant_preferences(preferences, user_terms)` — which scores each preference by the overlap-term count between `user_terms` and the preference's extracted terms from `description` plus `corrected_text`, sorts descending by score, and breaks ties by placing `is_highly_reliable == True` preferences first. When `user_terms` is empty (absent or all stop-words) or no preferences score above zero, it returns the full eligible set with `reason = "fallback_all"`. For each injected preference, `_log_preference_injections()` writes a `preference_injected` task-log event using the provided `session_id`; missing task logging or per-event logging failures are non-blocking and do not prevent injection. `PerPreferenceStats` in `core/contracts.py` carries `applied_count`, `corrected_count`, `injected_count`, and `injection_correction_count`; `get_global_audit_summary()` in both `SessionStore` and `SQLiteSessionStore` accumulates `injected_count` by scanning `preference_injected` task-log events by valid `preference_id`, creating a zeroed stats entry when the preference has no prior applied/corrected stats; events without a valid `preference_id` are silently skipped. If that same session also has a correction event (`correction_submitted`, corrected text/outcome, or explicit preference-correction evidence), the summary increments `injection_correction_count` once for each injected preference in that session; this audit signal now feeds the shared reliability projection without changing the `reliability_stats` shape.
...
- this contract should come before broader `edit` expansion because that action still operates on one source-message candidate version, while the recurrence-key-plus-same-session-aggregate boundary is the first truthful cross-source identity needed before same-family aggregation or repeated-signal promotion can be honest (`reject` and `defer` are now shipped alongside `accept`)
  ...
  - the next architecture widening should still stay closed:
    - keep the shipped readiness surface separate from the blocked-only status object:
      ...
      - the truthful same-aggregate `unblocked_all_required` path above the now-materialized basis and now-complete internal source family is now implemented; the enabled aggregate-card submit boundary is now open; one truthful aggregate-level `reviewed_memory_transition_record` is now emitted at that boundary; the reviewed-memory apply boundary is now also implemented: after emission the aggregate card shows a `검토 메모 적용 실행` button, clicking it changes `record_stage` from `emitted_record_only_not_applied` to `applied_pending_result` and adds `applied_at` via POST `/api/aggregate-transition-apply`; the apply result is now also implemented: after the apply boundary the card shows `결과 확정`, clicking it changes `record_stage` to `applied_with_result` and creates `apply_result` with `result_version = first_reviewed_memory_apply_result_v1`, `applied_effect_kind = reviewed_memory_correction_pattern`, `result_stage = effect_active`, and `result_at`; the memory effect on future responses is now active (`result_stage = effect_active`); active effects are stored on the session as `reviewed_memory_active_effects`; future responses include a `[검토 메모 활성]` prefix with the operator's reason and pattern fingerprint; stop-apply (`future_reviewed_memory_stop_apply`) is now also implemented: after the effect is active the aggregate card shows an `적용 중단` button; clicking it changes `record_stage` to `stopped`, sets `apply_result.result_stage` to `effect_stopped`, removes the effect from `reviewed_memory_active_effects`, and future responses no longer include the `[검토 메모 활성]` prefix; reversal (`future_reviewed_memory_reversal`) is now also implemented: after the effect is stopped the aggregate card shows an `적용 되돌리기` button; clicking it changes `record_stage` to `reversed`, sets `apply_result.result_stage` to `effect_reversed`, and adds `reversed_at`; aggregate identity, supporting refs, and contracts are retained; reversal is separate from stop-apply; `future_reviewed_memory_conflict_visibility` is now also implemented: after reversal the aggregate card shows a `충돌 확인` button; clicking it creates a separate `reviewed_memory_conflict_visibility_record` with `transition_action = future_reviewed_memory_conflict_visibility`, `record_stage = conflict_visibility_checked`, `source_apply_transition_ref`, evaluated `conflict_entries`, and `conflict_entry_count`
    - disable contract stays read-only; stop-apply lifecycle (`future_reviewed_memory_stop_apply`) is shipped on `record_stage`; per-precondition disable satisfaction booleans remain later
    - rollback contract stays read-only; reversal lifecycle (`future_reviewed_memory_reversal`) is shipped on `record_stage`; per-precondition rollback satisfaction booleans remain later
...
- the first narrow reviewed-memory operator surface is now implemented as one aggregate-level `future_reviewed_memory_apply` trigger-source affordance:
  ...
  - the reviewed-memory apply boundary is now also implemented: after emission the aggregate card shows a `검토 메모 적용 실행` button; clicking it POSTs to `/api/aggregate-transition-apply` and changes `record_stage` from `emitted_record_only_not_applied` to `applied_pending_result` with `applied_at` added; the apply result is now also implemented: after the apply boundary the card shows `결과 확정`, clicking it changes `record_stage` to `applied_with_result` and creates `apply_result` with `result_version = first_reviewed_memory_apply_result_v1`, `applied_effect_kind = reviewed_memory_correction_pattern`, `result_stage = effect_active`, and `result_at`; the memory effect on future responses is now active (`result_stage = effect_active`); active effects are stored on the session as `reviewed_memory_active_effects`; future responses include a `[검토 메모 활성]` prefix with the operator's reason and pattern fingerprint; stop-apply (`future_reviewed_memory_stop_apply`) is now also implemented: after the effect is active the aggregate card shows an `적용 중단` button; clicking it changes `record_stage` to `stopped`, sets `apply_result.result_stage` to `effect_stopped`, removes the effect from `reviewed_memory_active_effects`, and future responses no longer include the `[검토 메모 활성]` prefix; reversal (`future_reviewed_memory_reversal`) is now also implemented: after the effect is stopped the aggregate card shows an `적용 되돌리기` button; clicking it changes `record_stage` to `reversed`, sets `apply_result.result_stage` to `effect_reversed`, and adds `reversed_at`; aggregate identity, supporting refs, and contracts are retained; reversal is separate from stop-apply; `future_reviewed_memory_conflict_visibility` is now also implemented: after reversal the aggregate card shows a `충돌 확인` button; clicking it creates a separate `reviewed_memory_conflict_visibility_record` with `transition_action = future_reviewed_memory_conflict_visibility`, `record_stage = conflict_visibility_checked`, `source_apply_transition_ref`, evaluated `conflict_entries`, and `conflict_entry_count`
