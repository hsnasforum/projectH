...
- controller Queue presentation coverage now includes a browser-level smoke scenario for the sidebar `Queue` row and marquee payload text, plus a socket-free `controller/js/cozy.js` Queue behavior guard in `tests/test_controller_queue_presentation.py`; current local Playwright execution can still be environment-held by `local_socket_guard_auto_held`, so this records coverage shape rather than controller-smoke pass or release readiness
...
### Milestone 45: Preference Reliability Aggregate

#### Goal
PreferencePanel header에서 활성 선호 전체의 `total_applied` / `total_corrected`
합계를 보여 사용자가 현재 활성 선호가 실제 응답에 얼마나 적용/교정되고 있는지
빠르게 확인하게 한다.
...
- `app/handlers/preferences.py`: preferences list payload에 active preference만
  합산한 `total_applied`, `total_corrected` 추가.
- `app/frontend/src/api/client.ts`: `PreferencesPayload`에 optional aggregate
  fields 추가.
- `app/frontend/src/components/PreferencePanel.tsx`: active preference가 있을 때
  header compact line으로 `총 적용 N회 · 총 교정 N회` 표시.
...
Axis 1 (CONTROL_SEQ 1202): TaskLogEntry TypedDict — ACTIVE
`core/contracts.py`에 `TaskLogEntry(TypedDict, total=False)` 4개 필드 추가;
`task_log.py` `iter_session_records()` 반환 타입 annotation 업데이트.

Axis 2 (CONTROL_SEQ 1204): SQLiteTaskLog iter_session_records 반환 타입 — ACTIVE
