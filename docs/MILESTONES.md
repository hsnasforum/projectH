...
- entity-card 붉은사막 browser natural-reload follow-up source-path plurality smoke covering `namu.wiki`, `ko.wikipedia.org`, `blog.example.com` provenance in context box after natural reload + follow-up
...
- entity-card 붉은사막 natural-reload second-follow-up source-path + response-origin continuity service + browser smoke covering `namu.wiki`, `ko.wikipedia.org`, `blog.example.com` provenance, `WEB` badge, `설명 카드`, `설명형 다중 출처 합의`, `백과 기반` after natural reload + second follow-up
