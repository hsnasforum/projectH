...
- preference session injection relevance: `AgentLoop._get_active_preferences()` accepts optional `user_input` and, after the existing reliable-preference eligibility filter, prioritizes preferences whose `description` or `corrected_text` shares at least one simple keyword with the current user input; when there is no user input or no related preference, it keeps the existing `fallback_all` behavior and returns the full eligible candidate set; each injected preference records one `preference_injected` task-log event with `reason`, `preference_id`, and `user_input_snippet`
...
- reviewed-candidate preference audit contract:
  ...
  - `AgentLoop._get_active_preferences(user_input=..., session_id=...)` now narrows that eligible injection set by simple keyword overlap against preference `description` plus `corrected_text`; it returns only related preferences when there is a match, otherwise returns the full eligible set with `fallback_all`, and task-log failures while recording `preference_injected` do not block preference injection
