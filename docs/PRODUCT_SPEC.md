...
- preference reliability signal badge and aggregate header in `PreferencePanel`: `/api/preferences` includes per-preference `is_highly_reliable`, which is true only when `quality_info.is_high_quality == True`, `reliability_stats.applied_count >= 3`, and `reliability_stats.corrected_count / reliability_stats.applied_count < 0.15`; `/api/preferences` also includes active-only `highly_reliable_active_count`, and the header renders `신뢰도 높음 N개` only when that count is greater than zero while each card renders `신뢰도 높음` only when `is_highly_reliable` is true; the existing `reliability_stats` / `quality_info` payload structures remain unchanged
