...
- reviewed-candidate preference audit contract:
  ...
  - `list_preferences_payload` exposes the latest relevant `reason_note` as `review_reason_note`, source session title as `source_session_title`, top-level status counts including `paused_count`, and active-only reliability aggregates `total_applied` / `total_corrected`
