...
- implemented:
  ...
  - accepted source-message and global reviewed candidates can write local preference candidate records through the existing preference store with `status = candidate` unless separately activated; global rejects can write rejected preference records to silence that fingerprint; reviewed-candidate source refs can carry `session_id`, optional `session_title`, optional review `reason_note`, review action, candidate id, candidate timestamp, artifact id, and source-message id; `list_preferences_payload` lifts the current audit fields as `review_reason_note` and `source_session_title`, reports `active_count` / `candidate_count` / `paused_count`, and includes active-only `total_applied` / `total_corrected` aggregates from per-preference `reliability_stats`, `high_quality_active_count` for active preferences whose `quality_info.is_high_quality` is `True`, and `highly_reliable_active_count` for active preferences whose `is_highly_reliable` is `True`; each preference also carries `is_highly_reliable`, computed only when `quality_info.is_high_quality is True`, `reliability_stats.applied_count >= 3`, and `reliability_stats.corrected_count / reliability_stats.applied_count < 0.15`; each enriched `conflict_info` also carries `conflict_severity` (`"high"` when either side is `is_highly_reliable=True`, `"normal"` for other conflicts, `"none"` when no conflict); `PreferencePanel` conditionally renders audit fields as `결정 사유` / `출처 세션`, filters visible non-rejected preferences by 전체/후보/활성/일시중지 tabs, shows `총 적용 N회 · 총 교정 N회` plus `고품질 N개` and `신뢰도 높음 N개` in the header only when the corresponding active counts exist, renders a per-card `신뢰도 높음` badge only when `is_highly_reliable` is `true`, and elevates the conflict badge to amber only when `conflict_severity` is `"high"` without changing activate/pause/reject behavior
