...
- reviewed-candidate preference audit contract:
  ...
  - `list_preferences_payload` exposes the latest relevant `reason_note` as `review_reason_note`, source session title as `source_session_title`, and top-level status counts including `paused_count`
  - the current preference panel displays those fields only when present and does not treat them as activation, application, rollback, or cross-session memory evidence by themselves; it filters non-rejected preferences by 전체/후보/활성/일시중지 tabs without changing lifecycle actions
