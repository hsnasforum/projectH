...
- implemented:
  ...
  - source-type-aware short-summary, per-chunk, and reduce prompt split plus source-type-aware `summary_chunks` selection heuristics that keep local file or uploaded-document summaries narrative-friendly and document-oriented while keeping selected local search-result summaries on source-backed synthesis; the short-summary and reduce prompts further distinguish multi-result summaries (shared facts, meaningful differences, actions, and conclusion) from single-result summaries (non-comparative wording focused on main facts, actions, and grounded conclusion), while per-chunk chunk-note prompts keep uniform search-synthesis wording regardless of result count
