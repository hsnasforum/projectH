...
- reviewed-candidate preference audit contract:
  ...
  - `get_global_audit_summary()` in both `SessionStore` and `SQLiteSessionStore` now includes `injected_count` in each `PerPreferenceStats` entry, counting `preference_injected` task-log events per preference id and creating zeroed entries for valid preference ids that have only injection events; this enables tracking the injection-to-application conversion rate alongside `applied_count` and `corrected_count`, and `/api/preferences` exposes the same count as top-level per-preference `injected_count`
