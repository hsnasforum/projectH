...
- implemented:
  ...
  - one local `review_queue_items` session projection plus one compact existing-shell `검토 후보` section fed only by current `durable_candidate` items with `promotion_eligibility = eligible_for_review`, plus `accept`/`reject`/`defer`/`edit` review actions that each record source-message `candidate_review_record` with the corresponding `review_status` (`accepted`/`rejected`/`deferred`/`edited`), remove the item from pending `review_queue_items`, and persistently show the review outcome label on the source-message transcript meta and quick meta; each current queue item also carries source-session linkage (`source_session_id`, `source_session_title`), up to three immediately preceding `context_turns` (role/text/message anchor, text capped for display), and an `evidence_summary` count block (`artifact_count`, `signal_count`, `confirmation_count`, `recurring_session_count`) so the review panel can show source session, conversation context, and evidence-count chips without opening a new store
  - accepted source-message and global reviewed candidates can write local preference candidate records through the existing preference store with `status = candidate` unless separately activated; global rejects can write rejected preference records to silence that fingerprint; reviewed-candidate source refs can carry `session_id`, optional `session_title`, optional review `reason_note`, review action, candidate id, candidate timestamp, artifact id, and source-message id; `list_preferences_payload` lifts the current audit fields as `review_reason_note` and `source_session_title`, reports `active_count` / `candidate_count` / `paused_count`, and includes active-only `total_applied` / `total_corrected` aggregates from per-preference `reliability_stats`, `high_quality_active_count` for active preferences whose `quality_info.is_high_quality` is `True`, `highly_reliable_active_count` for active preferences whose `is_highly_reliable` is `True`, and `high_severity_conflict_count` for active preferences whose `conflict_info.conflict_severity` is `"high"`; each preference also carries top-level `injected_count`, `injection_correction_count`, `injection_correction_rate`, and `is_highly_reliable`; when `injected_count >= 3` and `injection_correction_rate > 0.25`, `is_highly_reliable` is forced `False` even if an explicit stored value exists; otherwise an explicit stored value is honored, and user-triggered `activate_preference()` stores `True` so a reviewed-candidate preference that the user manually activates can pass the session-injection filter immediately even before `quality_info.is_high_quality` or `reliability_stats.applied_count >= 3` exists; without an explicit stored value the signal is computed when `quality_info.is_high_quality is True`, `reliability_stats.applied_count >= 3`, and `reliability_stats.corrected_count / reliability_stats.applied_count < 0.15`; each enriched `conflict_info` also carries `conflict_severity` (`"high"` when either side is `is_highly_reliable=True`, `"normal"` for other conflicts, `"none"` when no conflict); correction-submit responses can report `auto_activated: true` with `preference_id` when a repeated correction activates a highly reliable preference; correction history detail is read-only through `GET /api/corrections/<correction_id>` and the frontend `fetchCorrectionDetail()` client; `PreferencePanel` conditionally renders audit fields as `결정 사유` / `출처 세션`, filters visible non-rejected preferences by 전체/후보/활성/일시중지 tabs, shows `총 적용 N회 · 총 교정 N회` plus `고품질 N개`, `신뢰도 높음 N개`, and `충돌 위험 N건` in the header only when the corresponding active counts exist, renders a per-card `신뢰도 높음` badge only when `is_highly_reliable` is `true`, renders a per-card `N회 주입` badge when `injected_count > 0` and adds `(R% 적용)` when `applied_count > 0`, shows a short inline `선호도로 자동 저장됨` notice with a `선호에서 보기` link when an auto-activation response event arrives, lets recent correction rows open `data-testid="correction-detail-panel"` with original text, corrected text, and delta-summary reason, and elevates the conflict badge to amber only when `conflict_severity` is `"high"` without changing activate/pause/reject behavior
...
- not implemented:
  ...
  - durable preference activation and automatic cross-session application (accepted reviewed candidates are now persisted as local preference candidates through the existing preference store, but activation and cross-session application remain not-implemented)
...
- active context metadata (`active_context` session field for follow-up answers, updated by correction-submit `summary_hint`); `active_context` now also carries an explicit `summary_hint_basis` field (`current_summary` | `recorded_correction`) that is seeded as `current_summary` when the document/search/web active context is built and flipped to `recorded_correction` when the correction submit rewrites `active_context.summary_hint`; the document context box labels this `summary_hint` block as `후속 질문 / 재요약 기준 (기록된 수정본):` when `summary_hint_basis` is `recorded_correction` and `후속 질문 / 재요약 기준 (현재 요약):` otherwise, and the correction helper copy reflects the same basis contract (no recorded correction → follow-ups use the original summary; a recorded correction → follow-ups use the recorded correction; unrecorded editor change → follow-ups still use the last recorded correction until `수정본 기록` is submitted again)
...
- session JSON with messages, `active_context` (follow-up context updated by correction-submit `summary_hint`, with explicit `summary_hint_basis = current_summary | recorded_correction`), `pending_approvals`, `permissions` (`{web_search, web_search_label}`), timestamps
...
- stores local JSON history with in-session reload and history-card badges (answer-mode badges, color-coded verification-strength badges, color-coded source-role trust badges)
- supports entity-card and latest-update answer-mode distinction with separate verification labels, source-role surfaces, claim coverage panel (status tags, actionable hints, fact-strength summary bar, dedicated plain-language focus-slot reinvestigation explanation covering reinforced / regressed / still single-source / still unresolved / or remains in an explicit `정보 상충` state), and entity-card strong-badge downgrade
...
- the internal controller Office View renders a main operator attention board for active `needs_operator` / `automation_health=needs_operator` states, showing the runtime-provided reason, target lane/role or repository release gate, required decision, evidence summary, control file/seq, and next action; missing reason metadata is surfaced as `개입 필요 사유 누락` rather than hidden behind a generic stop
...
- the watcher-owned `turn_state.json` mirror now includes `active_role` and `active_lane`, so pipeline GUI current-turn summaries can follow the actual bound owner lane even when implement/verify ownership swaps
...
- `active_context` — `{kind, label, source_paths, summary_hint, summary_hint_basis, suggested_prompts, record_path, claim_coverage_progress_summary}`; updated by correction-submit `summary_hint` (and `summary_hint_basis` flipped from `current_summary` to `recorded_correction` on correction submit)
- `created_at`
- `updated_at`
...
- `review_queue_items`
  ...
  - each source-message item carries `source_session_id`, `source_session_title`, up to three preceding `context_turns`, and `evidence_summary` counts for artifacts, memory signals, confirmations, and recurring sessions; global recurring items keep `context_turns = []` while still carrying source-session linkage and evidence-summary counts
  - not persisted as a separate store field
...
- `actions_taken` — list of actions taken during processing (default `[]`, never `null`)
...
- `follow_up_suggestions` — list of localized follow-up suggestion strings (default `[]`, never `null`)
- `search_results` — list of `{path, matched_on, snippet}` for document search result preview (default `[]`, never `null`)
...
- preference reliability aggregate header in `PreferencePanel`: when at least one active preference exists, the header renders `총 적용 N회 · 총 교정 N회` from active preference totals (`total_applied`, `total_corrected`) while preserving each card's per-preference `적용 N회 · 교정 N회` line
- preference quality aggregate header in `PreferencePanel`: `/api/preferences` includes `high_quality_active_count` for active preferences with `quality_info.is_high_quality == True`, and the header renders `고품질 N개` only when that count is greater than zero while preserving each card's `고품질` badge behavior
- preference reliability signal badge and aggregate header in `PreferencePanel`: `/api/preferences` includes per-preference `is_highly_reliable`; when `injected_count >= 3` and `injection_correction_rate > 0.25`, the projected value is forced `False`; otherwise a stored explicit value is honored, while records without one use `quality_info.is_high_quality == True`, `reliability_stats.applied_count >= 3`, and `reliability_stats.corrected_count / reliability_stats.applied_count < 0.15`; user-triggered `activate_preference()` stores `is_highly_reliable = True`, so a manually activated reviewed-candidate preference is eligible for immediate session injection without waiting for runtime reliability stats unless later injection-correction evidence crosses the demotion threshold; `/api/preferences` also includes active-only `highly_reliable_active_count`, and the header renders `신뢰도 높음 N개` only when that count is greater than zero while each card renders `신뢰도 높음` only when `is_highly_reliable` is true; the existing `reliability_stats` / `quality_info` payload structures remain unchanged
- preference session injection relevance: `AgentLoop._get_active_preferences()` accepts optional `user_input` and, after the existing reliable-preference eligibility filter, uses `_preference_context_terms()` to extract valid terms (lowercase, length ≥ 2, stop-words removed) and `_select_context_relevant_preferences()` to rank preferences by overlap-term count descending with `is_highly_reliable == True` preferences first on equal scores; when there is no user input, all terms are stop-words, or no preferences overlap, it keeps the existing `fallback_all` behavior and returns the full eligible candidate set; each injected preference records one `preference_injected` task-log event with `reason`, `preference_id`, and `user_input_snippet`
- preference injection feedback visibility: `get_global_audit_summary()` creates a zeroed `per_preference_stats` entry for valid `preference_injected` events that have no prior applied/corrected stats, then increments `injected_count`; when a session has both injected preferences and a correction event, the same stats entry increments `injection_correction_count` once per injected preference for that session; `/api/preferences` exposes `injected_count`, `injection_correction_count`, and `injection_correction_rate` as per-preference top-level fields while keeping `reliability_stats` limited to `applied_count` and `corrected_count`; the shared reliability projection consumes those top-level fields to demote preferences with at least three injections and a correction-after-injection rate above 25%; `PreferencePanel` displays `N회 주입` for positive counts and adds `(R% 적용)` when `applied_count > 0`
- preference auto-activation notice in `PreferencePanel`: correction-submit responses may include `auto_activated: true` and `preference_id` when repeated corrections activate a highly reliable preference; the React preview shell routes that event to `PreferencePanel`, which displays a short `선호도로 자동 저장됨` inline notice and a `선호에서 보기` link targeting `#pref-card-{preference_id}`
- correction history detail in `PreferencePanel`: `GET /api/corrections/<correction_id>` returns correction detail JSON, `fetchCorrectionDetail()` calls that endpoint, and selecting a recent correction row renders `data-testid="correction-detail-panel"` with original text, corrected text, and delta-summary reason
- 선호 삭제: PreferencePanel의 각 선호 카드에서 `data-testid="delete-preference-btn"` 명시적 삭제 버튼으로 `deletePreference()`를 호출해 선호를 제거할 수 있다; ACTIVE 선호는 삭제 전 비활성화되어 이후 주입에서 제외된다
- 선호 신뢰도 토글: PreferencePanel의 각 선호 카드에서 `data-testid="toggle-reliability-btn"` 버튼으로 `is_highly_reliable` 값을 반전할 수 있다; 저장된 명시적 값은 일반 runtime projection보다 우선하지만 `injected_count >= 3` 및 `injection_correction_rate > 0.25` 강등 조건이 최종 우선하며, ACTIVE 선호의 신뢰도 변경은 다음 주입 사이클부터 반영된다
- 선호 내용 편집: PreferencePanel의 각 선호 카드에서 `data-testid="edit-preference-btn"` 버튼으로 인라인 편집 모드를 열어 `corrected_text`를 수정할 수 있다; `data-testid="save-preference-text-btn"`으로 저장하며, 빈 텍스트는 저장 불가
- correction history 상태 필터: PreferencePanel의 교정 이력 섹션에서 `data-testid="correction-status-filter"` 드롭다운으로 교정 상태(전체/RECORDED/CONFIRMED/PROMOTED/SUPERSEDED)별로 목록을 필터링할 수 있다; 기존 `/api/corrections/list?status=` API를 재사용
- correction history 검색 및 목록 확장: PreferencePanel의 교정 이력 섹션에서 `data-testid="correction-search-input"` 검색 입력으로 텍스트 검색이 가능하다; `data-testid="correction-show-more-btn"`으로 기본 표시 한도(20건)를 초과해 더 많은 교정을 불러올 수 있다
- correction history pagination: PreferencePanel의 `data-testid="correction-show-more-btn"` 버튼이 offset 기반으로 다음 배치를 append 방식으로 불러온다; 마지막 페이지에서는 버튼이 자동으로 숨겨진다
- preference 검색: PreferencePanel의 선호 목록 섹션에서 `data-testid="preference-search-input"` 검색 입력으로 클라이언트-사이드 텍스트 필터링이 가능하다; description, corrected_text, original_snippet, status 필드를 검색 대상으로 한다
- preference list pagination: PreferencePanel의 `data-testid="preference-show-more-btn"` 버튼이 offset 기반으로 다음 preference batch를 append 방식으로 불러온다; 마지막 페이지에서는 버튼이 자동으로 숨겨진다; 기본 limit는 20건
- review queue search filter: ReviewQueuePanel 상단 `data-testid="review-queue-search-input"` 검색 입력으로 현재 로드된 review queue items를 `statement` 텍스트 기준 클라이언트 사이드 필터링한다; 검색 결과가 없으면 `검색 결과 없음` 메시지를 표시한다; 별도 API 호출 없이 세션 payload에서 로드된 items 대상 동작
- review queue item count: ReviewQueuePanel의 `data-testid="review-queue-item-count"` 헤더가 현재 로드된 항목 수를 표시한다; 검색 필터 적용 중이면 `{F} / {N}개 항목 (검색 결과)` 형태로 표시하고, 필터 없으면 `{N}개 항목`을 표시한다; 클라이언트 사이드 파생 — 별도 API 호출 없음
- review queue item badges: ReviewQueuePanel 각 항목 row에 Age(`review-queue-item-age`, derived_at 기준 N분/시간/일 전), Family(`review-queue-item-family`, candidate_family 값), Quality(`review-queue-item-quality`, is_high_quality === true일 때만 고품질) 배지를 인라인으로 표시한다; 기존 ReviewQueueItem 필드만 사용하며 별도 API 호출 없음
- review queue inline actions: ReviewQueuePanel 각 항목에서 사유 입력(review-reason-note)과 accept/defer/reject 액션 버튼이 context-turns 섹션보다 앞(evidence badges 직후)에 렌더링된다; 긴 증거·맥락 섹션이 있어도 스크롤 없이 액션 버튼에 즉시 접근 가능; 로직·API 변경 없음
- preference conflict severity signal in `PreferencePanel`: each enriched `conflict_info` includes `conflict_severity`, with `"high"` when either the preference or any conflicting preference is `is_highly_reliable=True`, `"normal"` for other active conflicts, and `"none"` when no conflict; the conflict badge uses elevated amber styling only for `"high"` while preserving the existing normal conflict badge behavior
- preference conflict aggregate header in `PreferencePanel`: `/api/preferences` includes `high_severity_conflict_count` for active preferences with `conflict_info.conflict_severity == "high"`; the header renders `충돌 위험 N건` with `data-testid="high-severity-conflict-count"` only when that count is greater than zero; existing per-card conflict badge behavior is unchanged
...
- verified-vs-uncertain explanation: response-body section headers annotated with matching status tags (`확인된 사실 [교차 확인]`, `상충하는 정보 [정보 상충]`, `단일 출처 정보 [단일 출처]`, `확인되지 않은 항목 [미확인]`), claim-coverage panel hint maps tags to explanations
- slot-level reinvestigation UX: claim-coverage panel shows a dedicated plain-language explanation line for each `is_focus_slot` slot, telling the user whether the slot was reinforced, regressed, is still single-source, is still unresolved, or remains in an explicit `정보 상충` state after reinvestigation; non-focus slots keep the lighter status/hint rendering
...
- claim coverage panel with status tags (`[교차 확인]`, `[정보 상충]`, `[단일 출처]`, `[미확인]`), actionable hints for weak or unresolved slots, source role with trust level labels, color-coded fact-strength summary bar, and dedicated plain-language focus-slot reinvestigation explanation (reinforced / regressed / still single-source / still unresolved / or remains in an explicit `정보 상충` state)
- entity-card agreement-over-noise baseline: multi-source consensus items preferred before single-source claims, noisy single-source items capped; claim-slot `strong` (`[교차 확인]`) coverage now requires trusted agreement (≥2 distinct trusted-role supporters on the chosen value) and is downgraded to `weak` when a competing same-slot value also has its own trusted agreement, so raw untrusted-only multi-source agreement no longer counts as cross-verified
- weak-slot reinvestigation baseline: weak/missing/conflict slots all eligible for reinvestigation suggestions, with MISSING preferred over WEAK and CONFLICT below both, weak slots rendered as uncertain rather than stable facts, progress and focus-slot state recorded; legacy stored `claim_coverage` slot labels (`개발사`, `장르`, `플랫폼`, `출시일`) are normalized in memory to current core slots (`개발`, `장르/성격`, `이용 형태`, `상태`) for reinvestigation prompt selection, for the reload response `claim_coverage` public surface, for reload-follow-up progress comparison, and for stored `claim_coverage_progress_summary` public text (on reload via `active_context` and on `session.web_search_history` history-card metadata) — so history-record reload and reload-follow-up keep canonical slot names, keep targeted suggestions, report truthful `unchanged` / `재조사했지만 ... 아직 단일 출처 상태입니다` progress instead of a false `미확인 -> 단일 출처` improvement, and surface canonical progress-summary wording (`재조사했지만 이용 형태는 아직 단일 출처 상태입니다.`) instead of legacy slot names. Both the `load_web_search_record_id` reload path and the natural-language show-only reload path return `claim_coverage` and `claim_coverage_progress_summary` at the top level of the response payload (and on the appended session message) with identical values to the reload `active_context`, keeping the two reload paths aligned on the same public contract. Reload-follow-up answers (`load_web_search_record_id + user_text` follow-ups and natural-language reload followed by a plain follow-up) that reuse the internal web-search `active_context` also propagate the same top-level `claim_coverage` and `claim_coverage_progress_summary` from the context, so the browser's claim-coverage panel and fact-strength bar stay visible across reload-follow-up chains on entity records that actually carry claim coverage. Latest-update and other no-claim-coverage contexts keep these top-level fields empty so their current empty-meta contract is not widened. Persisted history JSON files are not rewritten; the compatibility layer is applied at read/use time only.
- verified-vs-uncertain explanation: response-body sections annotated with status tags (`[교차 확인]`, `[정보 상충]`, `[단일 출처]`, `[미확인]`) matching claim-coverage panel statuses
...
- correction submit also updates the current session `active_context.summary_hint` to the corrected text (truncated to 240 chars) and flips `active_context.summary_hint_basis` from `current_summary` to `recorded_correction`, so subsequent same-session follow-up responses and re-summaries use the corrected version as their basis rather than the original draft and the browser can label the basis without inferring it from text shape
...
- the first post-aggregate promotion boundary is now fixed above that shipped aggregate surface:
  ...
  - current reviewed-but-not-applied traces remain narrower than the shipped reviewed-memory layer:
    ...
    - reviewed memory is now a shipped rollbackable, disableable, conflict-visible, operator-auditable layer above both (apply / stop-apply / reversal / conflict-visibility are shipped; promotion and cross-session counting remain later)
...
- current next practical path:
  ...
  - the current shipped review actions (`accept`/`reject`/`defer`) now also consume that same current source-message-anchored basis instead of inventing a second canonical candidate source
  ...
  - satisfying the full family means one exact internal capability-source family resolves actual reviewed-memory-layer presence for the same exact aggregate scope; this path is now shipped through the current source-family-plus-basis chain:
    ...
    - `disable_ready_reviewed_memory_effect` is satisfied only when one internal `disable_source_ref` can point to an effect-capability source that can actually stop apply without claiming reversal
    ...
    - `conflict_visible_reviewed_memory_scope` is satisfied only when one internal `conflict_source_ref` can point to a local source that actually exposes competing reviewed-memory targets inside the same exact scope
    - `operator_auditable_reviewed_memory_transition` is satisfied only when one internal `transition_audit_source_ref` can point to a local source that can emit canonical transition records separate from `task_log`
  ...
  - the next widening should still stay closed:
    ...
    - the reviewed-memory apply result is now shipped (apply / stop-apply / reversal / conflict-visibility); boundary draft stays read-only and separate from the apply result
...
- current shipped first slice:
  ...
  - source-message queue items also carry `source_session_id` / `source_session_title`, up to three preceding `context_turns` with compact role/text/message metadata, and an `evidence_summary` with artifact, signal, confirmation, and recurring-session counts; the panel renders these as source-session text, a small conversation-context block, and evidence-count chips
  - the current queue supports four review actions:
    - `accept` / `reject` / `defer` / `edit`
    - all four remain reviewed-but-not-applied only
    - review submissions can carry an optional `reason_note`; `edit` still uses that field for the operator's edited statement text, while accepted/rejected reviewed candidates may use it as decision rationale in preference source refs
...
- minimum shipped `candidate_review_record` contract:
  ...
  - optional `reason_note` (present when the review submission includes an explicit note)
  - `recorded_at`
...
- current shipped review action implementation:
  ...
  - the result stays reviewed-but-not-applied:
    ...
    - `reason_note` stores review rationale or an edited review statement depending on the submitted action; it does not rewrite source-message text
- reviewed-candidate preference audit contract:
  - a reviewed-candidate preference source ref may carry `session_id`, optional `session_title`, optional `reason_note`, `review_action`, `candidate_id`, `candidate_updated_at`, `artifact_id`, and `source_message_id`
  - `list_preferences_payload` exposes the latest relevant `reason_note` as `review_reason_note`, source session title as `source_session_title`, top-level status counts including `paused_count`, active-only reliability aggregates `total_applied` / `total_corrected`, active-only quality aggregate `high_quality_active_count`, active-only highly reliable aggregate `highly_reliable_active_count`, active-only high severity conflict aggregate `high_severity_conflict_count`, per-preference top-level `injected_count`, `injection_correction_count`, and `injection_correction_rate`, per-preference `is_highly_reliable` forced false when `injected_count >= 3` and `injection_correction_rate > 0.25`, otherwise from an explicit stored value when present and otherwise from high quality, at least three applications, and a correction rate below 15%, and every enriched `conflict_info.conflict_severity` (`"high"` / `"normal"` / `"none"`); `PreferencePanel` renders positive `injected_count` as `N회 주입` and adds `(R% 적용)` when `reliability_stats.applied_count > 0`
  - user-triggered `activate_preference()` now stores `is_highly_reliable = True` on the preference record, so a reviewed-candidate preference manually activated from the review/preference flow is immediately eligible for `AgentLoop._get_active_preferences()` session injection without waiting for runtime `applied_count` or `quality_info.is_high_quality`, unless later injection-correction evidence crosses the reliability demotion threshold; `_auto_activate_candidate_if_ready()` remains the statistics-based auto-activation path and is not broadened by this manual activation rule
  - `AgentLoop._get_active_preferences(user_input=..., session_id=...)` now narrows that eligible injection set using `_preference_context_terms()` (stop-word removal, length ≥ 2 filter) and `_select_context_relevant_preferences()` (overlap-term count descending, `is_highly_reliable` tiebreaker); stop-word-only or absent `user_input` falls back to the full eligible set with `fallback_all`; task-log failures while recording `preference_injected` do not block preference injection
  - `get_global_audit_summary()` in both `SessionStore` and `SQLiteSessionStore` now includes `injected_count` and `injection_correction_count` in each `PerPreferenceStats` entry, counting `preference_injected` task-log events per preference id, creating zeroed entries for valid preference ids that have only injection events, and counting one injection-correction session when the same session has an injected preference plus `correction_submitted`, corrected text, corrected outcome, or explicit preference-correction evidence; this enables tracking the injection-to-application and injection-to-correction rates alongside `applied_count` and `corrected_count`, and `/api/preferences` exposes those values as top-level per-preference `injected_count`, `injection_correction_count`, and `injection_correction_rate`; the shared reliability projection uses the rate to demote preferences after at least three injections when the injection-correction rate is above 25%
...
- `make e2e-test` runs through `e2e/start-server.sh`, which first health-checks `http://127.0.0.1:8879/healthz`; it reuses an already healthy smoke server, otherwise starts an isolated mock `app.web` server with inherited provider/model overrides cleared, waits until healthy, and cleans up the temporary server/SQLite DB after Playwright exits
- Playwright smoke coverage for:
  ...
  - review queue source session, previous conversation context, evidence-summary counts, optional review reason note, and preference audit trace visibility
