...
- preference reliability signal badge and aggregate header in `PreferencePanel`: `/api/preferences` includes per-preference `is_highly_reliable`; a stored explicit value is honored first, while records without one use `quality_info.is_high_quality == True`, `reliability_stats.applied_count >= 3`, and `reliability_stats.corrected_count / reliability_stats.applied_count < 0.15`; user-triggered `activate_preference()` stores `is_highly_reliable = True`, so a manually activated reviewed-candidate preference is eligible for immediate session injection without waiting for runtime reliability stats; `/api/preferences` also includes active-only `highly_reliable_active_count`, and the header renders `신뢰도 높음 N개` only when that count is greater than zero while each card renders `신뢰도 높음` only when `is_highly_reliable` is true; the existing `reliability_stats` / `quality_info` payload structures remain unchanged
...
- 선호 신뢰도 토글: PreferencePanel의 각 선호 카드에서 `data-testid="toggle-reliability-btn"` 버튼으로 `is_highly_reliable` 값을 반전할 수 있다; 저장된 명시적 값이 runtime projection보다 우선하며, ACTIVE 선호의 신뢰도 변경은 다음 주입 사이클부터 반영된다
