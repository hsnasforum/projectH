...
55. Entity-card 붉은사막 browser natural-reload second-follow-up noisy single-source exclusion Playwright smoke coverage (negative `출시일` / `2025` / `blog.example.com` in response text and origin detail + `namu.wiki`, `ko.wikipedia.org` context box continuity)
