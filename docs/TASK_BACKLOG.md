...
41. History-card entity-card zero-strong-slot `다시 불러오기` reload response-origin + answer-mode + verification-label + source-path continuity Playwright smoke coverage (`WEB` badge, `설명 카드` answer-mode badge, downgraded `설명형 단일 출처`, `백과 기반`, `namu.wiki`, `ko.wikipedia.org` in context box)
42. History-card entity-card zero-strong-slot `다시 불러오기` follow-up response-origin + source-path continuity service + Playwright smoke coverage (`WEB` badge, `설명 카드`, `설명형 단일 출처`, `백과 기반`, `namu.wiki`, `ko.wikipedia.org` drift prevention)
43. History-card entity-card zero-strong-slot click-reload second-follow-up response-origin + source-path continuity Playwright smoke + natural-reload service coverage (`WEB` badge, `설명 카드`, `설명형 단일 출처`, `백과 기반`, `namu.wiki`, `ko.wikipedia.org` drift prevention)
