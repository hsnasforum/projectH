...
- next phase target: preference injection, visibility, and reliability feedback loop shipped in M49–M52 (cross-session ACTIVE+is_highly_reliable=True 선호 주입; 웹 조사 제외; applied/corrected 통계; 명시적 교정 피드백; 신뢰도 저하 경고). Remaining: M54–M58 TypedDict layer complete; M61–M70 Correction Lifecycle axis complete; M70–M78 v1.5 structural hardening complete (store validation M72–M74, SQLite modularization M75, handler decomp trilogy M70/M77/M78); M79–M83 correction→preference→active observability loop complete (M79 docs, M80 handlers/__init__, M81 auto-activate, M82 activated_count, M83 promote-result feedback UI); M84 doc-sync (M81-M83); M85 correction reliability seed + preference reliability feedback + dist/E2E + automation health; M86 Axis 1 SQLite preference candidate query parity (get_candidates, find_by_fingerprint); M87 Axis 1 SQLiteSessionStore.get_global_audit_summary() parity; M88 Axis 1 list_preferences_payload() candidate_preferences wiring (get_candidates()); M89 Axis 1 PreferencesPayload TypeScript type sync (candidate_preferences); M90 Axis 1-2 PreferencePanel candidatePreferences state + dist rebuild + E2E; M91 Axis 1-2 filteredPreferences 후보 탭 candidatePreferences 분기 + dist rebuild + E2E; M92 Axis 1 PreferencesPayload high_severity_conflict_count 타입 완결 + 캐스트 제거; M93 Axis 1 conflict_info 불필요 타입 캐스트 제거 (M85-M93 arc 우회 캐스트 완전 제거); M94 Axis 1-2 applied preferences 팝오버 신뢰도 높음 배지 + dist rebuild + E2E; M95 Axis 1-2 팝오버 선호에서 보기 이동 링크 + dist rebuild + E2E; M96 Axis 1-2 pref-navigate-to-card data-testid 추가 + dist rebuild (팝오버 data-testid 패턴 완결); M97 Axis 1 correction-submit auto activation notice source + React preview wiring complete; M98 Axis 1-2 correction history detail (`GET /api/corrections/<correction_id>`, `fetchCorrectionDetail`, `correction-detail-panel`) + dist rebuild complete; M99 advisory-loop-recovery-guard (watcher runtime — product feature 아님; DEFAULT_ADVISORY_RECOVERY_SEC 상수화, prompt quality, control seq monotonicity fix, root memory doc-sync); M102 preference delete (Axis 1: DELETE /api/preferences/<id> + deletePreference() + delete-preference-btn UI; Axis 2: dist rebuild + preference delete removes preference from list E2E smoke); M103 preference reliability toggle (Axis 1: POST /api/preferences/<id>/toggle-reliability + togglePreferenceReliability() + toggle-reliability-btn UI + preference_utils explicit reliability; Axis 2: dist rebuild + preference reliability toggle updates badge E2E smoke); M104 preference text edit (Axis 1: PATCH /api/preferences/<id> + editPreferenceText() + edit-preference-btn/save-preference-text-btn UI; Axis 2: dist rebuild + preference text edit updates corrected text E2E smoke); M105 correction history status filter (Axis 1: correction-status-filter UI in PreferencePanel using existing GET /api/corrections/list?status= API; Axis 2: dist rebuild + correction history status filter narrows list E2E smoke); M106 correction search expansion (Axis 1: limit=5 hardcoding 제거 + query/limit params + correction-search-input + correction-show-more-btn UI; Axis 2: dist rebuild + correction history search filters by query E2E smoke); M107 correction history pagination (Axis 1: offset param in list_filtered + GET /api/corrections/list?offset= + fetchCorrectionList offset? + correction-show-more-btn append mode + correctionListHasMore; Axis 2: dist rebuild + correction history show more appends next page E2E smoke); M108 preference search visibility parity (Axis 1: preference-search-input client-side filter in PreferencePanel by description/corrected_text/status — no API change; Axis 2: dist rebuild + preference search filters preference list E2E smoke); M109 preference list pagination (Axis 1: limit/offset in preference_store + list_preferences_payload + GET /api/preferences?limit=&offset= + fetchPreferences limit/offset + preference-show-more-btn append mode + preferenceListHasMore; Axis 2: dist rebuild + preference show more appends next page E2E smoke); M110 Review Queue 검색 필터 (Axis 1: review-queue-search-input client-side filter in ReviewQueuePanel by statement text — no API change; Axis 2: dist rebuild + review queue search filter narrows list E2E smoke); Fix: controller lane state display truth (cozy.js effectiveLaneState single-truth, pipeline_gui READY+closeout label); pipeline runtime zombie pidfile guard (cli.py _pid_is_zombie); M111 Review Queue 항목 수 헤더 (Axis 1: review-queue-item-count client-side count header — {N}개 항목 / {F}/{N}개 항목 (검색 결과) — no API change; Axis 2: dist rebuild + review queue item count reflects filter state E2E smoke); M112 Review Queue 항목 배지 (Axis 1: review-queue-item-age/family/quality inline badges using derived_at/candidate_family/quality_info.is_high_quality — no API change; Axis 2: dist rebuild + review queue item badges show age family and quality E2E smoke); M113 Review Queue 액션 인라인 이동 (Axis 1: JSX 재배치 — reason textarea + accept/defer/reject buttons moved before context-turns section — no logic/style/testid change; Axis 2: dist rebuild + review queue action buttons appear before context section E2E smoke); M114 user-triggered `activate_preference()` explicit `is_highly_reliable=True` persistence + docs sync complete (manual activation immediately qualifies reviewed-candidate preferences for session injection; auto-activation remains statistics-based); M115 `_get_active_preferences(user_input=...)` context relevance + `preference_injected` audit docs sync complete; M116 stop-word filter + overlap-score ranking + `is_highly_reliable` tiebreaker docs sync complete; M117 `injected_count` in `PerPreferenceStats` + `get_global_audit_summary()` injection event scan docs sync complete; M118 injection-only stats creation + `/api/preferences` top-level `injected_count` + PreferencePanel injection badge + E2E complete; M119 injection-correction stats + `/api/preferences` top-level rate exposure complete; M120 injection-correction reliability demotion complete (`injected_count >= 3` and rate > 25% forces low reliability); broader cross-session memory remains later; M121 watcher self-restart lease reclamation complete (PaneLease.archive_for_restart() — 4 slot archive before _terminate_pid_file(), 3 regression tests, PR #117 merged); M122 Axis 1 trust-gated multi-source agreement complete (trusted peer ≥4 gate in _entity_source_fact_agreement_score; STRONG requires trusted_source_count ≥ 2; 2 regression tests; entity-card noise reduction in progress); M122 Axis 2 UNRESOLVED status separation complete (CoverageStatus.UNRESOLVED for trusted_count=0; probe-first + 2-query boost in _build_entity_second_pass_queries; display/hint propagation pending as Axis 3); M122 Axis 3 UNRESOLVED display/hint propagation complete (_claim_coverage_status_label "미해결"; rank 0; unresolved_slots set expanded; 156 tests pass; M122 entity-card investigation quality arc complete); M123 Axis 1 UNRESOLVED early-return suppression complete (_build_entity_second_pass_queries now guards early return when unresolved_slots is non-empty; 158 tests pass).
- long-term north star: teachable local personal agent with later approval-gated local action
- operator tooling (`controller.server`, `pipeline_gui/`, `windows-launchers/`, `_data/`) remains outside the current release gate unless explicitly promoted; the cozy controller runtime lives under shared `/controller-assets/js/cozy.js` ownership and is loaded from `controller/index.html` via a single `<script src="/controller-assets/js/cozy.js">` tag instead of one standalone inline runtime copy; controller has its own Playwright smoke (`e2e/playwright.controller.config.mjs`, `make controller-test`) covering shared cozy module load, scene feature hooks for the time-of-day window / pneumatic tube / courier packet-owl / pettable cat-audio path via `window.getSceneDebug()` and `window.testPetCat()`, toolbar chip, event-log storage warning, deduplicated repeated `/api/runtime/status` fetch failures with one recovery event after reconnect, runtime-owned automation attention detail in the Incident Room, main operator attention board rendering for active `needs_operator` reason/action/evidence plus lane-less repository release-gate stops, agent-card fatigue observability (`data-fatigue` attribute), deterministic `fatigued`/`coffee` state transitions via `window.setAgentFatigue` test hook, zone-bounded idle roam (home desk zone) via `window.testPickIdleTargets` + `window.getRoamBounds` test hooks, zone-isolation stacking avoidance via `window.testAntiStacking` test hook (delegates to `testPickIdleTargets` because separate desk zones already keep agents apart), and `window.testHistoryPenalty` returning an empty array because zone-bounded idle roam uses continuous micro-roam rather than discrete spot history; smoke port is configurable via `CONTROLLER_SMOKE_PORT` (default `8781`); `ready`/`idle` agents perform zone-bounded continuous micro-roam inside their own desk zone (`claude_desk`, `codex_desk`, `gemini_desk`) with zone-rect clamping so they never land on other agents' desks or outside the zone; zone isolation removes the need for proximity anti-stacking or `_roamHistory` spot penalty; 오래 방치된 `operator_request.md`는 watcher가 Codex re-triage로 다시 넘기고 supervisor canonical `control` block은 그 동안 `needs_operator`를 숨겨 internal controller/launcher가 같은 stop에 고정되지 않게 합니다
- controller Queue presentation coverage now includes a browser-level controller smoke scenario for sidebar `Queue` row and marquee payload text, plus a socket-free `controller/js/cozy.js` Queue behavior guard in `tests/test_controller_queue_presentation.py`; local Playwright execution may still be environment-held by `local_socket_guard_auto_held`, so this is coverage documentation for operator tooling, not a release/full-smoke pass claim
...
9. PDF text-layer handling (readable → visible summary body with `문서 요약` label in quick meta + transcript meta) and OCR-not-supported guidance (scanned/image-only → visible `요약할 수 없습니다`, `OCR`, `이미지형 PDF`, `다음 단계:`)
10. Permission-gated web investigation with local JSON history (in-session reload, history-card badges: answer-mode badges, color-coded verification-strength badges, color-coded source-role trust badges, investigation progress summary), entity-card / latest-update answer-mode distinction with separate verification labels and entity-card strong-badge downgrade
11. Claim coverage panel with status tags, actionable hints, source role with trust level labels, a 4-segment color-coded fact-strength summary bar (`교차 확인 / 정보 상충 / 단일 출처 / 미확인`), and dedicated plain-language focus-slot reinvestigation explanation (reinforced / regressed / still single-source / still unresolved / or remains in an explicit `정보 상충` state)
...
21. First review actions (`accept`/`reject`/`defer`) on the shipped queue with source-message `candidate_review_record`, pending queue removal, reviewed-but-not-applied semantics, and focused regression
...
27. History-card latest-update `다시 불러오기` noisy community source exclusion Playwright smoke coverage (negative `보조 커뮤니티` / `brunch` in response body, origin detail, and context box + `기사 교차 확인`, `보조 기사`, `hankyung.com`, `mk.co.kr` positive retention)
...
29. History-card entity-card `다시 불러오기` dual-probe source-path + response-origin exact-field drift-prevention Playwright smoke coverage (`pearlabyss.com/200`, `pearlabyss.com/300` in context box, `WEB` badge, `설명 카드`, `설명형 다중 출처 합의`, `공식 기반` · `백과 기반`)
30. History-card latest-update `다시 불러오기` mixed-source source-path + response-origin exact-field drift-prevention Playwright smoke coverage (`store.steampowered.com`, `yna.co.kr` in context box, `WEB` badge, `최신 확인`, `공식+기사 교차 확인`, `보조 기사` · `공식 기반`)
31. History-card latest-update single-source `다시 불러오기` verification-label exact-field drift-prevention Playwright smoke coverage (`단일 출처 참고`, `보조 출처` in origin detail)
32. History-card latest-update news-only `다시 불러오기` verification-label exact-field drift-prevention Playwright smoke coverage (`기사 교차 확인`, `보조 기사` in origin detail)
33. History-card latest-update news-only `다시 불러오기` source-path exact-field drift-prevention Playwright smoke coverage (`hankyung.com`, `mk.co.kr` in context box)
34. History-card latest-update single-source `다시 불러오기` source-path exact-field drift-prevention Playwright smoke coverage (`example.com/seoul-weather` in context box)
35. History-card latest-update single-source `다시 불러오기` follow-up response-origin exact-field drift-prevention service + Playwright smoke coverage (`단일 출처 참고`, `보조 출처` drift prevention)
36. History-card latest-update news-only `다시 불러오기` follow-up response-origin exact-field drift-prevention service + Playwright smoke coverage (`기사 교차 확인`, `보조 기사` drift prevention)
37. History-card entity-card `다시 불러오기` follow-up dual-probe source-path + response-origin exact-field drift-prevention service + Playwright smoke coverage (`pearlabyss.com/200`, `pearlabyss.com/300` in context box, `WEB` badge, `설명 카드`, `설명형 다중 출처 합의`, `공식 기반` · `백과 기반` drift prevention)
38. History-card latest-update mixed-source `다시 불러오기` follow-up source-path + response-origin exact-field drift-prevention service + Playwright smoke coverage (`store.steampowered.com`, `yna.co.kr` in context box, `WEB` badge, `최신 확인`, `공식+기사 교차 확인`, `보조 기사` · `공식 기반` drift prevention)
39. History-card latest-update single-source `다시 불러오기` follow-up source-path exact-field drift-prevention service + Playwright smoke coverage (`example.com/seoul-weather` in context box)
40. History-card latest-update news-only `다시 불러오기` follow-up source-path exact-field drift-prevention service + Playwright smoke coverage (`hankyung.com`, `mk.co.kr` in context box)
41. History-card entity-card zero-strong-slot `다시 불러오기` reload response-origin + answer-mode + verification-label + source-path exact-field drift-prevention Playwright smoke coverage (`WEB` badge, `설명 카드` answer-mode badge, downgraded `설명형 단일 출처`, `백과 기반`, `namu.wiki`, `ko.wikipedia.org` in context box)
42. History-card entity-card zero-strong-slot `다시 불러오기` follow-up response-origin + source-path exact-field drift-prevention service + Playwright smoke coverage (`WEB` badge, `설명 카드`, `설명형 단일 출처`, `백과 기반`, `namu.wiki`, `ko.wikipedia.org` drift prevention)
43. History-card entity-card zero-strong-slot click-reload second-follow-up response-origin + source-path exact-field drift-prevention Playwright smoke + natural-reload service coverage (`WEB` badge, `설명 카드`, `설명형 단일 출처`, `백과 기반`, `namu.wiki`, `ko.wikipedia.org` drift prevention)
44. Entity-card zero-strong-slot browser natural-reload exact-field Playwright smoke coverage (`방금 검색한 결과 다시 보여줘` path, `WEB` badge, `설명 카드`, `설명형 단일 출처`, `백과 기반`, `namu.wiki`, `ko.wikipedia.org` in context box)
45. Entity-card zero-strong-slot browser natural-reload follow-up response-origin exact-field drift-prevention Playwright smoke coverage (`WEB` badge, `설명 카드`, `설명형 단일 출처`, `백과 기반`, `namu.wiki`, `ko.wikipedia.org` drift prevention)
46. Entity-card 붉은사막 검색 결과 browser natural-reload exact-field + noisy exclusion Playwright smoke coverage (`방금 검색한 결과 다시 보여줘` path, `WEB` badge, `설명 카드`, `설명형 다중 출처 합의`, `백과 기반`, `namu.wiki`, `ko.wikipedia.org` positive retention, `출시일`/`2025`/`blog.example.com` 본문/detail 미노출, context box `blog.example.com` provenance 포함)
47. Entity-card dual-probe browser natural-reload source-path exact-field drift-prevention Playwright smoke coverage (`pearlabyss.com/ko-KR/Board/Detail?_boardNo=200`, `pearlabyss.com/ko-KR/Board/Detail?_boardNo=300` in context box)
...
49. Entity-card dual-probe browser natural-reload follow-up source-path exact-field drift-prevention service + Playwright smoke coverage (`pearlabyss.com/ko-KR/Board/Detail?_boardNo=200`, `pearlabyss.com/ko-KR/Board/Detail?_boardNo=300` in context box)
...
55. Entity-card 붉은사막 browser natural-reload second-follow-up noisy single-source exclusion Playwright smoke coverage (negative `출시일` / `2025` / `blog.example.com` in response text and origin detail + `namu.wiki`, `ko.wikipedia.org`, `blog.example.com` provenance context box continuity)
56. History-card entity-card `다시 불러오기` actual-search source-path plurality + response-origin exact-field drift-prevention service + Playwright smoke coverage (`namu.wiki`, `ko.wikipedia.org` in context box, `WEB` badge, `설명 카드`, `설명형 다중 출처 합의`, `백과 기반`)
57. History-card entity-card `다시 불러오기` follow-up actual-search source-path plurality + response-origin exact-field drift-prevention service + Playwright smoke coverage (`namu.wiki`, `ko.wikipedia.org` in context box, `WEB` badge, `설명 카드`, `설명형 다중 출처 합의`, `백과 기반` drift prevention)
58. History-card entity-card `다시 불러오기` second-follow-up actual-search source-path plurality + response-origin exact-field drift-prevention service + Playwright smoke coverage (`namu.wiki`, `ko.wikipedia.org` in context box, `WEB` badge, `설명 카드`, `설명형 다중 출처 합의`, `백과 기반` drift prevention)
59. History-card entity-card `다시 불러오기` second-follow-up dual-probe source-path + response-origin exact-field drift-prevention service + Playwright smoke coverage (`pearlabyss.com/200`, `pearlabyss.com/300` in context box, `WEB` badge, `설명 카드`, `설명형 다중 출처 합의`, `공식 기반` · `백과 기반` drift prevention)
60. Entity-card dual-probe natural-reload second-follow-up source-path + response-origin exact-field drift-prevention service + Playwright smoke coverage (`pearlabyss.com/ko-KR/Board/Detail?_boardNo=200`, `pearlabyss.com/ko-KR/Board/Detail?_boardNo=300` in context box, `WEB` badge, `설명 카드`, `설명형 다중 출처 합의`, `공식 기반` · `백과 기반` drift prevention)
61. Entity-card 붉은사막 actual-search natural-reload second-follow-up source-path + response-origin exact-field drift-prevention service + Playwright smoke coverage (`namu.wiki`/`ko.wikipedia.org` in context box, `WEB` badge, `설명 카드`, `설명형 다중 출처 합의`, `백과 기반` drift prevention)
62. History-card latest-update mixed-source `다시 불러오기` second-follow-up source-path + response-origin exact-field drift-prevention service + Playwright smoke coverage (`store.steampowered.com`, `yna.co.kr` in context box, `WEB` badge, `최신 확인`, `공식+기사 교차 확인`, `보조 기사` · `공식 기반` drift prevention)
63. History-card latest-update single-source `다시 불러오기` second-follow-up source-path + response-origin exact-field drift-prevention service + Playwright smoke coverage (`example.com/seoul-weather` in context box, `WEB` badge, `최신 확인`, `단일 출처 참고`, `보조 출처` drift prevention)
64. History-card latest-update news-only `다시 불러오기` second-follow-up source-path + response-origin exact-field drift-prevention service + Playwright smoke coverage (`hankyung.com`, `mk.co.kr` in context box, `WEB` badge, `최신 확인`, `기사 교차 확인`, `보조 기사` drift prevention)
...
68. Latest-update mixed-source natural-reload follow-up source-path + response-origin exact-field drift-prevention service + Playwright smoke coverage (`store.steampowered.com`, `yna.co.kr` in context box, `WEB` badge, `최신 확인`, `공식+기사 교차 확인`, `보조 기사` · `공식 기반` drift prevention)
69. Latest-update mixed-source natural-reload second-follow-up source-path + response-origin exact-field drift-prevention service + Playwright smoke coverage (`store.steampowered.com`, `yna.co.kr` in context box, `WEB` badge, `최신 확인`, `공식+기사 교차 확인`, `보조 기사` · `공식 기반` drift prevention)
70. Latest-update single-source natural-reload follow-up source-path + response-origin exact-field drift-prevention service + Playwright smoke coverage (`example.com/seoul-weather` in context box, `WEB` badge, `최신 확인`, `단일 출처 참고`, `보조 출처` drift prevention)
71. Latest-update single-source natural-reload second-follow-up source-path + response-origin exact-field drift-prevention service + Playwright smoke coverage (`example.com/seoul-weather` in context box, `WEB` badge, `최신 확인`, `단일 출처 참고`, `보조 출처` drift prevention)
72. Latest-update news-only natural-reload follow-up source-path + response-origin exact-field drift-prevention service + Playwright smoke coverage (`hankyung.com`, `mk.co.kr` in context box, `WEB` badge, `최신 확인`, `기사 교차 확인`, `보조 기사` drift prevention)
73. Latest-update news-only natural-reload second-follow-up source-path + response-origin exact-field drift-prevention service + Playwright smoke coverage (`hankyung.com`, `mk.co.kr` in context box, `WEB` badge, `최신 확인`, `기사 교차 확인`, `보조 기사` drift prevention)
...
84. Browser file picker readable text-layer PDF success summary Playwright smoke coverage (OCR guidance 미노출, visible summary body with extracted text `local-first approval-based document assistant`, context box + quick meta + transcript meta `readable-text-layer.pdf`, quick meta + transcript meta `문서 요약` label)
...
86. History-card latest-update noisy-community natural-reload reload-only (no follow-up) Playwright smoke coverage (`WEB` badge, `최신 확인` answer-mode badge, `보조 커뮤니티` / `brunch` negative in origin detail, response body, context box + `기사 교차 확인`, `보조 기사`, `hankyung.com`, `mk.co.kr` positive retention, zero-count `.meta` no-leak)
87. History-card entity-card noisy single-source initial-render Playwright smoke coverage (visible `다시 불러오기` button, history-card `.meta` exactly `사실 검증 교차 확인 3 · 미확인 2`, no `설명 카드` / `최신 확인` / `일반 검색` answer-mode label leak inside `.meta`, and `출시일` / `2025` / `blog.example.com` noisy single-source leakage negative at initial render, no click reload / natural reload / follow-up)
88. History-card entity-card actual-search initial-render Playwright smoke coverage (visible `다시 불러오기` button, history-card `.meta` exactly `사실 검증 교차 확인 3 · 미확인 2`, no `설명 카드` / `최신 확인` / `일반 검색` answer-mode label leak inside `.meta`, no click reload / natural reload / follow-up)
89. History-card entity-card dual-probe initial-render Playwright smoke coverage (visible `다시 불러오기` button, history-card `.meta` exactly `사실 검증 교차 확인 1 · 단일 출처 4` mixed count-summary, no `설명 카드` / `최신 확인` / `일반 검색` answer-mode label leak inside `.meta`, no click reload / natural reload / follow-up)
90. History-card latest-update noisy community source initial-render Playwright smoke coverage (visible `다시 불러오기` button, history-card `.meta` count exactly `0`, no `사실 검증` text leak via accidental `.meta` creation, `보조 커뮤니티` / `brunch` noisy leakage negative at initial render, no click reload / natural reload / follow-up)
91. History-card latest-update mixed-source initial-render Playwright smoke coverage (visible `다시 불러오기` button, history-card `.meta` count exactly `0`, no `사실 검증` text leak via accidental `.meta` creation, no click reload / natural reload / follow-up)
92. History-card latest-update single-source initial-render Playwright smoke coverage (visible `다시 불러오기` button, history-card `.meta` count exactly `0`, no `사실 검증` text leak via accidental `.meta` creation, no click reload / natural reload / follow-up)
93. History-card latest-update news-only initial-render Playwright smoke coverage (visible `다시 불러오기` button, history-card `.meta` count exactly `0`, no `사실 검증` text leak via accidental `.meta` creation, no click reload / natural reload / follow-up)
94. History-card entity-card store-seeded actual-search initial-render Playwright smoke coverage (visible `다시 불러오기` button, history-card `.meta` count exactly `0`, no `사실 검증` text leak via accidental `.meta` creation, no click reload / natural reload / follow-up)
95. History-card entity-card store-seeded actual-search 자연어 reload-only Playwright smoke coverage (visible `다시 불러오기` button, history-card `.meta` count exactly `0`, no `사실 검증` text leak, `WEB` / `설명 카드` / `설명형 다중 출처 합의` / `백과 기반` / `namu.wiki` / `ko.wikipedia.org` visible continuity)
96. History-card entity-card store-seeded actual-search `다시 불러오기` click reload-only Playwright smoke coverage (visible `다시 불러오기` button, history-card `.meta` count exactly `0`, no `사실 검증` text leak, `WEB` / `설명 카드` / `설명형 다중 출처 합의` / `백과 기반` / `namu.wiki` / `ko.wikipedia.org` visible continuity)
97. History-card entity-card store-seeded actual-search `다시 불러오기` follow-up Playwright smoke coverage (history-card `.meta` count exactly `0`, no `사실 검증` text leak, `WEB` / `설명 카드` / `설명형 다중 출처 합의` / `백과 기반` / `namu.wiki` / `ko.wikipedia.org` visible continuity drift prevention)
98. History-card entity-card store-seeded actual-search `다시 불러오기` second-follow-up Playwright smoke coverage (history-card `.meta` count exactly `0`, no `사실 검증` text leak, `WEB` / `설명 카드` / `설명형 다중 출처 합의` / `백과 기반` / `namu.wiki` / `ko.wikipedia.org` visible continuity drift prevention)
99. History-card entity-card store-seeded actual-search 자연어 reload chain (자연어 reload → follow-up → second follow-up) Playwright smoke coverage (history-card `.meta` count exactly `0`, no `사실 검증` text leak, `WEB` / `설명 카드` / `설명형 다중 출처 합의` / `백과 기반` / `namu.wiki` / `ko.wikipedia.org` visible continuity drift prevention)
100. History-card entity-card zero-strong-slot `다시 불러오기` Playwright smoke coverage (history-card `.meta` exactly `사실 검증 미확인 5` missing-only count-summary, no `설명 카드` / `최신 확인` / `일반 검색` answer-mode label leak inside `.meta`, `WEB` / `설명 카드` / `설명형 단일 출처` / `백과 기반` / `namu.wiki` / `ko.wikipedia.org` visible continuity)
101. History-card entity-card zero-strong-slot `다시 불러오기` second-follow-up Playwright smoke coverage (history-card `.meta` exactly `사실 검증 미확인 5` missing-only count-summary drift prevention, `WEB` / `설명 카드` / `설명형 단일 출처` / `백과 기반` / `namu.wiki` / `ko.wikipedia.org` visible continuity drift prevention)
102. History-card entity-card zero-strong-slot 자연어 reload second-follow-up Playwright smoke coverage (history-card `.meta` exactly `사실 검증 미확인 5` missing-only count-summary drift prevention, `WEB` / `설명 카드` / `설명형 단일 출처` / `백과 기반` / `namu.wiki` / `ko.wikipedia.org` visible continuity drift prevention)
103. History-card entity-card dual-probe `다시 불러오기` reload Playwright smoke coverage (history-card `.meta` exactly `사실 검증 교차 확인 1 · 단일 출처 4` mixed count-summary, no `설명 카드` / `최신 확인` / `일반 검색` answer-mode label leak inside `.meta`, `WEB` / `설명 카드` / `설명형 다중 출처 합의` / `공식 기반` · `백과 기반` / `pearlabyss.com/200` / `pearlabyss.com/300` visible continuity)
104. History-card entity-card dual-probe `다시 불러오기` second-follow-up Playwright smoke coverage (history-card `.meta` exactly `사실 검증 교차 확인 1 · 단일 출처 4` mixed count-summary drift prevention, no leading/trailing separator artifact in the count-summary line, `WEB` / `설명 카드` / `설명형 다중 출처 합의` / `공식 기반` · `백과 기반` visible continuity drift prevention)
105. Entity-card dual-probe 자연어 reload Playwright smoke coverage (history-card `.meta` exactly `사실 검증 교차 확인 1 · 단일 출처 4` mixed count-summary, no `설명 카드` / `최신 확인` / `일반 검색` answer-mode label leak inside `.meta`, `WEB` / `설명 카드` / `설명형 다중 출처 합의` / `공식 기반` · `백과 기반` visible continuity)
106. Entity-card dual-probe 자연어 reload second-follow-up Playwright smoke coverage (history-card `.meta` exactly `사실 검증 교차 확인 1 · 단일 출처 4` mixed count-summary drift prevention, no leading/trailing separator artifact in the count-summary line, `WEB` / `설명 카드` / `설명형 다중 출처 합의` / `공식 기반` · `백과 기반` visible continuity drift prevention)
107. History-card entity-card actual-search `다시 불러오기` click reload Playwright smoke coverage (history-card `.meta` exactly `사실 검증 교차 확인 3 · 미확인 2` strong-plus-missing count-summary, no `설명 카드` / `최신 확인` / `일반 검색` answer-mode label leak inside `.meta`)
108. History-card entity-card actual-search `다시 불러오기` follow-up Playwright smoke coverage (history-card `.meta` exactly `사실 검증 교차 확인 3 · 미확인 2` strong-plus-missing count-summary drift prevention, no leading/trailing separator artifact in the count-summary line)
109. History-card entity-card actual-search `다시 불러오기` second-follow-up Playwright smoke coverage (history-card `.meta` exactly `사실 검증 교차 확인 3 · 미확인 2` strong-plus-missing count-summary drift prevention, no leading/trailing separator artifact in the count-summary line)
110. Entity-card actual-search 자연어 reload Playwright smoke coverage (history-card `.meta` exactly `사실 검증 교차 확인 3 · 미확인 2` strong-plus-missing count-summary, no `설명 카드` / `최신 확인` / `일반 검색` answer-mode label leak inside `.meta`)
111. Entity-card actual-search 자연어 reload follow-up Playwright smoke coverage (history-card `.meta` exactly `사실 검증 교차 확인 3 · 미확인 2` strong-plus-missing count-summary drift prevention, no leading/trailing separator artifact in the count-summary line)
112. Entity-card actual-search 자연어 reload second-follow-up Playwright smoke coverage (history-card `.meta` exactly `사실 검증 교차 확인 3 · 미확인 2` strong-plus-missing count-summary drift prevention, no leading/trailing separator artifact in the count-summary line)
113. History-card entity-card noisy single-source `다시 불러오기` click reload Playwright smoke coverage (history-card `.meta` exactly `사실 검증 교차 확인 3 · 미확인 2` strong-plus-missing count-summary, no `설명 카드` / `최신 확인` / `일반 검색` answer-mode label leak inside `.meta`)
114. History-card entity-card noisy single-source `다시 불러오기` follow-up Playwright smoke coverage (history-card `.meta` exactly `사실 검증 교차 확인 3 · 미확인 2` strong-plus-missing count-summary drift prevention, no leading/trailing separator artifact in the count-summary line)
115. History-card entity-card noisy single-source `다시 불러오기` second-follow-up Playwright smoke coverage (history-card `.meta` exactly `사실 검증 교차 확인 3 · 미확인 2` strong-plus-missing count-summary drift prevention, no leading/trailing separator artifact in the count-summary line)
116. Entity-card noisy single-source 자연어 reload Playwright smoke coverage (history-card `.meta` exactly `사실 검증 교차 확인 3 · 미확인 2` strong-plus-missing count-summary, no `설명 카드` / `최신 확인` / `일반 검색` answer-mode label leak inside `.meta`)
117. Entity-card noisy single-source 자연어 reload follow-up Playwright smoke coverage (history-card `.meta` exactly `사실 검증 교차 확인 3 · 미확인 2` strong-plus-missing count-summary drift prevention, no leading/trailing separator artifact in the count-summary line)
118. Entity-card noisy single-source 자연어 reload second-follow-up Playwright smoke coverage (history-card `.meta` exactly `사실 검증 교차 확인 3 · 미확인 2` strong-plus-missing count-summary drift prevention, no leading/trailing separator artifact in the count-summary line)
119. History-card entity-card single-source `다시 불러오기` click reload Playwright smoke coverage (stored `{weak:1, missing:1}` count-summary + `단일 출처 상태 1건, 미확인 1건.` progress summary composed into history-card `.meta` exactly `사실 검증 단일 출처 1 · 미확인 1 · 단일 출처 상태 1건, 미확인 1건.`, count-summary before progress summary, no `설명 카드` / `최신 확인` / `일반 검색` answer-mode label leak inside `.meta`, no leading/trailing separator artifact in the composed line)
120. History-card entity-card single-source `다시 불러오기` follow-up Playwright smoke coverage (stored `{weak:1}` count-summary + `단일 출처 상태 1건.` progress summary composed into history-card `.meta` exactly `사실 검증 단일 출처 1 · 단일 출처 상태 1건.`, count-summary before progress summary, no answer-mode label leak inside `.meta`, no leading/trailing separator artifact)
121. History-card entity-card single-source `다시 불러오기` second-follow-up Playwright smoke coverage (stored `{weak:1}` count-summary + `단일 출처 상태 1건.` progress summary composed into history-card `.meta` exactly `사실 검증 단일 출처 1 · 단일 출처 상태 1건.` drift prevention, count-summary before progress summary, no answer-mode label leak inside `.meta`, no leading/trailing separator artifact)
122. History-card entity-card single-source 자연어 reload second-follow-up Playwright smoke coverage (stored `{weak:1}` count-summary + `단일 출처 상태 1건.` progress summary composed into history-card `.meta` exactly `사실 검증 단일 출처 1 · 단일 출처 상태 1건.` drift prevention, count-summary before progress summary, no answer-mode label leak inside `.meta`, no leading/trailing separator artifact)
123. `web-search history card header badges` investigation mixed count+progress composition Playwright smoke coverage (multi-category count-summary + non-empty progress summary composed into history-card `.meta` exactly `사실 검증 교차 확인 2 · 단일 출처 1 · 혼합 지표: 교차 확인과 단일 출처가 함께 관찰되었습니다.`, count-summary before progress summary, no `설명 카드` / `최신 확인` / `일반 검색` answer-mode label leak inside `.meta`, no leading/trailing separator artifact in the composed line)
124. `web-search history card header badges` general label+count+progress composition Playwright smoke coverage (general answer-mode + multi-category count-summary + non-empty progress summary composed into history-card `.meta` exactly `일반 검색 · 사실 검증 교차 확인 2 · 단일 출처 1 · 일반 지표: 커뮤니티 단서와 교차 확인이 함께 관찰되었습니다.`, stable `label → count → progress` composition order, no `혼합 지표:` / `설명 카드` / `최신 확인` investigation answer-mode leak inside `.meta`, no leading/trailing separator artifact in the composed line)
125. `web-search history card header badges` general label+count-only composition Playwright smoke coverage (general answer-mode + single-category count-summary + empty progress summary composed into history-card `.meta` exactly `일반 검색 · 사실 검증 교차 확인 2`, label followed only by the count-only segment, no `일반 진행:` / `혼합 지표:` / `일반 지표:` / `설명 카드` / `최신 확인` absent segment leak inside `.meta`, no leading/trailing separator artifact in the composed line)
...
130. M114: User-activated preference injection fidelity completed 2026-04-30 — user-triggered `activate_preference()` now persists `is_highly_reliable=True` in JSON/SQLite preference records so manually activated reviewed-candidate preferences immediately qualify for session injection; `_auto_activate_candidate_if_ready()` stays statistics-based.
131. M115: Preference injection context relevance completed 2026-04-30 — `_get_active_preferences(user_input=..., session_id=...)` now prioritizes eligible ACTIVE preferences whose `description` or `corrected_text` overlaps current user-input words, falls back to the full eligible set with `fallback_all` when there is no input or no match, and records `preference_injected` task-log events per injected preference.
132. M116: Preference injection relevance quality improvement completed 2026-04-30 — `_preference_context_terms()` removes stop-words and filters to length ≥ 2; `_select_context_relevant_preferences()` ranks by overlap-term count descending and places `is_highly_reliable == True` preferences first on equal scores; stop-word-only input falls back to `fallback_all`.
133. M117: Injection feedback loop completed 2026-04-30 — `PerPreferenceStats` gains `injected_count`; `get_global_audit_summary()` in both `SessionStore` and `SQLiteSessionStore` now counts `preference_injected` task-log events per preference id, enabling injection-to-application conversion rate tracking.
134. M118: Injected-count API exposure and UI visibility completed 2026-04-30 — injection-only preferences now get zeroed `per_preference_stats` entries before `injected_count` increments, `list_preferences_payload()` exposes per-preference top-level `injected_count` without changing `reliability_stats`, and `PreferencePanel` shows positive injection counts with optional application-rate text.
135. M119: Injection-correction feedback loop Axis 1 completed 2026-04-30 — JSON/SQLite `get_global_audit_summary()` adds `injection_correction_count` for sessions that have both injected preferences and correction events, and `list_preferences_payload()` exposes top-level `injection_correction_count` / `injection_correction_rate` without changing `is_highly_reliable_preference()`.
...
140. M121: Watcher self-restart lease reclamation completed 2026-05-08 — PaneLease.archive_for_restart() archives slot locks to locks/archive/ before watcher self-restart terminates the experimental PID, reducing the window where a stale supervisor-owned lease blocks the next watcher acquire; 3 new regression tests in PaneLeaseOwnerPidWiringTest cover active-lease move, no-op on missing lock, and acquire success after archive; commit 8fc5e6d, PR #117 merged.
141. M122 Axis 1: Trust-gated multi-source agreement completed 2026-05-08 — `_entity_source_fact_agreement_score()` now requires at least one trust-score-≥4 peer per label before that label contributes to the agreement score; `summarize_slot_coverage()` STRONG condition simplified to `trusted_source_count >= 2`; 2 regression tests added to `SmokeTest` covering mixed-trust WEAK/STRONG/CONFLICT and low-trust peer non-contribution; 152 tests pass.
...
1. Keep the supervisor-owned runtime surface (`status.json`, `events.jsonl`, `receipts/`) as the only current-truth path for internal pipeline tooling, including the representative `degraded_reason` plus full `degraded_reasons` list.
2. Keep the live adoption-gate harness (`python3 scripts/pipeline_runtime_gate.py fault-check|soak ...`) working for baseline and adoption gates, but treat current day-to-day verification as launcher live stability gate + incident replay + real work sessions rather than default 6h / 24h reruns.
3. Leave a small adoption/cutover docs cleanup pass on the backlog so older 6h/24h-centered wording is clearly historical context and does not drift back into the day-to-day verification default.
...
45. Define the review action vocabulary with shipped and later actions:
  - `accept | reject | defer` are now shipped
  - `edit` remains later
  - all shipped actions stay reviewed-but-not-applied
...
101. The enabled submit and emitted-record materialization are now shipped; keep the shipped operator-visible `future_reviewed_memory_apply` trigger-source affordance narrow:
...
111. The truthful same-aggregate `unblocked_all_required` capability path is now implemented (shipped this round):
  ...
  - one truthful aggregate-level `reviewed_memory_transition_record` is now emitted at the enabled aggregate-card submit boundary; the reviewed-memory apply boundary is now also shipped above this emitted record: `record_stage` transitions from `emitted_record_only_not_applied` to `applied_pending_result` with `applied_at` added via POST `/api/aggregate-transition-apply`; the apply result is now also shipped: `결과 확정` confirms the result and transitions `record_stage` to `applied_with_result`, creating `apply_result` with `result_version = first_reviewed_memory_apply_result_v1`, `applied_effect_kind = reviewed_memory_correction_pattern`, `result_stage = effect_active`, and `result_at`; the memory effect on future responses is now active (`result_stage = effect_active`); active effects are stored on the session as `reviewed_memory_active_effects`; future responses include a `[검토 메모 활성]` prefix with the operator's reason and pattern fingerprint; stop-apply (`future_reviewed_memory_stop_apply`) is now also shipped: after the effect is active the aggregate card shows an `적용 중단` button; clicking it changes `record_stage` to `stopped`, sets `apply_result.result_stage` to `effect_stopped`, removes the effect from `reviewed_memory_active_effects`, and future responses no longer include the `[검토 메모 활성]` prefix; reversal (`future_reviewed_memory_reversal`) is now also shipped: after stop the card shows `적용 되돌리기`; clicking it changes `record_stage` to `reversed`, sets `apply_result.result_stage` to `effect_reversed`, and adds `reversed_at`; conflict-visibility (`future_reviewed_memory_conflict_visibility`) is now also shipped: after reversal the card shows `충돌 확인`; clicking it creates a separate `reviewed_memory_conflict_visibility_record` with `transition_action = future_reviewed_memory_conflict_visibility`, `record_stage = conflict_visibility_checked`, `source_apply_transition_ref`, evaluated `conflict_entries`, and `conflict_entry_count`
...
3. correction-memory schema — **complete**: TypedDict contracts (M54-M58) + Correction Lifecycle axis (M61-M70) + physical validation for all stores (M72-M74) + SQLite store structural decomposition (M75)
4. durable preference memory — partial: accepted reviewed candidates are persisted as local preference candidate records via `record_reviewed_candidate_preference`; cross-session preference application and prompt injection shipped in M49 (ACTIVE+is_highly_reliable=True 선호만 document summary/chat 경로에 주입; 웹 조사 제외). Visibility, reliability tracking, and explicit correction feedback shipped in M50-M52.
