...
- next phase target: preference injection, visibility, and reliability feedback loop shipped in M49–M52 (cross-session ACTIVE+is_highly_reliable=True 선호 주입; 웹 조사 제외; applied/corrected 통계; 명시적 교정 피드백; 신뢰도 저하 경고). Remaining: structured correction-memory schema; broader cross-session memory remains later.
