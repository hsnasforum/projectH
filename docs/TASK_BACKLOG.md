...
134. M118: Injected-count API exposure completed 2026-04-30 — injection-only preferences now get zeroed `per_preference_stats` entries before `injected_count` increments, and `list_preferences_payload()` exposes per-preference top-level `injected_count` without changing `reliability_stats`.
