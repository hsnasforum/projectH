...
41. History-card entity-card zero-strong-slot `다시 불러오기` reload answer-mode + verification-label + source-path continuity Playwright smoke coverage (`설명 카드` answer-mode badge, downgraded `설명형 단일 출처`, `백과 기반`, `namu.wiki`, `ko.wikipedia.org` in context box)
