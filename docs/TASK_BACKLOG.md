...
61. Entity-card 붉은사막 actual-search natural-reload second-follow-up source-path + response-origin continuity service + Playwright smoke coverage (`namu.wiki`/`ko.wikipedia.org` in context box, `WEB` badge, `설명 카드`, `설명형 다중 출처 합의`, `백과 기반` drift prevention)
