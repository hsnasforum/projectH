...
56. History-card entity-card `다시 불러오기` actual-search source-path plurality + response-origin continuity service + Playwright smoke coverage (`namu.wiki`, `ko.wikipedia.org` in context box, `WEB` badge, `설명 카드`, `설명형 다중 출처 합의`, `백과 기반`)
