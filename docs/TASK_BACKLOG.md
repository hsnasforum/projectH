...
53. Entity-card 붉은사막 browser natural-reload follow-up source-path plurality service + Playwright smoke coverage (`namu.wiki`, `ko.wikipedia.org`, `blog.example.com` provenance in context box)
...
61. Entity-card 붉은사막 natural-reload second-follow-up source-path + response-origin continuity service + Playwright smoke coverage (`namu.wiki`, `ko.wikipedia.org`, `blog.example.com` provenance in context box, `WEB` badge, `설명 카드`, `설명형 다중 출처 합의`, `백과 기반` drift prevention)
