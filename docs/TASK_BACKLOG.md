...
51. Entity-card 붉은사막 actual-search browser natural-reload follow-up response-origin truth-sync service + Playwright smoke coverage (`WEB` badge, `설명 카드`, `설명형 다중 출처 합의`, `백과 기반` drift prevention)
...
53. Entity-card 붉은사막 actual-search browser natural-reload follow-up source-path plurality service + Playwright smoke coverage (`namu.wiki`, `ko.wikipedia.org` in context box)
