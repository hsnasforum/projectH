...
-- Canonical schema for a local-only assistant MVP.
-- IDs are text to keep the implementation compatible with UUID or ULID values.
-- Timestamps are stored as ISO-8601 UTC strings.
--
-- Note path semantics:
--   requested_path: user/system requested path hint. Drafts may exist without it.
--   resolved_path: final normalized path after policy/path resolution. Required when status = 'saved'.
--   requested_path is required once a note enters 'pending_approval' or 'saved'.
--
-- FTS path semantics:
--   document_chunks_fts indexes only chunk content.
--   Any UI/API path should be resolved by joining document metadata so renames cannot leave stale paths in FTS.
...
CREATE TABLE IF NOT EXISTS notes (
  ...
  CHECK (
    status NOT IN ('pending_approval', 'saved') OR requested_path IS NOT NULL
  ),
  CHECK (
