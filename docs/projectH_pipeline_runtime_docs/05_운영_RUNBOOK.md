...
- `pr_merge_gate`는 PR merge 전에는 `needs_operator + pr_boundary`가 정상입니다. merge 후에도 같은 control slot이 남아 있으면 supervisor/watcher가 `gh pr view`로 참조 PR의 merged 상태를 확인해 `pr_merge_completed` recovery로 내리고, 확인에 실패하면 fail-closed로 operator wait를 유지합니다.
