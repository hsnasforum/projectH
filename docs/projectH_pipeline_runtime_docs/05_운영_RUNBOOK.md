...
7. operator stop 판정은 free-form prose가 아니라 `OPERATOR_POLICY` 우선, `REASON_CODE` 다음 순서로 봅니다. 구조화 metadata가 없거나 알 수 없으면 fail-closed로 즉시 `needs_operator` / operator wait에 남기며, 외부 publication 실행으로 해석하지 않습니다.
...
- `commit_push_bundle_authorization + internal_only + release_gate`는 승인된 큰 묶음의 commit/push follow-up이므로 operator wait나 hibernate가 아니라 verify/handoff-owner triage로 흐릅니다. `commit_push_pr_creation_<milestone>_bundle`, `mNN_publish_bundle_authorization`, 또는 `dirty_bundle_publication_or_hold_decision`처럼 commit/push, PR 생성, publish-or-hold 결정을 한 label에 섞은 ad-hoc reason도 같은 canonical reason으로 정규화합니다. `operator_only_publication_boundary` policy는 `internal_only`, `publish_authorization` 및 `publication_or_hold` decision class는 `release_gate`로 봅니다.
- `pr_creation_gate + gate_24h + release_gate`는 승인된 큰 묶음의 draft PR 생성 follow-up이므로 operator wait가 아니라 verify/handoff-owner triage로 흐르고 `automation_health=attention`, `automation_next_action=verify_followup`으로 확인합니다. 이때 기존 draft PR이 이미 merge 승인 backlog에 있으면 같은 PR에 계속 덧대지 말고 stacked child branch/PR을 기본값으로 사용합니다. parent branch를 child PR의 base로 두고, parent/child linkage를 `/work`에 남긴 뒤 parent merge 후 child PR을 저장소 기본 base branch로 retarget합니다.
- `pr_merge_gate + internal_only + merge_gate`는 PR merge 전에는 merge 승인 대기 backlog로 남기되 local 구현을 막지 않는 것이 정상입니다. 정상 표시는 `automation_health=attention`, `automation_next_action=verify_followup`, compat active control은 `operator_request.md`입니다. merge 후에도 같은 control slot이 남아 있으면 supervisor/watcher가 `gh pr view`로 참조 PR의 merged 상태를 확인해 `pr_merge_completed` recovery로 내리고, 확인에 실패하면 fail-closed로 operator wait를 유지합니다. control 본문 `HEAD`가 merged PR의 `headRefOid`와 다르면 닫힌 PR 번호를 재사용한 잘못된 gate이므로 `pr_merge_head_mismatch` recovery로 보고 새 PR/control 정정이 필요합니다. `external_publication_boundary`나 structured metadata가 불명확한 publication stop은 여전히 `needs_operator + pr_boundary`가 정상입니다.
- `pr_merge_completed` 뒤에는 같은 PR merge backlog를 다시 operator wait로 보존하거나 implement lane에 publish 작업을 넘기지 않습니다. verify/retriage가 같은 PR에 대해 metadata-only `/work` 또는 docs-only local guard closeout을 반복하면, 새 local guard를 계속 쌓지 말고 남은 docs/control truth를 한 번에 정리하는 bounded docs bundle 또는 실제 operator-only boundary 중 하나로 닫습니다.
- `advisory_request.md`가 오래 열려 있는데 같은 `CONTROL_SEQ`의 `advisory_advice.md`가 없으면 watcher가 `advisory_recovery` event를 남기고 verify/handoff-owner에게 recovery prompt를 보낼 수 있습니다. 이때 정상 목표는 advisory 대필이 아니라 더 높은 `CONTROL_SEQ`의 다음 control 하나로 회수하는 것입니다. 같은 stale advisory를 다시 열지 말고, 새 advisory가 필요하면 더 좁은 근거가 있어야 합니다.
...
- dispatch 전부터 보이는 `[Pasted Content ...]`는 stale input으로 보고 watcher가 덮어쓰지 않습니다. watcher가 새로 paste한 Codex prompt가 첫 `Enter` 뒤에도 보이는 경우에만 `C-j`를 한 번 더 보내고, 그래도 남으면 `pasted_prompt_after_submit_retry` cleanup을 먼저 확인합니다. cleanup으로 입력줄이 비워진 경우에만 one-line literal fallback을 한 번 사용하며, cleanup 또는 fallback이 실패하면 fail-closed 하는 것이 정상입니다.
...
- 같은 fingerprint에서 이 pre-accept stall이나 short idle retry가 한 번 더 반복되면 runtime은 이를 `dispatch_stall` incident로 승격하고, 추가 자동 재큐잉 대신 `degraded_reason=dispatch_stall`, `automation_next_action=verify_followup`, 해당 lane note를 남기는 편이 맞습니다.
...
- pending 또는 immediate control-resolution notify도 `expected_control_seq`가 있으면 dispatch 직전에 active control을 다시 대조해야 합니다. 더 높은 `CONTROL_SEQ`의 `safety_stop + immediate_publish`가 active가 된 뒤에는 같은 `operator_request.md` 경로를 가리키는 오래된 `verify_operator_retriage` prompt라도 `control_seq_drift`로 drop하고 다시 paste하지 않아야 합니다.
- Gemini lane이 `Allow execution of [git]?` permission prompt에서 멈췄고 visible `Shell` command가 read-only git 조회(`branch`, `log`, `status`, `show`, `diff`, `rev-parse` 등)만 포함하면 watcher가 `2. Allow for this session`을 자동 입력합니다. `gemini_git_permission_auto_allow_session` event가 남는 것이 정상이며, non-git 또는 `git push/reset/checkout/commit` 같은 write/publication command는 자동 승인하지 않습니다.
- Gemini lane이 오래 `Thinking... (esc to cancel)`로 보일 때는 active control을 먼저 확인합니다. current turn이 `VERIFY_FOLLOWUP`/operator/implement로 넘어갔거나 같은 advisory에 current advice가 이미 있으면 watcher가 `inactive_advisory_cancel_sec` 뒤 stale advisory에 `Escape`를 보내고 `advisory_lane_cancelled` event를 남기는 것이 정상입니다. current `ADVISORY_ACTIVE + request_open` 자체가 오래 걸리는 경우는 `advisory_recovery` 경로로 verify/handoff에게 회수됩니다.
- pane tail에서 단순 `Working (...)` busy marker 뒤에 `❯` / `›` / `>` 입력 prompt가 다시 보이면 prompt-ready로 취급합니다. 다만 현재 진행형 `Waiting for background terminal`, `thinking with ...`, `esc to interrupt/cancel` 같은 active busy marker가 같은 tail에 남아 있으면 아직 busy입니다. 과거형 `Waited for background terminal` 완료 로그 뒤에 prompt가 보이면 ready로 풀고, 이후 `TASK_ACCEPTED` / `TASK_DONE` / receipt-close chain이 이어지는지를 확인합니다.
...
- GUI/launcher가 `automation_health=recovering|attention|ok`와 `status.control=none`을 받았는데도 compat `operator_request.md` 때문에 active operator wait를 빨간색으로 보여 주면 thin-client surface 회귀입니다.
...
- 같은 stale-control advisory grace도 advisory가 켜진 profile에서는 `advisory_followup`으로, 현재 Codex-only처럼 `advisory_enabled=false`인 profile에서는 `verify_followup`으로 표면화해야 합니다.
...
- `publish_boundary_accumulated_dirty_tree` 또는 `dirty_bundle_publication_or_hold_decision`처럼 advisory가 누적 dirty tree를 publish boundary나 publish-or-hold 결정으로 표현한 compatibility reason은 `commit_push_bundle_authorization + internal_only + release_gate`로 정규화되어야 합니다. 이 reason이 `hibernate + ok/attention` 또는 `immediate_publish + needs_operator`로 누우면 Codex가 한 라운드 뒤 멈춘 것처럼 보이는 생산성 차단 회귀입니다.
- `commit_push_pr_creation_<milestone>_bundle` 또는 `mNN_publish_bundle_authorization`처럼 canonical helper가 아닌 ad-hoc publish bundle reason이 `hibernate`로 보이면 reason 정규화 회귀입니다. 정상 표시는 `commit_push_bundle_authorization`, `automation_health=attention`, `automation_next_action=verify_followup`입니다.
...
- merge가 이미 끝난 `pr_merge_gate` 뒤에 같은 PR의 post-merge local guard closeout만 반복되면, 이는 추가 publish 승인이 아니라 control 수렴 품질 문제로 봅니다. 운영자는 새 merge/PR 작업을 지시하지 말고, verify/handoff가 advisory 비활성 조건에서는 bounded docs bundle로 정리했는지 또는 실제 operator boundary를 구조화 metadata로 다시 열었는지 확인합니다.
