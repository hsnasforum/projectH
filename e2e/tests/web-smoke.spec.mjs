...
test("history-card entity-card 다시 불러오기 후 noisy single-source claim이 본문과 origin detail에 노출되지 않습니다", async ({ page }) => {
...
test("entity-card 붉은사막 자연어 reload 후 follow-up에서 response origin badge와 answer-mode badge가 drift하지 않습니다", async ({ page }) => {
  const sessionId = await prepareSession(page, "entity-actual-natural-reload-followup-origin");
...
test("entity-card 붉은사막 자연어 reload 후 두 번째 follow-up에서 response origin badge와 answer-mode badge가 drift하지 않습니다", async ({ page }) => {
  const sessionId = await prepareSession(page, "entity-actual-natural-reload-second-followup-origin");
...
test("entity-card 붉은사막 자연어 reload 후 follow-up에서 noisy single-source claim이 미노출되고 continuity가 유지됩니다", async ({ page }) => {
  const sessionId = await prepareSession(page, "entity-crimson-natural-reload-followup-noisy-excl");
...
test("entity-card 붉은사막 자연어 reload 후 두 번째 follow-up에서 noisy single-source claim이 미노출되고 continuity가 유지됩니다", async ({ page }) => {
  const sessionId = await prepareSession(page, "entity-crimson-natural-reload-second-followup-noisy-excl");
...
test("entity-card noisy single-source claim이 자연어 reload 후 follow-up에서도 본문과 origin detail에 다시 노출되지 않습니다", async ({ page }) => {
  const sessionId = await prepareSession(page, "entity-noisy-natural-reload-followup");
...
test("entity-card noisy single-source claim이 자연어 reload 후 두 번째 follow-up에서도 본문과 origin detail에 다시 노출되지 않습니다", async ({ page }) => {
  const sessionId = await prepareSession(page, "entity-noisy-natural-reload-second-followup");
