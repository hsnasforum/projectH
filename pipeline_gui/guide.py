...
`.pipeline/gemini_request.md`, and `.pipeline/gemini_advice.md` remain read-only
compatibility inputs for the same logical slots, not a second control plane.
...
2. 최신 유효 슬롯이 advisory_request (STATUS: request_open)이면 -> advisory request
3. 최신 유효 슬롯이 advisory_advice (STATUS: advice_ready)이면 -> verify follow-up
4. 최신 /work가 same-day /verify보다 새로우면 -> verify/handoff owner (미검증 구현 우선 검증)
5. 최신 유효 슬롯이 implement_handoff (STATUS: implement)이면 -> implement owner
...
호출 조건:
- .pipeline/implement_handoff.md의 STATUS가 implement
...
- sentinel field name은 role-based `verify_triage`를 우선 사용하며, legacy `codex_triage`는 compatibility alias로만 봅니다
...
호출 조건:
- .pipeline/advisory_request.md의 STATUS가 request_open이고
...
- advisory owner는 report/gemini/...md와 .pipeline/advisory_advice.md를 둘 다 남겨야 canonical closeout으로 봄
...
advisory owner가 .pipeline/advisory_advice.md를 STATUS: advice_ready로 갱신하면 watcher가 verify follow-up을 재호출합니다.

verify/handoff owner -> advisory_request.md -> advisory owner -> advisory_advice.md -> watcher -> verify follow-up
...
- .pipeline/implement_handoff.md -> implement handoff
- .pipeline/advisory_request.md -> advisory request
- .pipeline/advisory_advice.md -> verify follow-up
...
주요 슬롯:
- implement_handoff.md, advisory_request.md, advisory_advice.md, operator_request.md
