...
기본 운용 대상은 `projectH` checkout입니다. source `.cmd` launcher에서 외부 repo를
target으로 지정할 수는 있지만, launcher source(`WSL_LAUNCHER_ROOT`)는 같은 WSL
distro 안의 projectH checkout이어야 합니다. packaged exe도 target repo 안에 staged
runtime asset을 만들지만, runtime Python package와 agent 프로토콜은 projectH checkout
기준으로 검증합니다.
...
| target repo | | ✅ 같은 WSL distro 내부 clone, `AGENTS.md` 필수 |
...
exe 내부 자산은 먼저 `sys._MEIPASS/_data/` 아래에서 찾고, 최신 exe는 필요한
runtime 파일을 target project 아래 `.pipeline/gui-runtime/_data/`로 staging한 뒤
WSL bash에 그 안정 경로를 전달합니다.
...
- CLI: tmux, python3
- active profile이 launch 가능한 경우: profile의 enabled lane에 필요한 agent CLI
...
**Guide 파일 생성**: project validation은 `AGENTS.md`가 이미 있어야 통과합니다. 설정 흐름에서는 누락된 보조 guide 파일을 승인 후 기본 템플릿으로 생성할 수 있으며, 기존 파일은 덮어쓰지 않습니다.
...
| external target repo Start | 제한적 | source launcher는 `WSL_LAUNCHER_ROOT` projectH checkout에서 실행하고 target path를 인자로 넘길 때만 지원 |
...
| PyInstaller temp dir 수명 | 완화됨 | runtime asset은 target project 아래 staged copy를 우선 사용 |
| Windows 10 (WSLg 없음) | 미테스트 | GUI 창이 뜨지 않을 것으로 예상 |

## 알려진 제한

1. **PyInstaller onefile temp 디렉터리**: 최신 exe는 bundled runtime asset을 target project의 `.pipeline/gui-runtime/_data/`로 staging한 뒤 그 경로를 사용하므로, 실행 중 watcher가 `_MEIxxxx` temp 디렉터리에 직접 의존하지 않게 완화되었습니다. staged asset이 불완전하면 launcher는 이를 재사용하지 않고 다시 staging을 시도합니다.
