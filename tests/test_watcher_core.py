...
from pipeline_runtime.role_routes import VERIFY_FOLLOWUP_ROUTE
from pipeline_runtime.turn_arbitration import legacy_turn_state_name
from pipeline_runtime.wrapper_events import append_wrapper_event
from verify_fsm import (
    CODEX_VERIFY_DISPATCH_FAILURE_LOOP_REASON,
    DISPATCH_FAILED_SUBMIT_STAGE,
    JobState,
    JobStatus,
    make_job_id,
)
from watcher_stabilizer import compute_file_sha256
from watcher_state import ControlSignal, PaneLease, WatcherTurnState
...
class WorkNoteFilteringTest(unittest.TestCase):
    ...
    def test_poll_creates_verify_job_for_latest_metadata_only_note(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            core._transition_turn(WatcherTurnState.IDLE, "test_setup")
    ...
    def test_claude_active_metadata_only_closeout_releases_to_verify(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            core._initial_turn_checked = True
            core._transition_turn(WatcherTurnState.IMPLEMENT_ACTIVE, "test_setup")
    ...
    def test_verify_feedback_sig_detects_cross_day_verify_tree_change(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            job_id = make_job_id(watch_dir, work_note)
            job = JobState.from_artifact(job_id, str(work_note), run_id=core.run_id)
...
""",
            ...
            with mock.patch("watcher_core._shared_capture_pane_text", side_effect=lambda target: pane_texts[target]), \
                 mock.patch("watcher_dispatch._is_pane_dead", return_value=False):
                core._poll()
...
""",
            ...
            self.assertNotEqual(core._current_turn_state, WatcherTurnState.IMPLEMENT_ACTIVE)
...
""",
            ...
            with mock.patch("watcher_core._shared_capture_pane_text", side_effect=lambda target: pane_texts[target]):
                with mock.patch("watcher_dispatch._is_pane_dead", side_effect=fake_dead):
                    core._poll()
...
class WatcherPromptAssemblyTest(unittest.TestCase):
    def test_verify_prompt_context_includes_runtime_dispatch_surface(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            status = json.loads(core.run_status_path.read_text(encoding="utf-8"))

            self.assertFalse(status["runtime_controls"]["advisory_enabled"])
            self.assertEqual(status["automation_reason_code"], "stale_control_advisory")
            ...
            self.assertIn("automation_next_action: verify_followup", prompt)
            self.assertNotIn("automation_next_action: advisory_followup", prompt)
    ...
    def test_verify_prompt_preserves_local_socket_guard_rules(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            watch_dir = root / "work"
            base_dir = root / ".pipeline"
            ...
            work_note = watch_dir / "2026-05-19-local-socket-held.md"
            work_note.write_text(
                "## 변경 파일\n- work/5/19/2026-05-19-local-socket-held.md\n",
                encoding="utf-8",
            )
            ...
            self.assertNotIn(".pipeline/advisory_request.md [request_open]", prompt)
            self.assertIn(
                "if controller Playwright webServer startup fails with socket permission denial",
                prompt,
            )
            self.assertIn("do not write another `.pipeline/operator_request.md`", prompt)
            self.assertIn("record `local_socket_guard_auto_held` in `/verify`", prompt)
            self.assertIn("do not claim controller-smoke pass/release readiness", prompt)
            self.assertIn(
                "choose the next safe local `.pipeline/implement_handoff.md`",
                prompt,
            )
            self.assertIn(
                "if local full-smoke guard external-check evidence reports `local_socket_guard_auto_held`",
                prompt,
            )
            self.assertIn("do not reissue the same full-smoke handoff", prompt)
            self.assertIn("claim release-ready/full-smoke-pass", prompt)
            self.assertIn(
                "write `/verify` that the local full smoke is environment-held",
                prompt,
            )
            self.assertIn("choose the next safe local slice", prompt)
    ...
    def test_operator_retriage_prompt_advisory_disabled_commit_push_holds_publication_locally(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            watch_dir = root / "work"
            base_dir = root / ".pipeline"
            ...
            self.assertIn("REASON: commit_push_bundle_authorization", prompt)
            self.assertIn("ADVISORY_DISABLED: true", prompt)
            self.assertIn("PUBLISH_HELD: true", prompt)
            self.assertIn("hold the publish backlog by default", prompt)
            self.assertIn(
                "do not run commit, push, branch publication, PR creation, or merge commands",
                prompt,
            )
            self.assertIn("do not hand commit/push/PR work to the implement lane", prompt)
            self.assertIn(
    ...
    def test_operator_retriage_prompt_preserves_source_reason(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            watch_dir = root / "work"
            base_dir = root / ".pipeline"
            ...
            prompt = core.prompt_assembler.format_operator_retriage_prompt(
                {
                    ...
                    "source_reason": "pr_merge_completed",
                    "publish_held": True,
                }
            )

            self.assertIn("REASON: pr_merge_completed", prompt)
            self.assertIn("ADVISORY_DISABLED: false", prompt)
            self.assertIn("PUBLISH_HELD: true", prompt)
            self.assertNotIn("REASON: operator_retriage_no_next_control", prompt)
    ...
    def test_pr_creation_gate_routes_to_verify_owner_publish_followup(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("PUBLISH_HELD: true", prompt)
            self.assertIn("hold draft PR creation by default", prompt)
    ...
    def test_b1_dirty_tree_release_gate_operator_request_routes_to_verify_followup(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            watch_dir = root / "work"
            base_dir = root / ".pipeline"
            ...
            prompt = core.prompt_assembler.format_operator_retriage_prompt(marker)
            self.assertIn("commit_push_bundle_authorization + internal_only + release_gate", prompt)
            self.assertIn("pr_creation_gate + gate_24h + release_gate", prompt)
            self.assertIn("do not hand commit/push/PR work to the implement lane", prompt)
...
class ClaudeImplementBlockedTest(unittest.TestCase):
    ...
    def test_waiting_for_claude_routes_explicit_implement_blocked_to_codex(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            handoff_sha = compute_file_sha256(handoff_path)
            pane_texts = {
                "claude-pane": (
                    "STATUS: implement_blocked\n"
            ...
            with mock.patch("watcher_core._shared_capture_pane_text", side_effect=lambda target: pane_texts[target]):
                with mock.patch("watcher_dispatch.tmux_send_keys", return_value=True) as send:
                    core._poll()
...
class PaneLeaseOwnerPidWiringTest(unittest.TestCase):
    ...
    def test_owner_dead_returns_true_when_supervisor_pid_no_longer_exists_as_process(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch("watcher_state.os.kill", side_effect=ProcessLookupError):
    ...
    def test_lease_persists_owner_pid_and_clears_after_owner_restart(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch("watcher_state.os.kill", side_effect=_kill):
                self.assertFalse(lease.is_active("slot_verify"))
                self.assertFalse((lock_dir / "slot_verify.lock").exists())
...
class DedupeGuardPersistenceTest(unittest.TestCase):
    def test_forget_survives_guard_reload(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            events_dir = Path(tmp) / "events"
            guard = watcher_state.DedupeGuard(events_dir)

            guard.mark_dispatch("job-1", 1, "hash-1", "slot_verify", "%1", False)
            self.assertTrue(guard.is_duplicate("job-1", 1, "hash-1", "slot_verify"))

            guard.forget("job-1", 1, "hash-1", "slot_verify")
            self.assertFalse(guard.is_duplicate("job-1", 1, "hash-1", "slot_verify"))

            reloaded = watcher_state.DedupeGuard(events_dir)
            self.assertFalse(reloaded.is_duplicate("job-1", 1, "hash-1", "slot_verify"))
...
class WatcherDispatchQueueControlMismatchTest(unittest.TestCase):
    ...
    def test_new_control_dispatch_blocks_stale_codex_pasted_content(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            queue = watcher_dispatch.WatcherDispatchQueue(
                ...
                role_owner=lambda role: {"implement": "Codex"}.get(role, role),
                log_raw=mock.Mock(),
            ...
            ok = queue.dispatch(
                watcher_dispatch.DispatchIntent(
                    pending_key="codex_implement:implement_handoff:78",
                    notify_kind="implement_handoff",
                    lane_role="implement",
                    functional_role="implement",
                    lane_id="codex_implement",
                    agent_kind="codex",
                    model_alias=None,
            ...
            self.assertIn("codex_implement:implement_handoff:78", queue.pending_notifications)
            event_names = [call.args[0] for call in append_runtime_event.call_args_list]
            self.assertEqual(event_names, ["codex_stale_paste_blocked", "lane_input_deferred"])
            blocked_payload = append_runtime_event.call_args_list[0].args[1]
            self.assertEqual(blocked_payload["recovery_reason"], "prompt_contains_pasted_content")
            self.assertEqual(blocked_payload["control_seq"], 78)

    def test_flush_pending_blocks_codex_pasted_content_when_still_unsubmitted(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            ...
            active_control = ControlSignal(
                ...
                canonical_file="advisory_advice.md",
            )
            send_prompt = mock.Mock(return_value=True)
            append_runtime_event = mock.Mock()
            queue = watcher_dispatch.WatcherDispatchQueue(
                ...
                capture_pane_text=mock.Mock(
                    side_effect=[
                        (
                            "› [Pasted Content 1024 chars]\n"
                            "  [Pasted Content 1024 chars] #2\n"
                            "  [Pasted Content 1024 chars] #3 .pipeline/advisory_request.md\n"
                        ),
                        "processing view without prompt\n",
                    ]
                ),
                send_keys=send_prompt,
                get_path_sig=lambda path: path.read_text(encoding="utf-8") if path.exists() else "",
                role_owner=lambda role: {"verify": "Codex"}.get(role, role),
                log_raw=mock.Mock(),
                append_runtime_event=append_runtime_event,
            ...
            queue.pending_notifications = {
                "codex_verify:advisory_advice_followup:77": {
                    "notify_kind": "advisory_advice_followup",
                    "lane_role": "verify",
                    "functional_role": "verify",
                    "lane_id": "codex_verify",
                    "agent_kind": "codex",
            ...
            send_prompt.assert_not_called()
            self.assertIn("codex_verify:advisory_advice_followup:77", queue.pending_notifications)
            event_names = [call.args[0] for call in append_runtime_event.call_args_list]
            self.assertEqual(event_names, ["codex_stale_paste_blocked", "lane_input_deferred"])
            blocked_payload = append_runtime_event.call_args_list[0].args[1]
            self.assertEqual(blocked_payload["recovery_reason"], "prompt_contains_pasted_content")

    def test_stale_codex_paste_blocked_does_not_repaste_during_backoff(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            prompt_path = root / ".pipeline" / "operator_request.md"
            prompt_path.parent.mkdir(parents=True, exist_ok=True)
            prompt_path.write_text("STATUS: needs_operator\nCONTROL_SEQ: 1624\n", encoding="utf-8")
            send_prompt = mock.Mock(return_value=False)
            append_runtime_event = mock.Mock()
            capture = mock.Mock(return_value="› [Pasted Content 2048 chars]\n")
            active_control = ControlSignal(
                kind="operator",
                path=prompt_path,
                status="needs_operator",
                ...
                canonical_file="operator_request.md",
            )
            queue = watcher_dispatch.WatcherDispatchQueue(
                lane_input_defer_cooldown_sec=0.0,
                capture_pane_text=capture,
                send_keys=send_prompt,
                get_path_sig=lambda path: path.read_text(encoding="utf-8") if path.exists() else "",
                role_owner=lambda role: {"verify": "Codex"}.get(role, role),
                log_raw=mock.Mock(),
            ...
            intent = watcher_dispatch.DispatchIntent(
                pending_key="codex_verify:verify_operator_retriage:1624",
                ...
                expected_control_slot="operator_request",
                expected_control_seq=1624,
            )

            self.assertFalse(queue.dispatch(intent))
            queue.flush_pending()

            send_prompt.assert_not_called()
            self.assertIn("codex_verify:verify_operator_retriage:1624", queue.pending_notifications)
            event_names = [call.args[0] for call in append_runtime_event.call_args_list]
            self.assertIn("codex_stale_paste_blocked", event_names)
            self.assertIn("lane_input_deferred", event_names)

    def test_stale_operator_retriage_dispatch_drops_after_new_safety_stop(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            prompt_path = root / ".pipeline" / "operator_request.md"
            prompt_path.parent.mkdir(parents=True, exist_ok=True)
            ...
            ok = queue.dispatch(
                watcher_dispatch.DispatchIntent(
                    pending_key="codex_verify:verify_operator_retriage:2030",
                    notify_kind="verify_operator_retriage",
                    lane_role="verify",
                    functional_role="verify",
                    lane_id="codex_verify",
                    ...
                    prompt="stale prompt for seq 2030",
                    prompt_path=prompt_path,
                    target="codex-pane",
                    pane_type="codex",
                    control_seq=2030,
            ...
            send_prompt.assert_not_called()
            self.assertNotIn(
                "codex_verify:verify_operator_retriage:2030",
                queue.pending_notifications,
            )
            append_runtime_event.assert_called_once()
            event_name, payload = append_runtime_event.call_args.args
            self.assertEqual(event_name, "lane_input_deferred_dropped")
            self.assertEqual(payload["reason_code"], "control_seq_drift")
            self.assertEqual(payload["expected_control_seq"], 2030)
            self.assertEqual(payload["active_control_seq"], 2031)

    def test_codex_send_success_with_remaining_paste_marker_is_blocked(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            prompt_path = root / ".pipeline" / "operator_request.md"
            prompt_path.parent.mkdir(parents=True, exist_ok=True)
            ...
            queue = watcher_dispatch.WatcherDispatchQueue(
                ...
                capture_pane_text=mock.Mock(
                    side_effect=[
                        "› \n",
                        "› [Pasted Content 1024 chars][Pasted Content 2048 chars]\n",
                    ]
                ),
                send_keys=send_prompt,
                get_path_sig=lambda path: path.read_text(encoding="utf-8") if path.exists() else "",
            ...
            intent = watcher_dispatch.DispatchIntent(
                pending_key="codex_verify:verify_operator_retriage:1627",
                notify_kind="verify_operator_retriage",
                lane_role="verify",
                functional_role="verify",
                lane_id="codex_verify",
                agent_kind="codex",
                model_alias=None,
                reason="startup_turn_operator_gated",
                prompt="prompt 1627",
                prompt_path=prompt_path,
                target="codex-pane",
                pane_type="codex",
            ...
            send_prompt.assert_called_once_with("codex-pane", "prompt 1627", "codex")
            self.assertIn("codex_verify:verify_operator_retriage:1627", queue.pending_notifications)
            event_names = [call.args[0] for call in append_runtime_event.call_args_list]
            self.assertIn("codex_stale_paste_blocked", event_names)

    def test_ready_codex_send_failure_with_remaining_paste_marker_is_blocked(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            prompt_path = root / ".pipeline" / "operator_request.md"
            prompt_path.parent.mkdir(parents=True, exist_ok=True)
            ...
            deferred_payloads = [
                call.args[1]
                for call in append_runtime_event.call_args_list
                if call.args[0] == "lane_input_deferred"
            ]
            self.assertEqual(deferred_payloads[-1]["defer_reason"], "codex_stale_paste_blocked")
    ...
    def test_flush_pending_drops_stale_implement_when_higher_seq_advisory_is_active(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            base_dir = root / ".pipeline"
            handoff_path = base_dir / "implement_handoff.md"
            ...
            queue = watcher_dispatch.WatcherDispatchQueue(
                ...
                append_runtime_event=append_runtime_event,
                get_active_control_signal=mock.Mock(return_value=active_control),
                is_active_control=mock.Mock(return_value=False),
            )
            queue.pending_notifications = {
                "claude_implement:implement_handoff:2019": {
                    "notify_kind": "implement_handoff",
                    "lane_role": "implement",
                    "functional_role": "implement",
                    ...
                    "agent_kind": "claude",
                    "reason": "implement_handoff_updated",
                    "prompt": "stale prompt 2019",
                    "prompt_path": str(handoff_path),
                    "target": "claude-pane",
                    "pane_type": "claude",
                    "control_seq": 2019,
                    "expected_status": "implement",
            ...
            event, path, job_id, payload = log_raw.call_args.args
            self.assertEqual(event, "lane_input_deferred_dropped")
            self.assertEqual(path, str(handoff_path))
            self.assertEqual(job_id, "turn_signal")
            self.assertEqual(payload["reason"], "control_mismatch")
            self.assertEqual(payload["reason_code"], "control_file_drift")
            self.assertEqual(payload["expected_control_seq"], 2019)
            self.assertEqual(payload["active_control_seq"], 2022)
            self.assertEqual(payload["expected_prompt_path"], str(handoff_path))
            self.assertEqual(payload["active_prompt_path"], str(request_path))
            self.assertEqual(payload["expected_status"], "implement")
            self.assertEqual(payload["active_status"], "request_open")
            self.assertEqual(payload["active_control"], "advisory_request")
            append_runtime_event.assert_called_once_with("lane_input_deferred_dropped", payload)
...
class ClaudeHandoffDispatchTest(unittest.TestCase):
    ...
    def test_handoff_update_waits_until_verify_lease_released(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(core, "_notify_implement_owner") as notify:
                ...
                self.assertEqual(core._current_turn_state, WatcherTurnState.IMPLEMENT_ACTIVE)
    ...
    def test_handoff_update_releases_when_supervisor_pid_is_dead(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                mock.patch("watcher_state.os.kill", side_effect=ProcessLookupError),
    ...
    def test_running_verify_job_finishes_before_older_unverified_work_is_discovered(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            job = JobState.from_artifact("job-current", str(current_work))
            job.run_id = core.run_id
            with mock.patch("watcher_dispatch.tmux_send_keys", return_value=True):
            ...
            stale_job_id = make_job_id(watch_dir, old_work)
            ...
            current_job = JobState.load(core.state_dir, job.job_id)
            self.assertIsNotNone(current_job)
            assert current_job is not None
            self.assertEqual(current_job.status, JobStatus.VERIFY_DONE)
            self.assertEqual(core._current_turn_state, WatcherTurnState.IMPLEMENT_ACTIVE)
...
class RuntimePlanConsumptionTest(unittest.TestCase):
    def test_prompt_contract_follows_nondefault_role_owners(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            job = JobState.from_artifact("job-role-neutral", str(work_note))
...
class VerifyPromptScopeHintTest(unittest.TestCase):
    def test_docs_only_round_gets_docs_only_fast_path_hint(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            job = JobState.from_artifact("job-docs", str(work_note))
    ...
    def test_code_mixed_round_keeps_standard_verify_scope_hint(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            job = JobState.from_artifact("job-mixed", str(work_note))
...
class TurnResolutionTest(unittest.TestCase):
    ...
    def test_verified_referenced_work_suppresses_completed_handoff_dispatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(turn, VERIFY_FOLLOWUP_ROUTE)
    ...
    def test_accumulated_dirty_tree_publish_boundary_routes_to_verify_followup(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            repo, _remote = _init_repo_with_commit_push(root)
            watch_dir = repo / "work"
    ...
    def test_real_operator_boundary_supersedes_lower_seq_advisory_slots(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(core._current_turn_state, WatcherTurnState.OPERATOR_WAIT)
            self.assertEqual(core._get_pending_advisory_request_mtime(), 0.0)
            self.assertEqual(core._get_pending_advisory_advice_mtime(), 0.0)
...
class RollingSignalTransitionTest(unittest.TestCase):
    def test_stale_control_seq_does_not_trigger_transition(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(core._current_turn_state, WatcherTurnState.CODEX_VERIFY)

    def test_higher_seq_advisory_keeps_stale_handoff_signal_unconsumed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            watch_dir = root / "work"
            base_dir = root / ".pipeline"
            ...
            request = base_dir / "advisory_request.md"
            handoff = base_dir / "implement_handoff.md"
            request.write_text("STATUS: request_open\nCONTROL_SEQ: 2022\n", encoding="utf-8")
            handoff.write_text("STATUS: implement\nCONTROL_SEQ: 2019\n", encoding="utf-8")
            os.utime(request, (100.0, 100.0))
            os.utime(handoff, (200.0, 200.0))
            ...
            core._transition_turn(
                ...
                active_control_seq=2022,
            )
            core._last_advisory_request_sig = core._get_path_sig(request)
            core._last_implement_handoff_sig = "previous-handoff-sig"

            with mock.patch.object(core, "_notify_implement_owner") as notify:
                core._check_pipeline_signal_updates()

            notify.assert_not_called()
            self.assertEqual(core._last_implement_handoff_sig, "previous-handoff-sig")
            self.assertEqual(core._current_turn_state, WatcherTurnState.ADVISORY_ACTIVE)

    def test_safety_stop_drops_pending_operator_retriage_and_blocks_dispatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            watch_dir = root / "work"
            base_dir = root / ".pipeline"
            ...
            operator_request.write_text(
                ...
                "REASON_CODE: safety_stop\n"
                "OPERATOR_POLICY: immediate_publish\n"
                "DECISION_CLASS: automation_retriage_loop_hard_stop\n",
                encoding="utf-8",
            )
            handoff = base_dir / "implement_handoff.md"
            handoff.write_text("STATUS: implement\nCONTROL_SEQ: 2029\n", encoding="utf-8")
            ...
            core._last_operator_request_sig = ""
            core.dispatch_queue.pending_notifications["codex_verify:verify_operator_retriage:2030"] = {
                "notify_kind": "verify_operator_retriage",
                "lane_role": "verify",
                "functional_role": "verify",
                "lane_id": "codex_verify",
            ...
            with mock.patch("watcher_dispatch.tmux_send_keys", return_value=True) as send_prompt:
                core._check_pipeline_signal_updates()

            send_prompt.assert_not_called()
            self.assertEqual(core._current_turn_state, WatcherTurnState.OPERATOR_WAIT)
            self.assertEqual(core._turn_active_control_seq, 2031)
            self.assertEqual(core.dispatch_queue.pending_notifications, {})
            raw_events = [
                json.loads(line)
                for line in (core.events_dir / "raw.jsonl").read_text(encoding="utf-8").splitlines()
                if line.strip()
            ...
            self.assertEqual(drop_event["reason_code"], "control_seq_drift")
            self.assertEqual(drop_event["expected_control_seq"], 2030)
            self.assertEqual(drop_event["active_control_seq"], 2031)
    ...
    def test_operator_retriage_no_next_control_advisory_disabled_returns_to_verify(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            watch_dir = root / "work"
            base_dir = root / ".pipeline"
            ...
            with (
                ...
                mock.patch.object(core, "_notify_verify_operator_retriage") as notify,
                mock.patch.object(core, "_notify_advisory_owner") as advisory_notify,
            ):
                promoted = core._promote_operator_retriage_no_next_control()

            self.assertTrue(promoted)
            advisory_notify.assert_not_called()
            notify.assert_called_once()
            reason, payload = notify.call_args.args
            self.assertEqual(reason, "operator_retriage_no_next_control")
            self.assertTrue(payload["advisory_disabled"])
            self.assertTrue(payload["publish_held"])
            prompt = core.prompt_assembler.format_operator_retriage_prompt(payload)
            self.assertIn("ADVISORY_DISABLED: true", prompt)
            self.assertIn("PUBLISH_HELD: true", prompt)
            self.assertIn("do not write `.pipeline/advisory_request.md`", prompt)
            self.assertNotIn(".pipeline/advisory_request.md [request_open]", prompt)
            ...
            self.assertEqual(core._current_turn_state, WatcherTurnState.VERIFY_FOLLOWUP)
            turn_state = json.loads((base_dir / "state" / "turn_state.json").read_text(encoding="utf-8"))
            self.assertEqual(turn_state["reason"], "verify_followup_no_next_control")
            self.assertEqual(core._turn_active_control_seq, 1624)
            raw_text = (base_dir / "logs" / "experimental" / "raw.jsonl").read_text(encoding="utf-8")
            self.assertIn("operator_retriage_no_next_control", raw_text)
            self.assertIn("advisory_disabled", raw_text)
    ...
    def test_pr_merge_recovery_no_next_control_advisory_disabled_returns_to_verify(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            watch_dir = root / "work"
            base_dir = root / ".pipeline"
            ...
            self.assertIn("ADVISORY_DISABLED: true", prompt)
            self.assertIn("PUBLISH_HELD: true", prompt)
            self.assertIn("pr_merge_completed", prompt)
            self.assertNotIn(".pipeline/advisory_request.md [request_open]", prompt)
            self.assertFalse((base_dir / "advisory_request.md").exists())
            self.assertEqual(core._current_turn_state, WatcherTurnState.VERIFY_FOLLOWUP)
            turn_state = json.loads((base_dir / "state" / "turn_state.json").read_text(encoding="utf-8"))
...
class TransitionTurnTest(unittest.TestCase):
    ...
    def test_transition_writes_role_bound_turn_owner_for_nondefault_profile(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                data["legacy_state"],
                legacy_turn_state_name(WatcherTurnState.IMPLEMENT_ACTIVE.value),
            )
...
class BusyLaneNotificationDeferTest(unittest.TestCase):
    def test_stale_implement_dispatch_drops_when_higher_seq_advisory_is_active(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            watch_dir = root / "work"
            base_dir = root / ".pipeline"
            ...
            with (
                ...
                core._notify_implement_owner("stale_implement_retry", handoff_path)

            send_prompt.assert_not_called()
            self.assertNotIn(
                "claude_implement:implement_handoff:2019",
                core.dispatch_queue.pending_notifications,
            )
            raw_events = [
                json.loads(line)
                for line in (core.events_dir / "raw.jsonl").read_text(encoding="utf-8").splitlines()
            ...
            self.assertEqual(drop_event["expected_control_seq"], 2019)
            self.assertEqual(drop_event["active_control_seq"], 2022)
            self.assertEqual(drop_event["expected_prompt_path"], str(handoff_path))
            self.assertEqual(drop_event["active_prompt_path"], str(request_path))
...
class VerifyCompletionContractTest(unittest.TestCase):
    def test_control_slot_change_without_verify_note_keeps_verify_running(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            job = JobState.from_artifact("job-verify-contract", str(work_note))
            ...
            with mock.patch("watcher_core._shared_capture_pane_text", return_value="$ "), \
                 ...
                 mock.patch("watcher_core._shared_pane_text_has_input_cursor", return_value=True):
                job = core.sm._handle_verify_running(job)
            self.assertEqual(job.status, JobStatus.VERIFY_DONE)
    ...
    def test_verify_tree_change_without_current_round_receipt_keeps_verify_running(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            job = JobState.from_artifact("job-verify-receipt", str(work_note))
    ...
    def test_idle_timeout_with_incomplete_outputs_requeues_verify_pending(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            work_note = work_day / "2026-04-10-slice.md"
            _write_work_note(work_note, ["tests/test_web_app.py"])
            job = JobState.from_artifact("job-verify-retry", str(work_note))
    ...
    def test_idle_prompt_with_incomplete_outputs_requeues_before_long_timeout(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            work_note = work_day / "2026-04-10-slice.md"
            _write_work_note(work_note, ["tests/test_web_app.py"])
            job = JobState.from_artifact("job-verify-quick-idle", str(work_note))
    ...
    def test_repeated_dispatch_stall_marks_job_degraded_after_one_requeue(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            work_note = work_day / "2026-04-10-slice.md"
            _write_work_note(work_note, ["tests/test_web_app.py"])
            job = JobState.from_artifact("job-verify-stall-repeat", str(work_note))
            ...
            with mock.patch("watcher_dispatch.tmux_send_keys", return_value=True) as send_keys:
    ...
    def test_delayed_task_accepted_does_not_false_stall_before_accept_deadline(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            work_note = work_day / "2026-04-10-slice.md"
            _write_work_note(work_note, ["tests/test_web_app.py"])
            job = JobState.from_artifact("job-verify-delayed-accept", str(work_note))
    ...
    def test_verify_dispatch_persists_round_scoped_control_seq_across_requeue(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            work_note = work_day / "2026-04-10-slice.md"
            _write_work_note(work_note, ["tests/test_web_app.py"])
            job = JobState.from_artifact("job-verify-dispatch-seq", str(work_note))
    ...
    def test_task_done_missing_requeues_then_degrades_same_fingerprint(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            work_note = work_day / "2026-04-10-slice.md"
            _write_work_note(work_note, ["tests/test_web_app.py"])
            job = JobState.from_artifact("job-verify-task-done-missing", str(work_note))
    ...
    def test_background_terminal_wait_does_not_trigger_task_done_stall(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            work_note = work_day / "2026-04-10-slice.md"
            _write_work_note(work_note, ["tests/test_web_app.py"])
            job = JobState.from_artifact("job-verify-background-wait", str(work_note))
    ...
    def test_task_done_without_receipt_close_uses_post_done_machine_note(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            work_note = work_day / "2026-04-10-slice.md"
            _write_work_note(work_note, ["tests/test_web_app.py"])
            job = JobState.from_artifact("job-verify-receipt-close-missing", str(work_note))
    ...
    def test_late_old_task_done_does_not_close_new_dispatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            work_note = work_day / "2026-04-10-slice.md"
            _write_work_note(work_note, ["tests/test_web_app.py"])
            job = JobState.from_artifact("job-verify-stale-task-done", str(work_note))
    ...
    def test_outputs_complete_infers_task_done_after_done_deadline_when_wrapper_misses_done(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            job = JobState.from_artifact("job-verify-output-close", str(work_note))
    ...
    def test_matching_task_done_implies_accept_when_wrapper_model_already_folded_accept(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            job = JobState.from_artifact("job-verify-folded-accept", str(work_note))
    ...
    def test_dispatch_seen_missing_uses_distinct_machine_note_before_degrade(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            work_note = work_day / "2026-04-10-slice.md"
            _write_work_note(work_note, ["tests/test_web_app.py"])
            job = JobState.from_artifact("job-verify-seen-missing", str(work_note))
    ...
    def test_dispatch_seen_without_accept_uses_task_accept_machine_note(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            work_note = work_day / "2026-04-10-slice.md"
            _write_work_note(work_note, ["tests/test_web_app.py"])
            job = JobState.from_artifact("job-verify-task-accept-missing", str(work_note))
    ...
    def test_verify_running_startup_recovery_requeues_stale_idle_dispatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            work_note = work_day / "2026-04-10-slice.md"
            _write_work_note(work_note, ["tests/test_web_app.py"])
            job = JobState.from_artifact("job-startup-recovery", str(work_note))
            job.status = JobStatus.VERIFY_RUNNING
    ...
    def test_poll_replays_current_run_verify_pending_even_when_latest_work_is_already_verified(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            pending_job_id = make_job_id(watch_dir, older_work)
            pending_job = JobState.from_artifact(
                pending_job_id,
                str(older_work),
                run_id=core.run_id,
            )
            pending_job.status = JobStatus.VERIFY_PENDING
            pending_job.save(core.state_dir)

            core._initial_turn_checked = True
            core._current_turn_state = WatcherTurnState.CODEX_VERIFY
            ...
            self.assertEqual(stepped_job.status, JobStatus.VERIFY_PENDING)
    ...
    def test_poll_archives_verified_pending_job_before_latest_unverified_work_scan(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            old_job_id = make_job_id(watch_dir, old_work)
            old_job = JobState.from_artifact(
                old_job_id,
                str(old_work),
                run_id=core.run_id,
            )
            old_job.status = JobStatus.VERIFY_PENDING
            old_job.save(core.state_dir)
            latest_job_id = make_job_id(watch_dir, latest_work)
            ...
            self.assertIsNone(JobState.load(core.state_dir, old_job_id))
            archived = (
                base_dir
                / "runs"
                / core.run_id
    ...
    def test_archive_matching_verified_pending_releases_verify_lease_through_fsm(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            job_id = make_job_id(watch_dir, work_note)
            job = JobState.from_artifact(
                job_id,
                str(work_note),
                run_id=core.run_id,
            )
            job.status = JobStatus.VERIFY_PENDING
            job.artifact_hash = "artifact-hash"
            job.save(core.state_dir)
            ...
            self.assertIsNone(JobState.load(core.state_dir, job_id))
            archived = (
                base_dir
                / "runs"
                / core.run_id
    ...
    def test_poll_delegates_current_run_verify_running_close_chain_before_pending_advisory_advice(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            running_job_id = make_job_id(watch_dir, work_note)
            running_job = JobState.from_artifact(
                running_job_id,
                str(work_note),
                run_id=core.run_id,
            )
            running_job.status = JobStatus.VERIFY_RUNNING
            running_job.save(core.state_dir)

            core._initial_turn_checked = True
            core._current_turn_state = WatcherTurnState.VERIFY_FOLLOWUP
            ...
            self.assertEqual(stepped_job.status, JobStatus.VERIFY_RUNNING)
    ...
    def test_poll_defers_current_run_verify_pending_while_advisory_request_is_pending(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            pending_job_id = make_job_id(watch_dir, work_note)
            pending_job = JobState.from_artifact(
                pending_job_id,
                str(work_note),
                run_id=core.run_id,
            ...
            core._initial_turn_checked = True
            core._current_turn_state = WatcherTurnState.ADVISORY_ACTIVE
            ...
            persisted_job = JobState.load(core.state_dir, pending_job_id)
            self.assertIsNotNone(persisted_job)
            self.assertEqual(persisted_job.status, JobStatus.VERIFY_PENDING)
...
class CodexDispatchConfirmationTest(unittest.TestCase):
    ...
    def test_send_literal_text_terminates_tmux_options_before_dash_chunk(self) -> None:
        ...
        with mock.patch("watcher_dispatch.subprocess.run", side_effect=_run), \
            ...
            watcher_dispatch._send_literal_text_to_pane("%1", "- leading literal")

        self.assertEqual(
            run_calls,
            [["tmux", "send-keys", "-l", "-t", "%1", "--", "- leading literal"]],
        )
    ...
    def test_dispatch_codex_cancels_stale_pasted_prompt_before_paste(self) -> None:
        snapshots = iter([
            "› [Pasted Content 1024 chars]",
            "›",
            "› prompt pasted",
    ...
    def test_dispatch_codex_returns_false_when_hard_clear_leaves_pasted_prompt(self) -> None:
        snapshots = iter([
        ...
        self.assertEqual(run_calls[0], ["tmux", "send-keys", "-t", "%1", "C-c"])
        self.assertEqual(run_calls[1], ["tmux", "send-keys", "-t", "%1", "C-u"])
        self.assertNotIn(["tmux", "set-buffer", "ROLE: verify"], run_calls)
    ...
    def test_dispatch_claude_returns_true_when_busy_output_appears_after_prompt_echo(self) -> None:
        ...
        with mock.patch("watcher_dispatch.subprocess.run") as run_mock, \
             mock.patch("watcher_dispatch._shared_capture_pane_text", side_effect=lambda _pane: next(snapshots)), \
             mock.patch("watcher_dispatch.time.sleep", return_value=None):
            result = watcher_dispatch._dispatch_claude("%1", "ROLE: verify")
    ...
    def test_tmux_send_keys_skips_when_same_pane_dispatch_lock_is_busy(self) -> None:
        ...
        with mock.patch("watcher_dispatch._dispatch_lock_for", return_value=busy_lock), \
            ...
            result = watcher_dispatch.tmux_send_keys("%1", "ROLE: verify", pane_type="codex")

        self.assertFalse(result)
        self.assertEqual(busy_lock.timeout, watcher_dispatch._DISPATCH_LOCK_TIMEOUT_SEC)
    ...
    def test_tmux_send_keys_propagates_claude_dispatch_failure(self) -> None:
        ...
        with mock.patch("watcher_dispatch._dispatch_lock_for", return_value=tracking_lock), \
            ...
            result = watcher_dispatch.tmux_send_keys("%1", "ROLE: implement", pane_type="claude")
    ...
    def test_tmux_send_keys_propagates_gemini_dispatch_failure(self) -> None:
        ...
        with mock.patch("watcher_dispatch._dispatch_lock_for", return_value=tracking_lock), \
            ...
            result = watcher_dispatch.tmux_send_keys("%1", "ROLE: advisory", pane_type="gemini")
...
class VerifyPendingBackoffTest(unittest.TestCase):
    def test_failed_dispatch_sets_retry_backoff_state(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            job = JobState.from_artifact("job-backoff", str(work_note))
            job.status = JobStatus.VERIFY_PENDING
            ...
            with mock.patch("watcher_dispatch.tmux_send_keys", return_value=False), \
    ...
    def test_failed_dispatch_with_pasted_content_marks_operator_degraded(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            watch_dir = root / "work"
            base_dir = root / ".pipeline"
            ...
            work_note = watch_dir / "2026-04-10-slice.md"
            _write_work_note(work_note, ["watcher_dispatch.py"])
            job = JobState.from_artifact("job-paste-fail-closed", str(work_note))
            job.status = JobStatus.VERIFY_PENDING
            job.artifact_hash = "hash-1"
            ...
            with mock.patch("watcher_dispatch.tmux_send_keys", return_value=False), \
                 mock.patch(
                     "watcher_core._shared_capture_pane_text",
                     return_value="› [Pasted Content 1024 chars]ROLE: verify",
                 ):
                job = core.sm._handle_verify_pending(job)

            self.assertEqual(job.status, JobStatus.VERIFY_PENDING)
            self.assertEqual(job.degraded_reason, CODEX_VERIFY_DISPATCH_FAILURE_LOOP_REASON)
            self.assertEqual(job.lane_note, CODEX_VERIFY_DISPATCH_FAILURE_LOOP_REASON)
            self.assertEqual(job.dispatch_stall_stage, DISPATCH_FAILED_SUBMIT_STAGE)
            self.assertGreaterEqual(job.dispatch_fail_count, job.retry_budget)
            self.assertEqual(job.dispatch_stall_count, 1)
            self.assertGreater(job.dispatch_stall_detected_at, 0.0)

    def test_failed_dispatch_exhausts_retry_budget_marks_operator_degraded(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            watch_dir = root / "work"
            base_dir = root / ".pipeline"
            ...
            work_note = watch_dir / "2026-04-10-slice.md"
            _write_work_note(work_note, ["watcher_core.py"])
            job = JobState.from_artifact("job-backoff-budget", str(work_note))
            job.status = JobStatus.VERIFY_PENDING
            job.artifact_hash = "hash-1"
            job.retry_budget = 2
            ...
            with mock.patch.object(core.sm, "send_keys", return_value=False), \
                 mock.patch.object(core.sm, "capture_pane_text", return_value="› pasted prompt placeholder"):
                job = core.sm._handle_verify_pending(job)

            self.assertEqual(job.status, JobStatus.VERIFY_PENDING)
            self.assertEqual(job.dispatch_fail_count, 2)
            self.assertEqual(job.degraded_reason, CODEX_VERIFY_DISPATCH_FAILURE_LOOP_REASON)
            self.assertEqual(job.lane_note, CODEX_VERIFY_DISPATCH_FAILURE_LOOP_REASON)
            self.assertEqual(job.dispatch_stall_stage, DISPATCH_FAILED_SUBMIT_STAGE)
            self.assertEqual(job.dispatch_stall_count, 1)
            ...
            with mock.patch.object(core.lease, "acquire", side_effect=AssertionError("lease should not be reacquired")), \
                 mock.patch.object(core.sm, "send_keys", side_effect=AssertionError("dispatch should not rerun")):
                job = core.sm._handle_verify_pending(job)
    ...
    def test_failed_dispatch_backoff_skips_immediate_redispatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(core.lease, "acquire", side_effect=AssertionError("lease should not be acquired")), \
                 mock.patch("watcher_dispatch.tmux_send_keys", side_effect=AssertionError("dispatch should not run")):
                job = core.sm._handle_verify_pending(job)
    ...
    def test_dispatch_stall_pasted_content_snapshot_skips_retry_after_backoff(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            work_note = watch_dir / "2026-04-10-slice.md"
            _write_work_note(work_note, ["watcher_core.py"])
            job = JobState.from_artifact("job-dispatch-stall-backoff", str(work_note))
            job.status = JobStatus.VERIFY_PENDING
            job.artifact_hash = "hash-1"
    ...
    def test_dispatch_stall_unsubmitted_pasted_prompt_fail_closes_after_backoff(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            watch_dir = root / "work"
            base_dir = root / ".pipeline"
            ...
            work_note = watch_dir / "2026-04-10-slice.md"
            _write_work_note(work_note, ["watcher_dispatch.py"])
            job = JobState.from_artifact("job-pasted-prompt-retry", str(work_note))
            job.status = JobStatus.VERIFY_PENDING
            job.artifact_hash = "hash-1"
            job.last_failed_dispatch_at = time.time() - 30
            job.last_failed_dispatch_snapshot = "› [Pasted Content 1024 chars]ROLE: followup"
            ...
            stale_prompt_line = "› [Pasted Content 1024 chars]ROLE: followup"

            with mock.patch("watcher_dispatch.tmux_send_keys", side_effect=AssertionError("dispatch should not run")) as send_keys, \
                 mock.patch("watcher_core._shared_capture_pane_text", return_value=stale_prompt_line):
                job = core.sm._handle_verify_pending(job)
    ...
    def test_requeued_failed_dispatch_forgets_dedupe_after_restart(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            watch_dir = root / "work"
            base_dir = root / ".pipeline"
            ...
            work_note = watch_dir / "2026-04-10-slice.md"
            _write_work_note(work_note, ["watcher_dispatch.py"])
            job = JobState.from_artifact("job-requeued-dedupe", str(work_note))
            job.status = JobStatus.VERIFY_PENDING
            job.artifact_hash = "hash-1"
            job.last_dispatch_at = time.time() - 60
            job.last_failed_dispatch_at = time.time() - 30
            job.last_failed_dispatch_snapshot = "previous failed dispatch snapshot"

            first_core = watcher_core.WatcherCore(
                {
                    "watch_dir": str(watch_dir),
                    "base_dir": str(base_dir),
                    "repo_root": str(root),
            ...
            first_core.dedupe.mark_dispatch(
                job.job_id,
                job.round,
                job.artifact_hash,
                "slot_verify",
            ...
            reloaded_core = watcher_core.WatcherCore(
                {
                    "watch_dir": str(watch_dir),
                    "base_dir": str(base_dir),
                    "repo_root": str(root),
            ...
            self.assertTrue(
                reloaded_core.dedupe.is_duplicate(
                    job.job_id,
                    job.round,
                    job.artifact_hash,
                    "slot_verify",
            ...
            with mock.patch("watcher_dispatch.tmux_send_keys", return_value=True) as send_keys, \
                 mock.patch("watcher_core._shared_capture_pane_text", return_value=stale_prompt_line):
                job = reloaded_core.sm._handle_verify_pending(job)

            self.assertEqual(job.status, JobStatus.VERIFY_RUNNING)
            dispatch_events = [
                json.loads(line)
                for line in (base_dir / "logs" / "experimental" / "dispatch.jsonl")
                .read_text(encoding="utf-8")
                .splitlines()
                if line.strip()
            ]
            self.assertEqual(dispatch_events[-2]["event"], "forget")
            self.assertEqual(dispatch_events[-1]["event"], "dispatch")
            send_keys.assert_called_once()
