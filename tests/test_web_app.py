...
class WebAppServiceTest(unittest.TestCase):
    ...
    def test_handle_chat_actual_entity_search_reload_preserves_active_context_source_paths(self) -> None:
        ...
        with TemporaryDirectory() as tmp_dir:
            ...
            first = service.handle_chat(
                {
                    "session_id": "actual-entity-reload-sp-session",
                    "user_text": "붉은사막에 대해 알려줘",
                    "provider": "mock",
                    "web_search_permission": "enabled",
                }
    ...
    def test_handle_chat_latest_update_mixed_source_follow_up_preserves_source_paths(self) -> None:
        ...
        active_context.source_paths에 두 source URL이 모두 보존됩니다."""
        with TemporaryDirectory() as tmp_dir:
            tmp_path = Path(tmp_dir)
            settings = AppSettings(
                sessions_dir=str(tmp_path / "sessions"),
            ...
            first = service.handle_chat(
                {
                    "session_id": "latest-mixed-followup-sp-session",
                    "user_text": "최신 스팀 할인 소식 검색해줘",
                    "provider": "mock",
                    "web_search_permission": "enabled",
                }
    ...
    def test_handle_chat_latest_update_mixed_source_reload_second_follow_up_preserves_response_origin_and_source_paths(self) -> None:
        """latest_update mixed-source → click reload → first follow-up → second follow-up에서
        response_origin과 active_context.source_paths가 유지됩니다."""
        with TemporaryDirectory() as tmp_dir:
            tmp_path = Path(tmp_dir)
            settings = AppSettings(
            ...
            first = service.handle_chat({"session_id": "latest-mixed-2fu-session", "user_text": "최신 스팀 할인 소식 검색해줘", "provider": "mock", "web_search_permission": "enabled"})
            self.assertTrue(first["ok"])
            record_id = first["session"]["web_search_history"][0]["record_id"]

            second = service.handle_chat({"session_id": "latest-mixed-2fu-session", "provider": "mock", "load_web_search_record_id": record_id})
            self.assertTrue(second["ok"])

            third = service.handle_chat({"session_id": "latest-mixed-2fu-session", "user_text": "이 검색 결과 요약해줘", "provider": "mock", "load_web_search_record_id": record_id})
            self.assertTrue(third["ok"])
    ...
    def test_handle_chat_latest_update_news_only_follow_up_preserves_source_paths(self) -> None:
        """news-only latest_update 검색 → load_web_search_record_id + user_text follow-up에서
        active_context.source_paths에 기사 URL이 모두 보존됩니다."""
        with TemporaryDirectory() as tmp_dir:
            tmp_path = Path(tmp_dir)
            settings = AppSettings(
                sessions_dir=str(tmp_path / "sessions"),
            ...
            first = service.handle_chat(
                {
                    "session_id": "latest-news-followup-sp-session",
                    "user_text": "기준금리 속보 검색해봐",
                    "provider": "mock",
                    "web_search_permission": "enabled",
                }
    ...
    def test_handle_chat_zero_strong_slot_entity_card_history_card_reload_follow_up_preserves_stored_response_origin(self) -> None:
        ...
        with TemporaryDirectory() as tmp_dir:
            ...
            first = service.handle_chat(
                {
                    "session_id": "zero-strong-followup-origin-session",
                    "user_text": "테스트게임에 대해 알려줘",
                    "provider": "mock",
                    "web_search_permission": "enabled",
                }
    ...
    def test_handle_chat_dual_probe_natural_reload_follow_up_preserves_response_origin(self) -> None:
        """dual-probe entity-card → 자연어 reload → follow-up에서
        response_origin의 answer_mode, verification_label, source_roles가 drift하지 않습니다."""
        with TemporaryDirectory() as tmp_dir:
            tmp_path = Path(tmp_dir)
            settings = AppSettings(
                sessions_dir=str(tmp_path / "sessions"),
            ...
            first = service.handle_chat(
                {
                    "session_id": "dual-probe-natural-followup-origin-session",
                    "user_text": "붉은사막에 대해 알려줘",
                    "provider": "mock",
                    "web_search_permission": "enabled",
                }
                ...
                "search_web": _FakeWebSearchTool(
                    {
                        ...
                        "붉은사막 공식 플랫폼": [
                            SimpleNamespace(title="붉은사막 | 플랫폼 - 공식", url="https://www.pearlabyss.com/ko-KR/Board/Detail?_boardNo=200", snippet="붉은사막은 PC와 콘솔 플랫폼으로 출시 예정이다."),
                        ],
                        "붉은사막 서비스 공식": [
                            SimpleNamespace(title="붉은사막 | 서비스 - 공식", url="https://www.pearlabyss.com/ko-KR/Board/Detail?_boardNo=300", snippet="붉은사막은 펄어비스가 운영하는 게임이다."),
                        ],
                    },
                    pages={
                        "https://www.pearlabyss.com/ko-KR/Board/Detail?_boardNo=200": {"title": "붉은사막 | 플랫폼 - 공식", "text": "붉은사막은 PC와 콘솔 플랫폼으로 출시 예정이며 펄어비스가 개발 중입니다.", "excerpt": "PC와 콘솔 플랫폼으로 출시 예정"},
                        "https://www.pearlabyss.com/ko-KR/Board/Detail?_boardNo=300": {"title": "붉은사막 | 서비스 - 공식", "text": "붉은사막은 펄어비스가 운영하는 게임이며 배급도 펄어비스가 담당합니다.", "excerpt": "붉은사막은 펄어비스가 운영하는 게임입니다."},
                    },
                ),
                "write_note": WriteNoteTool(allowed_roots=[str(tmp_path), str(tmp_path / "notes")]),
            }

            first = service.handle_chat({"session_id": "dual-probe-nat-2fu-session", "user_text": "붉은사막에 대해 알려줘", "provider": "mock", "web_search_permission": "enabled"})
            self.assertTrue(first["ok"])
            record_id = first["session"]["web_search_history"][0]["record_id"]

            second = service.handle_chat({"session_id": "dual-probe-nat-2fu-session", "user_text": "방금 검색한 결과 다시 보여줘", "provider": "mock"})
            self.assertTrue(second["ok"])

            third = service.handle_chat({"session_id": "dual-probe-nat-2fu-session", "user_text": "이 검색 결과 요약해줘", "provider": "mock", "load_web_search_record_id": record_id})
            self.assertTrue(third["ok"])

            fourth = service.handle_chat({"session_id": "dual-probe-nat-2fu-session", "user_text": "더 자세히 알려줘", "provider": "mock", "load_web_search_record_id": record_id})
            self.assertTrue(fourth["ok"])
            origin = fourth["response"]["response_origin"]
            self.assertEqual(origin["badge"], "WEB")
            self.assertEqual(origin["answer_mode"], "entity_card")
    ...
    def test_handle_chat_actual_entity_search_natural_reload_follow_up_preserves_source_paths(self) -> None:
        ...
        with TemporaryDirectory() as tmp_dir:
            ...
            first = service.handle_chat(
                {
                    "session_id": "actual-entity-natural-followup-sp-session",
                    "user_text": "붉은사막에 대해 알려줘",
                    "provider": "mock",
                    "web_search_permission": "enabled",
                }
    ...
    def test_handle_chat_actual_entity_search_natural_reload_follow_up_preserves_response_origin(self) -> None:
        ...
        with TemporaryDirectory() as tmp_dir:
            ...
            first = service.handle_chat(
                {
                    "session_id": "actual-entity-natural-followup-origin-session",
                    "user_text": "붉은사막에 대해 알려줘",
                    "provider": "mock",
                    "web_search_permission": "enabled",
                }
    ...
    def test_handle_chat_latest_update_single_source_reload_second_follow_up_preserves_response_origin_and_source_paths(self) -> None:
        """latest_update single-source → click reload → first follow-up → second follow-up에서
        response_origin과 active_context.source_paths가 유지됩니다."""
        with TemporaryDirectory() as tmp_dir:
            tmp_path = Path(tmp_dir)
            settings = AppSettings(
            ...
            first = service.handle_chat({"session_id": "latest-single-2fu-session", "user_text": "서울 날씨 검색해봐", "provider": "mock", "web_search_permission": "enabled"})
            self.assertTrue(first["ok"])
            record_id = first["session"]["web_search_history"][0]["record_id"]

            second = service.handle_chat({"session_id": "latest-single-2fu-session", "provider": "mock", "load_web_search_record_id": record_id})
            self.assertTrue(second["ok"])

            third = service.handle_chat({"session_id": "latest-single-2fu-session", "user_text": "이 검색 결과 요약해줘", "provider": "mock", "load_web_search_record_id": record_id})
            self.assertTrue(third["ok"])
    ...
    def test_handle_chat_latest_update_news_only_reload_second_follow_up_preserves_response_origin_and_source_paths(self) -> None:
        ...
        with TemporaryDirectory() as tmp_dir:
            ...
            first = service.handle_chat({"session_id": "latest-news-2fu-session", "user_text": "기준금리 속보 검색해봐", "provider": "mock", "web_search_permission": "enabled"})
            self.assertTrue(first["ok"])
            record_id = first["session"]["web_search_history"][0]["record_id"]

            second = service.handle_chat({"session_id": "latest-news-2fu-session", "provider": "mock", "load_web_search_record_id": record_id})
            self.assertTrue(second["ok"])

            third = service.handle_chat({"session_id": "latest-news-2fu-session", "user_text": "이 검색 결과 요약해줘", "provider": "mock", "load_web_search_record_id": record_id})
            self.assertTrue(third["ok"])
    ...
    def test_handle_chat_latest_update_noisy_source_excluded_after_natural_reload_follow_up(self) -> None:
        ...
        with TemporaryDirectory() as tmp_dir:
            ...
            first = service.handle_chat({"session_id": "latest-noisy-nat-fu-session", "user_text": "기준금리 속보 검색해봐", "provider": "mock", "web_search_permission": "enabled"})
            self.assertTrue(first["ok"])
            record_id = first["session"]["web_search_history"][0]["record_id"]

            second = service.handle_chat({"session_id": "latest-noisy-nat-fu-session", "user_text": "방금 검색한 결과 다시 보여줘", "provider": "mock"})
            self.assertTrue(second["ok"])
    ...
    def test_handle_chat_latest_update_noisy_source_excluded_after_natural_reload_second_follow_up(self) -> None:
        ...
        with TemporaryDirectory() as tmp_dir:
            ...
            first = service.handle_chat({"session_id": "latest-noisy-nat-2fu-session", "user_text": "기준금리 속보 검색해봐", "provider": "mock", "web_search_permission": "enabled"})
            self.assertTrue(first["ok"])
            record_id = first["session"]["web_search_history"][0]["record_id"]

            second = service.handle_chat({"session_id": "latest-noisy-nat-2fu-session", "user_text": "방금 검색한 결과 다시 보여줘", "provider": "mock"})
            self.assertTrue(second["ok"])

            third = service.handle_chat({"session_id": "latest-noisy-nat-2fu-session", "user_text": "이 검색 결과 요약해줘", "provider": "mock", "load_web_search_record_id": record_id})
            self.assertTrue(third["ok"])
    ...
    def test_handle_chat_entity_card_noisy_single_source_claim_excluded_after_natural_reload_follow_up(self) -> None:
        ...
        noisy single-source claim이 텍스트에 미노출되고 agreement-backed 사실 카드가 유지됩니다."""
        with TemporaryDirectory() as tmp_dir:
            tmp_path = Path(tmp_dir)
            settings = AppSettings(
                sessions_dir=str(tmp_path / "sessions"),
            ...
            noisy_page_text = "붉은사막 출시일은 2025년 12월로 예정되어 있다. 로그인 회원가입 구독 광고 전체메뉴 기사제보 이용약관 개인정보 쿠키 facebook twitter"
            ...
            service._build_tools = lambda: {
                ...
                "search_web": _FakeWebSearchTool(
                    [
                        SimpleNamespace(title="붉은사막 - 나무위키", url="https://namu.wiki/w/%EB%B6%89%EC%9D%80%EC%82%AC%EB%A7%89", snippet="붉은사막은 펄어비스가 개발 중인 오픈월드 액션 어드벤처 게임이다."),
                        SimpleNamespace(title="붉은사막 - 위키백과", url="https://ko.wikipedia.org/wiki/%EB%B6%89%EC%9D%80%EC%82%AC%EB%A7%89", snippet="붉은사막은 펄어비스가 개발 중인 오픈월드 액션 어드벤처 게임이다."),
                        SimpleNamespace(title="붉은사막 출시일 정보", url="https://blog.example.com/crimson-desert", snippet="붉은사막 출시일은 2025년 12월로 예정되어 있다."),
                    ],
                    pages={
                        "https://namu.wiki/w/%EB%B6%89%EC%9D%80%EC%82%AC%EB%A7%89": {"title": "붉은사막 - 나무위키", "text": "붉은사막은 펄어비스가 개발 중인 오픈월드 액션 어드벤처 게임이다."},
                        "https://ko.wikipedia.org/wiki/%EB%B6%89%EC%9D%80%EC%82%AC%EB%A7%89": {"title": "붉은사막 - 위키백과", "text": "붉은사막은 펄어비스가 개발 중인 오픈월드 액션 어드벤처 게임이다."},
                        "https://blog.example.com/crimson-desert": {"title": "붉은사막 출시일 정보", "text": noisy_page_text},
                    },
                ),
                "write_note": WriteNoteTool(allowed_roots=[str(tmp_path), str(tmp_path / "notes")]),
            }

            first = service.handle_chat({"session_id": "entity-noisy-nat-fu-session", "user_text": "붉은사막에 대해 알려줘", "provider": "mock", "web_search_permission": "enabled"})
            self.assertTrue(first["ok"])
            record_id = first["session"]["web_search_history"][0]["record_id"]

            second = service.handle_chat({"session_id": "entity-noisy-nat-fu-session", "user_text": "방금 검색한 결과 다시 보여줘", "provider": "mock"})
            self.assertTrue(second["ok"])
            self.assertEqual(second["response"]["actions_taken"], ["load_web_search_record"])

            third = service.handle_chat({"session_id": "entity-noisy-nat-fu-session", "user_text": "이 검색 결과 요약해줘", "provider": "mock", "load_web_search_record_id": record_id})
            self.assertTrue(third["ok"])
            origin = third["response"]["response_origin"]
            self.assertEqual(origin["badge"], "WEB")
            self.assertEqual(origin["answer_mode"], "entity_card")
    ...
    def test_handle_chat_entity_card_noisy_single_source_claim_excluded_after_natural_reload_second_follow_up(self) -> None:
        ...
        with TemporaryDirectory() as tmp_dir:
            ...
            first = service.handle_chat({"session_id": "entity-noisy-nat-2fu-session", "user_text": "붉은사막에 대해 알려줘", "provider": "mock", "web_search_permission": "enabled"})
            self.assertTrue(first["ok"])
            record_id = first["session"]["web_search_history"][0]["record_id"]

            second = service.handle_chat({"session_id": "entity-noisy-nat-2fu-session", "user_text": "방금 검색한 결과 다시 보여줘", "provider": "mock"})
            self.assertTrue(second["ok"])

            third = service.handle_chat({"session_id": "entity-noisy-nat-2fu-session", "user_text": "이 검색 결과 요약해줘", "provider": "mock", "load_web_search_record_id": record_id})
            self.assertTrue(third["ok"])

            fourth = service.handle_chat({"session_id": "entity-noisy-nat-2fu-session", "user_text": "더 자세히 알려줘", "provider": "mock", "load_web_search_record_id": record_id})
            self.assertTrue(fourth["ok"])
            origin = fourth["response"]["response_origin"]
            self.assertEqual(origin["badge"], "WEB")
            self.assertEqual(origin["answer_mode"], "entity_card")
    ...
    def test_handle_chat_entity_card_noisy_single_source_claim_excluded_after_history_card_reload_follow_up(self) -> None:
        ...
        noisy single-source claim이 텍스트에 미노출되고 entity_card contract가 유지됩니다."""
        with TemporaryDirectory() as tmp_dir:
            tmp_path = Path(tmp_dir)
            settings = AppSettings(
                sessions_dir=str(tmp_path / "sessions"),
            ...
            first = service.handle_chat({"session_id": "entity-noisy-click-fu-session", "user_text": "붉은사막에 대해 알려줘", "provider": "mock", "web_search_permission": "enabled"})
            self.assertTrue(first["ok"])
            record_id = first["session"]["web_search_history"][0]["record_id"]

            # click reload (show-only)
            second = service.handle_chat({"session_id": "entity-noisy-click-fu-session", "provider": "mock", "load_web_search_record_id": record_id})
            self.assertTrue(second["ok"])

            # first follow-up
            third = service.handle_chat({"session_id": "entity-noisy-click-fu-session", "user_text": "이 검색 결과 요약해줘", "provider": "mock", "load_web_search_record_id": record_id})
            self.assertTrue(third["ok"])
            origin = third["response"]["response_origin"]
            self.assertEqual(origin["badge"], "WEB")
            self.assertEqual(origin["answer_mode"], "entity_card")
    ...
    def test_handle_chat_entity_card_noisy_single_source_claim_excluded_after_history_card_reload_second_follow_up(self) -> None:
        ...
        with TemporaryDirectory() as tmp_dir:
            ...
            first = service.handle_chat({"session_id": "entity-noisy-click-2fu-session", "user_text": "붉은사막에 대해 알려줘", "provider": "mock", "web_search_permission": "enabled"})
            self.assertTrue(first["ok"])
            record_id = first["session"]["web_search_history"][0]["record_id"]

            second = service.handle_chat({"session_id": "entity-noisy-click-2fu-session", "provider": "mock", "load_web_search_record_id": record_id})
            self.assertTrue(second["ok"])

            third = service.handle_chat({"session_id": "entity-noisy-click-2fu-session", "user_text": "이 검색 결과 요약해줘", "provider": "mock", "load_web_search_record_id": record_id})
            self.assertTrue(third["ok"])

            fourth = service.handle_chat({"session_id": "entity-noisy-click-2fu-session", "user_text": "더 자세히 알려줘", "provider": "mock", "load_web_search_record_id": record_id})
            self.assertTrue(fourth["ok"])
            origin = fourth["response"]["response_origin"]
            self.assertEqual(origin["badge"], "WEB")
            self.assertEqual(origin["answer_mode"], "entity_card")
