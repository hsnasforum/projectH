...
## 변경 이유
- 최신 `/work`(`work/4/17/2026-04-17-sqlite-browser-core-chat-shell-parity.md`)는 sqlite browser gate 문서 4개만 갱신한 docs-only truth-sync 라운드였으므로, 이번 검증도 문서가 현재 코드/테스트 정의와 실제로 맞는지부터 좁게 다시 확인해야 했습니다.
- 같은 날 sqlite browser parity 계열 문서 동기화가 반복되고 있으므로, 이번 `/verify`는 문서 truth만 간결하게 확인하고 다음 라운드는 실제 sqlite 브라우저 재실행 묶음으로 바로 넘깁니다.

## 핵심 변경
- 최신 `/work`의 문서 동기화 주장은 현재 트리 기준으로 사실이었습니다. `README.md`, `docs/ACCEPTANCE_CRITERIA.md`, `docs/MILESTONES.md`, `docs/TASK_BACKLOG.md` 모두 sqlite browser smoke gate를 24개 시나리오 기준으로 설명하고 있었고, 추가된 core chat shell 2개 제목도 `e2e/tests/web-smoke.spec.mjs`의 실제 테스트 제목과 일치했습니다.
...
- `rg -n "SQLite Browser Smoke|스트리밍 중 취소 버튼이 동작합니다|일반 채팅 응답에는 source-type label이 붙지 않습니다|24" README.md docs/ACCEPTANCE_CRITERIA.md docs/MILESTONES.md docs/TASK_BACKLOG.md e2e/tests/web-smoke.spec.mjs`
- `sed -n '266,336p' README.md`
- `sed -n '1488,1528p' docs/ACCEPTANCE_CRITERIA.md`
- `sed -n '150,194p' docs/MILESTONES.md`
- `sed -n '819,833p' docs/TASK_BACKLOG.md`
...
- sqlite browser gate는 현재 문서상 24개 시나리오까지 정렬됐지만, claim-coverage panel과 history-card reload 같은 secondary-mode 브라우저 계약은 아직 sqlite backend로 확인되지 않았습니다.
