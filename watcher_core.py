...
def _dispatch_codex(pane_target: str, command: str) -> None:
    ...
    Always: exit current codex (if running) → launch new codex with prompt argument.
    paste-buffer into running Codex interactive is unreliable.
    """
    prompt_path = _write_prompt_file(command)

    # Check if codex is running or we're at bash
    ...
    if not last_line.endswith("$"):
        # Codex running → exit it first
        subprocess.run(["tmux", "send-keys", "-t", pane_target, "/exit", "Enter"], check=False, capture_output=True)
        time.sleep(2.0)
        # Verify we got bash back
        pane_lines2 = [l.strip() for l in _capture_pane_text(pane_target).strip().splitlines() if l.strip()]
        if pane_lines2 and not pane_lines2[-1].endswith("$"):
            # Still not bash, try harder
            subprocess.run(["tmux", "send-keys", "-t", pane_target, "C-c"], check=False, capture_output=True)
            time.sleep(0.5)
            subprocess.run(["tmux", "send-keys", "-t", pane_target, "C-c"], check=False, capture_output=True)
            time.sleep(1.0)

    # Launch new codex interactive with prompt
    shell_cmd = f"codex --ask-for-approval never \"$(cat '{prompt_path}')\""
    subprocess.run(
        ["tmux", "send-keys", "-t", pane_target, shell_cmd, "Enter"],
        check=True, capture_output=True,
    )
    log.info("codex dispatched (new instance): %s", prompt_path)
