...
즉 실행 후에는 `.exe`와 동일하게 GUI 안에서 Browse… / Apply / Setup/Check / Start / Guide를 사용할 수 있습니다.
...
상단 `OPS / GUIDE` 전환도 공통으로 제공합니다.
- `OPS`: 실행/상태/로그를 보는 운영 화면
- `GUIDE`: read-only canonical guide를 보는 화면 (`Export .md`로 내보내기 가능)

최근 launcher는 agent 카드의 `Quota:` 줄에 pane 텍스트 추정값뿐 아니라
...
`TOKENS` 패널은 추가로 아래 read/write maintenance action을 제공합니다.
- `FULL HISTORY`: 기존 `usage.db`를 유지한 채 전체 히스토리를 누적 보강합니다. dedup로 중복을 막습니다.
- `REBUILD DB`: 임시 DB로 전체 스캔을 다시 수행한 뒤 성공 시 기존 `usage.db`를 backup으로 남기고 교체합니다.
...
- `Top jobs` 줄에는 대표 link method, 대표 confidence, 저신뢰 비율(`low=a/b`)이 함께 표시됩니다.
...
- 수동 token action 실행 중에는 `Action:` 줄에 `0~100%` 진행률과 `scan x/y` 진행 상황이 함께 표시됩니다.
- `FULL HISTORY`는 기존 `file_state`가 있어도 `force-rescan`으로 전체 로그를 다시 훑고, dedup로 이미 적재된 usage는 중복 없이 건너뜁니다.
- 수동 token action 시작 전 기존 collector가 살아 있으면 `stopping collector` 단계가 먼저 표시되고, 완료 후에는 `starting collector` 단계가 표시됩니다.
- 아직 `usage.db`가 없는 project에서 `FULL HISTORY` / `REBUILD DB`를 실행하면, 스캔이 시작되기 전까지 `Collector/Today/Agents/Top jobs`는 `loading...`으로 표시됩니다.
- 오늘 usage가 없지만 최근 usage가 남아 있는 경우, `TOKENS` 패널은 비우지 않고 최신 usage day를 기준으로 표시합니다.
- agent 카드의 `Quota:` usage summary는 첫 화면 표시를 막지 않도록 비동기 캐시로 갱신됩니다.
...
| project path | `.cmd`의 `WSL_PROJECT`로 기본값 주입 + GUI에서 Browse…/Apply/Recent 가능 | GUI 내 Browse…/Apply/Recent |
...
- **TOKENS 패널에서 Full History / Rebuild DB 실행 중**: collector가 잠시 멈췄다가 작업 후 다시 시작될 수 있음
